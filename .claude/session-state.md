...
### ★ 커밋했지만 **미배포** (자고 일어나서 `/배포` 한 마디면 나갑니다)
- **저장 후 보기 화면의 표 테두리 끊김** (`c27620a9`) — 읽기 전용의 `display:block` 이 원인이었다.
  ...
  ※ 이 커밋만 운영에 없다. 나머지는 v1.50.0 으로 이미 나가 있다.
...
- **대기열 30번** — 저장 후 보기 경로를 카나리로 묶는 방법. 이번에 두 번 실패했다(편집 초안 상태에서
  이탈하면 17분 행 · 순서를 바꾸니 편집 판정이 깨짐). 스위트를 둘로 가르는 게 맞는지 Fable 판단 필요.
