## 현재 작업 상태
**마지막 업데이트:** 2026-09-13 10:40 UTC
**작업 상태:** **완료 · 운영 배포 완료** (`c5cfef04`, **v1.48.22**) — 미배포 없음(dev == 운영)

### 이번 세션에서 끝낸 것
- **공유 링크가 열어 둔 채로도 최신을 보여준다** (Irene: "수정했는데도 링크에서 이전 내용")
  서버는 계속 신선했다(API `no-store` · OG 는 DB 직독). 멈춰 있던 것은 **화면** —
  공개 페이지 **12종이 전부 mount 때 한 번 읽고 끝**이었다.
  `hooks/usePublicRevalidate` 한 벌(탭 복귀 재조회 + 보이는 동안 60초 폴링, 숨으면 정지).
  **입력 중에는 멈춘다**: 서명은 `phase==='review'` 일 때만 · 청구서는 모달 열림 중 정지 ·
  비밀번호로 연 페이지는 `pwRef` 로 **같은 자격**으로 다시 읽는다(갱신이 곧 잠금이 되지 않게)
- **Q docs 상세 — 서브헤더까지 고정** (Irene: "헤더가 위로 자꾸 올라가")
  밴드1+밴드2 를 `StickyBands`(`data-testid="docs-detail-bands"`) 한 묶음으로 감쌌다.
  `top: 60px` 같은 숫자를 주지 않는다 — 폰에서 제목이 길면 밴드1 이 자라 그 숫자가 거짓이 된다.
  2026-09-08 의 "제목 밴드까지만" 결정을 **Irene 지시로 되돌린 것**
- **확인 필요 → Q sale 상담 우측패널** (Irene 지시)
  서버가 `drawer.inquiry` 를 통째로 싣고, `utils/saleInquiryView`(행→패널 값)·
  `services/saleRegister`(등록+초대)를 추출해 상담 목록과 Todo 가 **같은 함수**를 부른다
- **[고객으로 등록] 발송 확인** — 이 버튼은 초대 메일을 보낸다. `ConfirmDialog` 로 **주소를 적어** 묻는다.
  패널·목록 행 **둘 다**. (대기열 24번의 "Fable 이 봐야 할 것 4번" 을 닫은 것)
- **메일 별칭 등록** — 배포 게이트가 막혀서: 워크스페이스 3 에 `help@gitconsulting.group` 이
  90일간 10건 도착했는데 "우리 주소" 에 없었다(→ 명백한 요청이 '확인 권장' 으로 강등된다, #221 계열).
  **Irene 승인 후** `email_account_aliases` 에 등록(account 32 · biz 3). health-check 44/44 복구

### 검증 (★ Fable 미검증 — 자체 검증)
**Fable 을 2회 띄웠고 2회 모두 HTTP 429(한도 초과)로 실패.** 대기열 **24·25번** 에 남겼다.
"통과" 가 아니다. 마커는 `by:"unavailable"`.
- build EXIT 0 · `error TS` 0 · guard-invariants 49/50(1건 문서 신선도 **경고**) · health-check **44/44**
- 실HTTP: 상담 목록 vs 확인필요 **불일치 필드 0**, id 동일
  (dev 에 상담 데이터가 **0건**이라 게스트 문의 픽스처를 만들어 쟀다 — 빈 fixture 로 "0건=정상" 금지)
- 실브라우저: 패널 열림(클릭 전 0개 음성 대조군) · Q docs 밴드 3폭 제자리+보임(+양성 대조군에서 뒤집힘) ·
  공개 링크 탭 복귀 후 반영 · **서명 단계 재조회 0건**(review 1건) · 등록 확인창 **확인 전 0 → 후 1**
- 픽스처·테스트 스크립트 전부 삭제(잔존 0)

### 이번 세션의 측정 사고 2건 (다음에 또 당한다)
1. **짧은 문서로 재서 "헤더가 올라간다" 는 거짓 결과** — 본문이 짧으면 바깥 셸이 스크롤해
   밴드가 같이 올라간다. **긴 문서 픽스처**로 다시 재서 잡았다
2. **패널과 확인창이 둘 다 `[aria-modal="true"]`** — 첫 판정이 패널을 확인창으로 오인해
   거짓 통과 직전이었다. 제목 문구로 좁혀 다시 쟀다

### ★ 동시 세션 오염 (주의)
작업 내내 **다른 세션이 같은 파일을 편집**했다. `TodoPage.tsx` 에 중복 임포트와
**`onRegister` 없이 패널만 띄우는 중복 블록**(= 눌러도 아무 일 없는 등록 버튼)이 들어와 2회 제거했다.
최종 diff 가 의도한 것만 담고 있는지 제3자 대조가 필요하다.

### 다음 할 일
1. **Fable 한도 풀리면 `docs/FABLE_GATE_QUEUE.md` 12~25번을 한 라운드로** (쪼개 올리지 않는다)
2. **확인 필요 합산 모드 중복 계수** — `?business_id=` 없이 부르면 `sale-inbox-${it.id}` 에
   워크스페이스 축이 없다. 워크스페이스가 하나면 **드러나지 않는** 종류
3. **Q sale 권한/역할별 노출** — `collectSale` 은 담당 귀속을 적용하지 않는다.
   `requireMenu('qsale')` 없는 멤버·고객 역할에게 문의가 보이는지 실호출 판정
4. **Q docs 편집 모드 헤더 고정** — 보기 모드만 쟀다. 편집 밴드2(`MetaRow`)는 wrap 을 허용해
   폰에서 두 줄이 될 수 있다
5. **숨은 탭에서 60초 폴링이 실제로 멈추는지** 요청 카운트로 확인(공개 라우트는 rate-limit 만이 방어선)
6. **비밀번호 보호 공개 페이지**가 재조회로 잠금 화면에 되돌아가지 않는지 실측
7. Q sale 우측 패널 URL 싱크(`?client=`) · `clients.sales_source` 에 'visit' 추가 여부(운영 ENUM ALTER = R=1)

### 서명 관련 — Irene 에게 답한 내용 (다음 세션에서 이어질 수 있음)
- **본문에 직접 넣은 서명란(`서명: ______`)에는 서명이 들어가지 않는다.** 에디터에 서명란 노드가 없다
- 절차: Q docs 상세 [서명 받기] → 서명자 이메일+이름 N행 → 각자 `/sign/<토큰>` 메일 →
  본문 읽기 → **이메일 6자리 OTP** → **캔버스 서명** → 동의 → 서명/거절
- 서명 이미지는 **서명 진행 표**·`받은 서명` 에 남고 **PDF 에는 안 들어간다**(PDF=제목+본문)
- ★ **서명 요청을 보내면 그 post 는 수정·삭제·첨부가 409 로 잠긴다**(`blockIfSigned`).
  본문 확정 후에 요청을 보내야 한다
- 대상 문서: 운영 `K-DINE with MIN 추가 매장 가맹 및 운영계약서`(현재 ver.03), 서명자 3명
  (KIM MI JUNG / KIM SU JIN / SHIM KYUTAE)
- Irene 이 보낸 `planq.kr/public/posts/...` 는 **읽기 전용 공유 링크**(서명 링크 아님)
...
- ~~운영 피드백 #409 · #410 답글~~ ✅ 2026-09-12 22:29 작성 + status=done (고친 내용·남은 iOS 앱 재빌드 안내 포함)

---

## 2026-09-13 새벽 — 발송 확인 · 공용 모달 접근성 · **초대 토큰 비노출** (배포 완료)

배포: `c1c1b180` → 운영 **v1.48.21** · 358초 · 스키마 변경 0 · 개발현황 id=88 · 신고 #409·#410 닫음.
헬스 44/44 · 가드 49/50 · 빌드 EXIT 0 / error TS 0. **Fable 미검증(자체 검증)** — 오늘 15회 전부 429.

### 무엇을 했나
1. **메일이 나가기 전에 묻는다** (Irene: "확인을 받아") — Q sale 패널 [초대] · 고객 [초대 재발송].
   확인창이 **보낼 주소를 보여준다**. 재발송은 주소가 없으면 비활성 + 이유(종전엔 눌리고 서버가 400 으로
   조용히 거절 = 사용자에겐 "아무 일도 안 일어남").
2. **공용 모달 접근성** (`components/UI/Modal.tsx`) — `role="dialog"`·`aria-modal`·`aria-label` +
   scrollLock·escapeStack·focusTrap. 훅은 early return **위**. ConfirmDialog 포함 **15곳**이 같이 고쳐졌다.
   ★ 이게 없어서 하니스가 확인창을 **스코핑조차 못 했다**(§17 위반 상태였다).
3. **`invite_token` 이 고객 목록·상세 응답에 실려 나갔다** — `c.toJSON()` 을 그대로 내보낸 탓.
   그 토큰은 `register` 가 "그 고객으로" 계정을 붙이는 자격증명이다. `stripClientSecrets()` 로 7곳에서 내렸고
   **health-check 새 카테고리 `secrets`** 로 게이트에 붙였다(응답 원문 문자열 판정 + 빈 응답 거짓통과 방지).
   **운영 노출 실태(배포 후 실측): 고객 7명 중 토큰 보유 1건 · 미수락 0건 → 살아 있는 초대 링크 없음 =
   회전(재발급) 불필요.** 이 숫자를 확인했으므로 queue 19번의 판단 1은 사실상 해소됐다.

### 하니스가 **거짓 사유**로 빨간불이었다 (중요)
`toggles` 의 일정 "하루 종일" 실패는 **제품 정상 + 검사 눈가림**이었다 —
출근 자동기록 알림(StandardModal)이 전면 백드롭으로 클릭을 먹었고, 판정은 "저장이 10초 안에 안 갔다"
(= 제품 고장과 같은 말)고 보고했다. 라벨 글자를 누르면 `all_day:true` 가 정확히 저장됐다.
→ ① `clickAndSave` 가 **누르기 전에 그 좌표가 내 것인지** 본다(덮여 있으면 그 사유로 보고).
→ ② `scripts/e2e/lib/browser.js` **`dismissBlockers(page)`** 신설. `goto()` 에서 자동으로 부르지 않는다 —
   `canary-modal-top` 은 그 알림 자체가 검사 대상이다.
→ ③ 좌표 판정은 **클릭과 같은 렌즈**로(puppeteer 는 누르기 전 스크롤한다). 안 맞춰서 3건을 오탐했다.

### 자체 검증 숫자
초대 확인창 6/6 · 재발송 7/7(확인 전 POST **0건** / 확인 후 **정확히 1건** · `reinvite_count 0→1`) ·
모달 접근성 9/9(Tab 12회 모두 안 · 중첩 Esc 스택 · 잠금 누수 0) · 토큰 비노출(목록 16행 0 · 원문 0 ·
**되살리면 빨간불**) · 토글 6/6.

### 다음
- Fable 한도가 풀리면 `docs/FABLE_GATE_QUEUE.md` **12~19번을 한 라운드로** (쪼개 올리지 않는다).
  남은 판단: 다른 초대 지점(멤버·프로젝트 고객)에도 확인을 붙일 것인가 · 공용 모달 Esc 가 곤란한 창이 있는가.
- Q sale 우측 패널 URL 싱크(`?client=`) 미구현.
- **미배포 없음** — 이 시점에서 dev == 운영.
...
## 부록 — 2026-09-13 10:40 UTC Git 상태 (v1.48.22 배포 시점)

- **운영 배포 커밋:** `c5cfef04` (chore(release): v1.48.22 — 개발 현황 · 릴리즈노트)
- **직전 운영 커밋:** `c1c1b180` → 이번 배포에 **14개 커밋**이 실렸다
- **working tree:** 깨끗(미커밋 0) · dev == 운영
- 이번 세션 커밋: `21f96430`(구현) · `c5cfef04`(버전·현황·릴리즈노트)
- 릴리즈노트: `docs/release-notes/v1.48.22.json` → 운영 **발행 완료** (`/insights/update-1-48-22`)
- 개발 현황: `docs/dev-status/next.json` → 운영 발행 완료 (id=89, commit_to=c5cfef04)
- 롤백: `ssh irene@87.106.78.146 'tar -xzf /opt/planq/backups/20260913_103157/backend.tar.gz -C /opt/planq && pm2 reload planq-prod-backend'`
- **Fable 게이트 마커:** `by:"unavailable"` (2회 연속 429) — 대기열 24·25번
