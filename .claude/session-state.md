...
### 접수 (2026-09-14 · 장부에 없던 것을 지금 넣었다)
- **프로젝트 탭마다 레이아웃이 갈린다** — Irene: *"프로젝트 탭마다 레이아웃 바뀌는 거"*.
  신고 장부(feedback_items)·개발현황 어디에도 없었다. 지금 backlog 에 넣었다. 실측부터.
- **탭 메뉴 추가(사용자가 프로젝트 탭을 추가)** — 현재 탭은 코드 고정이다. 범위·권한·게스트 노출을 먼저 정해야 한다.
