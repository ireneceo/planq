...
- **구현 진행(미배포):**
  ...
  - ⏳ **§6-B** 드로어 연속성(이월 배너+이번주/전체 시간 토글) — TaskDetailDrawer.tsx 신규 + 백엔드 task별 주간 focus 집계 필요
  - ⏳ **§6-C 델타** 주별 델타(carry-in 분리, 월요일 0 시작) — daily-progress 주시작 baseline 차감
  - ⏳ **U4** 단조완화(되돌림 ↓마커) — 프론트 SVG, chart 회귀주의
- **검증:** A1·A2·U5 배포 라이브. §6-C 스코핑 빌드EXIT0·TS0/스코핑·보안 E2E/운영 read-only 153.6→8.8/헬스29/29/서빙200. **미배포: `9381d53`(다음 `/배포`).**
