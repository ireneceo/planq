## 현재 작업 상태
**마지막 업데이트:** 2026-09-10 밤 (Opus 5, 1M)
**운영:** **v1.48.20** 배포 완료 (`9fb173e9`) — 오늘 배포 3회 (v1.48.18 · 19 · 20)
**진행 중:** 신고 6건 수리 완료 · **커밋 8건 미배포** · Fable 재검증 대기

---

## 저녁 세션 (2026-09-10) — Irene 신고 6건, 전부 커밋 · **미배포**

배포 안 했다. `/배포` 지시를 아직 못 받았다.

| 커밋 | 신고 | 실제 원인 |
|---|---|---|
| `0cf5d5eb` | 아이폰 구글로그인 "엉망진창" | 네이티브 실패가 **302** 로 와서 SFSafariViewController 가 무시 → 팝오버 안에 웹 세션이 생겼다. OAuth state 가 메모리 Map 이라 PM2 재시작에 사라지던 것도 DB 로 |
| `e494596b` | 내 드라이브에서 아무것도 못 고름 | DocsView 가 하나뿐이라 **공유 드라이브만** 그렸다. 구글 문서는 `files.export` 로 (docx/xlsx/pptx/pdf) |
| `36dc862c` | 팝업 하단만 직각 | 09-07 에 StandardModal 만 고쳤고 **복사본 7개**가 그대로. `modalShell.ts` 로 규격 통합 + `--category=modalradius` 가드 신설 |
| `1146a0c6` | 팝아웃 진행버튼 열 안 맞음·재개≠시작 구분 없음 | 조건부 앞쪽 배치 + `running` 불리언 하나. **동작도** 갈라져 `/focus/start` 를 불러 세션을 갈아탔다 → `/focus/resume` |
| `29c8fa67` | 폴더 아이콘 무반응 · AI 업무추가 그룹 누락 | 메뉴는 **열리는데** 셀 `overflow:hidden` 이 잘라 안 보였다(→portal). AI 만 이름 문자열에 기대고 수동 3경로는 id 를 보냈다 |
| `996c4e16` | 프로젝트 메일 이상한 목록 | `?project=` 를 useState 초기값으로 **한 번만** 읽어 keep-alive 탭에서 안 걸렸다. 스레드 PUT 에 소유권 검사도 없었다 |
| `18181508` | 외부열람 허접 · 팝업 문구 이해 불가 | 폭 상한 **0건** · 알림 신청자가 **링크로 둔갑**(kind 필터 누락) · 회수가 두 벌로 갈라져 자식 링크를 안 닫았다 |
| `bcf41f49` | (Fable FAIL 수리) | 내가 **없는 컬럼**(`guest_email`)을 읽어 사람 칸이 전원 빈 값 · god-file 래칫을 좁은 가드로 놓침 |

### ★ 이번에도 Fable 게이트가 FAIL 을 냈고 둘 다 진짜였다
- **`--category` 로만 가드를 돌렸다** → god-file 빨간불을 못 보고 "가드 통과"라고 커밋에 적었다.
  memory `feedback_category_guard_hides_other_failures` 로 박제.
- **회수는 한 함수로 모으고 직렬화는 손으로 다시 썼다** → 없는 컬럼. 고치려던 칸이 비어 나갔다.
  부수로 `l.created_at` 이 언제나 undefined 인 것도 발견(Sequelize 속성명은 `createdAt`).

### 승인 대기 3건 (규모가 커서 시작 안 함)
1. **[AI로 일정 수정하기]** — 배관(services/actions 행동 계층)은 있고 **도구가 없다**(cue_tools 6개 전부 단건).
   권고: ①전체 N일 이동 ②목표 마감 비례 압축 둘부터. 핵심은 미리보기 + **묶음 되돌리기**.
2. **메일 자동연결** — `email_threads.project_id` 를 쓰는 코드가 **0곳**(dev 4,879건 중 연결 0건).
   고객 자동매칭은 받은메일+새스레드+발신자 완전일치일 때만. **고객 중심 vs 프로젝트 중심** 결정 필요.
3. **`PublicPageShell`** — 게스트 6파일이 공용 UI 컴포넌트 **0개**, 자체 styled 118개.
   `PageShell` 이 `UserChip` 을 무조건 그려 무로그인에 못 쓴다. 공개 11 + 게스트 6 = 17화면 통합.

---

## 오늘 (2026-09-10) 한 것 — 배포 3회

| 버전 | 내용 |
|---|---|
| v1.48.18 | 서버가 만든 문구의 언어(통계·Cue 카드 33코드) · 통화/타임존 하드코딩 · 죽은 링크 · `req.user.language` 부재 |
| v1.48.19 | Google Picker · 감사 기록 입구 단일화(21건) · 청구서 `issued_at` 훅 |
| v1.48.20 | **아이폰 앱 구글 로그인** · 메일 모바일 폭 맞춤 · 배포 스크립트 수정 |

### ★ Fable 게이트가 세 번 FAIL 을 냈고 **세 번 다 진짜 결함**이었다
1. **Picker 배포 절차 누락** — rsync 가 dev-backend 만 보내 **운영에 nginx 스크립트가 없었다.**
   그리고 내 문구가 "광고 차단 확장이 원인일 수 있습니다" 라고 **우리 잘못을 사용자 탓으로** 말했다.
2. **메일 폭 맞춤** — 내가 주석에 적은 근거 둘이 모두 틀렸다:
   ①"zoom 은 높이에 반영된다" → scrollHeight 는 축소 **전** 단위라 545px 빈 공간
   ②"데스크탑 무변경" → iframe 안 `innerWidth` 는 기기 폭이 아니라 **읽기 패널 폭**
3. **메일 제목 mojibake** — 내 원인 진단이 틀렸다. Fable 이 raw TLS 소켓으로 읽어 보니
   **Gmail 이 들고 있는 바이트가 이미 `EF BF BD`** — 우리가 깨뜨린 게 아니고 되살릴 수도 없다.
   내가 만든 복구 코드엔 일본어·중국어 제목이 **쓰레기 한글로 저장**되는 빈틈까지 있었다 → **되돌렸다.**

### ★ 게이트가 6일간 안 울린 두 구멍 — 둘 다 막았다
- **커밋하면 침묵** — 훅이 미커밋 변경만 봤다. 이제 **마지막 통과 이후 소스 커밋**도 센다.
  (2026-09-07 에도 같은 구멍이었는데 그때 안 고쳐 재발했다)
- **`.fable-gate-skip` 이 18일 켜진 채 잊혔다** — 이제 **24시간 자동 만료**.
- Irene 지시 반영: *"페이블 토큰 있을 때 페이블 쓰고 아닐 때 스킵하고"* →
  못 띄우면 대기열 등재 + `by:"unavailable"` 마커. **스킵해도 흔적이 남는다.**

---

## ▶ 지금 진행 중 (미커밋) — OAuth 상태 저장소 DB 이전

**R=1** (운영 스키마 변경 + 인증 경로). Fable 검증 대기.

OAuth 상태 셋이 메모리 `Map` 이라 **PM2 재시작마다 사라졌다**:
`oauthPairing.store`(10분, 진행 중 로그인) · `usedNativeCodes`(2분, **replay 차단**) ·
`confirmStash`(5분, 외부 연동 — **이번 범위 제외**, 별도 검증 필요).

- 신규 테이블 `ephemeral_tokens` (kind ENUM · token_key · payload JSON · expires_at)
- 신규 `services/ephemeralStore.js` — Map 과 같은 모양, 비동기. sweeper 1분
- **코드는 해시로만 저장**(`sha256(pairId::code)`) — DB 는 백업에도 남는다
- 마이그레이션 `scripts/migrate-ephemeral-tokens.js`(멱등), 배포 스크립트 406행 등록
- 자체 검증 23/23 — ★ **`pm2 restart` 를 검사 중간에 끼워** 재시작 뒤 claim 성공·replay 차단 유지 확인

---

## ★ Irene 이 해야 하는 것
1. **dev nginx 헤더** — `sudo /opt/planq/scripts/apply-nginx-security-headers.sh dev`
   (운영은 09-10 적용 완료 — 라이브 확인함. dev 만 남았다)
2. **아이폰 앱 재설치** — v1.48.18 의 알림 소리 수정이 적용되려면 필요
   (iOS 는 최초 인증의 옵션 집합을 고정한다)

## 남은 일 (결정 대기)
- **프로젝트 '주요 이슈' 자동화** — 지금 100% 수동. 설계 2단(①기계가 아는 사실부터 LLM 0
  ②대화 결론은 후보로만) 올려 뒀다. 규모 대
- **#407 나머지 두 축**(대화방 접근 좁히기 · 본문 암호화) — 비가역
- **#381·#382 Q sale** — 설계 v2 재검토 먼저
- **`confirmStash` DB 이전** — 위 작업의 나머지 한 조각
- **Finance 탭 '고정비'·'지출' 이 구조적으로 0** — OverheadItem·ProjectExpense 를 만드는
  라우트도 화면도 없다. 별건
- **메일 제목 깨짐은 우리가 못 고친다** — Gmail 도착 전에 이미 깨져 온다(Fable 실측)
