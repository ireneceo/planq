...
**작업 상태:** 배포 완료. Irene 신고 8건 전부 처리(§✅).
**다음 세션 첫 할 일 = 실기기 확인 2건** — 하단 버튼 안 잘리는지 · 좌측 메뉴 로고 보이는지.
(브라우저는 safe-area=0 이라 이 둘만은 코드 검증까지만 됐다.)

---

## ✅ Irene 신고 8건 — 7건 해결·배포 (2026-08-25 심야)

| # | 신고 | 처리 |
|---|---|---|
| 1 | 콘텐츠 하단 흰 여백이 **버튼을 덮음** | ✅ 원인은 여백이 아니라 **덮개**였다. `body::after` 가 `z-index:98` 로 콘텐츠 위에 흰 막. → 뒤로(z-index:0) + MainContent 가 safe-bottom 만큼 자리 양보 |
| 2 | 좌측 메뉴 로고가 상태바 밑으로 | ✅ 사이드바에 아래쪽 safe-area 만 있고 **위쪽이 빠져 있었다**. MobileHeader 와 같은 `padding-top: var(--pq-safe-top)` |
| 3 | Q Note 헤더 2행 필요 | ✅ ≤640px 1행 제목 / 2행 액션 |
| 4 | 요약 접기/펴기 | ✅ Q Note — 제목 토글, 접힘 상태 기기 기억 |
| 5 | Q docs 서브헤더 엉망 | ✅ ≤640px 두 줄 분리 (좌우가 서로 밀던 것) |
| 6 | Q Mail 뒤로가기 2개 | ✅ 좁은 화면에서 박스형 숨김 |
| 7 | 캘린더 상세 뒤로가기 없음 | ✅ 공용 `DetailDrawer.Header` 에 추가 → 일정·업무·고객 상세 동시 통일 |
| 8 | 프로젝트 히스토리 클릭 불가 | ✅ 메모·파일·프로젝트 상태에 이동 경로 부여 |

> ⚠️ **1·2 는 실기기에서만 검증 가능하다** — 브라우저는 `env(safe-area-inset-*)` 가 0 이라
> 규칙이 있어도 효과가 안 보인다. 빌드 산출물에서 규칙 존재는 확인했다
> (`.pq-native body:after{...z-index:0...}` · `padding-top:var(--pq-safe-top)`).
> **다음 세션 첫 확인 항목: Irene 앱에서 하단 버튼이 안 잘리는지 · 좌측 메뉴 로고가 보이는지.**
