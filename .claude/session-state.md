...
**작업 상태:** dev 반영 완료 · **미커밋 43파일** · **운영 미배포** (Irene 퇴근 — 다음 세션 계속)
...
2. **★ 배포 전에 반드시 확인할 것 — 내가 검증하지 못한 수정이 하나 있다.**
   `services/seoArtifacts.js` 의 **빈 상위 디렉터리 rmdir**. 내 스크래치 하니스가 생성기를 못 돌려
   (`ok=false`) **미측정**이다. 안 들으면 배포 후 `planq.kr/wiki/` 가 **403**(빈 폴더 → nginx 403,
   Fable 이 운영 `/sounds/` 403 으로 실증)이 된다. 반대로 rmdir 이 과하면 **배포마다 남의 파일을
   지운다(비가역)** — 음성 대조군까지 보고 배포하라.
...
### Git 상태 (오후 세션 종료 시점)
- `main` · HEAD `badec019` · **미커밋 43파일** · 운영 **1.63.5**(오후분 미배포)
