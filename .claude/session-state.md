## 현재 작업 상태
**마지막 업데이트:** 2026-09-13 00:40 UTC
**작업 상태:** **완료 · 운영 배포 완료** (`c1c1b180`, v1.48.21 유지) — 미배포 없음(dev == 운영)

### 이번 세션에서 끝낸 것
- **발송 전 확인** — [초대]·[초대 재발송] 이 **보낼 주소를 보여주고** 묻는다 (Irene: "확인을 받아")
- **공용 모달 접근성** — `UI/Modal.tsx` 에 ARIA + 훅 3종. 쓰는 곳 **15곳** 동시 수리
- **`invite_token` 비노출** — 고객 목록·상세 응답에서 내리고 health-check `secrets` 로 게이트화.
  운영 실측: 토큰 보유 1건 · 미수락 0건 → **회전 불필요**
- **하니스 2건 수리** — 가려진 클릭을 그 사유로 보고 · `dismissBlockers()`
- 앞 라운드(같은 세션, 이미 배포): Q sale 상담 리스트 구조·액션 연결 · 업무 추가 폼 4→1 · 메일 오수집

### 다음 할 일
1. **Fable 한도 풀리면 `docs/FABLE_GATE_QUEUE.md` 12~19번을 한 라운드로** (쪼개 올리지 않는다).
   남은 판단: 멤버·프로젝트 고객 초대에도 확인을 붙일지 · 공용 모달 Esc 가 곤란한 창이 있는지
2. **Q sale 우측 패널 URL 싱크(`?client=`)** — 새로고침·공유에서 맥락이 사라진다
3. `clients.sales_source` 에 'visit' 추가 여부 (운영 ENUM ALTER = R=1 이라 미실행)
4. dev 플랜 게이트(422)로 못 잰 신규 문의 생성 경로 — 운영 정책 확인 필요
