...
### ★ 다음 섹션 최우선 — Q sale 우측 패널 재설계 (2026-09-13 Irene 지시, 미착수)
**설계·신고 원문·코드 위치는 `docs/Q_SALE_DESIGN.md` §N 에 전부 적어 뒀다. 거기서 시작한다.**
요지: **어디서 열든 우측 패널은 "고객에서 연 창" 하나다.** 상담에서 열었다는 사실은 상단 **문의 정보 박스**
하나로만 표현하고 나머지는 고객 패널 그대로 — 액션 버튼도 같다. 저장·응대의 단위는 **고객**이고
상담 종류(메일·채팅·전화·게스트링크)는 표시 축일 뿐이다.
- 지금은 `ClientPanel` 이 `isInquiry` 로 **두 벌**로 갈라져 문의로 열면 고객 기능이 통째로 없다.
  2026-09-13 에 확인필요에서도 이 패널을 열게 한 것은 **갈라진 쪽을 한 곳 더 늘린 것**이다.
- 신고 12건 ↔ 코드 위치 표가 §N.2 에 있다(채팅 라벨 이분법 `SaleInboxList.tsx:266` · 되돌리기 ·
  등록 시 이메일 · 전체 히스토리 보기 · 전체 프로필 전부를 패널에 · 이메일↔프로젝트 다대다 ·
  제목/본문 이름 중복 `ClientPanel.tsx:244-260` vs `268-310` · 푸터 ActionButton **19곳** 레이아웃).
- **우측 드로어 스크롤 잠금 해제**(§N.4) — Irene: *"어둡게 했으면 됐지. 리스트들이든 뭐든 스크롤 반응은 막지마."*
  프리미티브 `DetailDrawer` 한 곳 + CLAUDE.md "body 스크롤 잠금 필수" 문장도 같이 고친다(안 고치면 되돌아간다).
  **가운데 모달은 제외** — 전면을 덮는 창은 뒤가 움직이면 안 된다.
- ★ **구현 전 Irene 확인 1건**: 미등록 문의에는 `clients` 행이 없다. ①임시 고객 뷰 ②`prospect` 자동 생성
  ③읽기 전용 골격 중 무엇으로 갈지. ②는 한도·원장에 영향이 있어 임의로 정하지 않는다.
