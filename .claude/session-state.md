...
### 다음 할 일
-1. **🚀 미배포 3건 운영 반영 대기** — (a) 멤버 닉네임 누락 전수 수정(04183bb, 백엔드) (b) 멤버 소속 채팅 hover(05b21b4) (c) 멤버 소속 업무상세 드로어(9f3b9cf). Irene `/배포` 대기.
   - 소속 표시 surface 현황: ✅채팅 UserInfoPopover · ✅TaskDetailDrawer 담당자 · ✅OrgPage(D1 기존). 업무 리스트 행(밀집)·계정 프로필은 미적용(노이즈 우려, 필요시 추가).
0. **D4 보안등급 확장** — ✅ 운영 배포 완료(v1.44.1, cfb1abe). Irene `/배포` 시 커밋 후 `./scripts/deploy-planq.sh --auto`. DB는 posts/documents/kb_documents 에 security_level ENUM 추가됨(운영 sync 자동 또는 ALTER).
