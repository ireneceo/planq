...
**남은 단 1건: #50 (이번 주 나의 업무 설정 persistence + 주간그래프 계산).**
⚠️ **Irene 노트북에서 '이번 주 나의 업무' 패널 UI 재구성 중 → 같은 QTaskPage 영역. 동시 편집 충돌 위험으로 보류.** Irene 확인 필요: 내가 #50 버그(설정 저장/그래프)를 맡을지, Irene이 노트북 작업과 함께 처리할지.
