...
## 현재 작업 상태
**마지막 업데이트:** 2026-06-28 (완성도/안전 스윕 세션)
**작업 상태:** **완성도 스윕 2배치 운영 배포 완료** — deploy `20260628_195500`(16커밋: transfer 쿼터·notify 보강·기본히스토리·통계대화형·P1완성도·Q Mail발송보강) + deploy `20260628_204552`(Q Mail 전달·rate-limit IPv6). planq.kr 헬스200·prod-backend(1.46.0)+qnote online·POS 무영향.
- **미배포 1건(배포 20260628_215148 이후):** `d9869fb` Tasks 진행중 예산초과 조기경고(in_progress_watch). 확정IA 미변경·완료분석 무수정·별도섹션. E2E 5/5(빈/양성 시드 est2·act5·+150%·불변식). dev 검증완료, /배포 대기. (이전 3건 임시저장·Finance추이·Tasks필터는 배포 20260628_215148 라이브)
- **심층 안전감사 통과:** 멀티테넌트/IDOR 0건·list pagination unbounded 0건·재무/서명 알림 완비·rate-limit IPv6 정상.
- **남은 작업 = 방향/승인/외부의존:** 통계 3탭 IA(확정구조 승인)·iOS푸시(Capacitor)·OAuth검증(GCP)·랜딩 리디자인(디자인)·운영 피드백 done 위생마킹.
- (이전 배포 이력: `20260628_111031` #63 Phase3+U4+job삭제 · `20260628_090725` #90·#95·#96)

### 진행 중인 작업
- 없음 (자율 고가치 작업 소진, 미배포 394f4d4 1건 /배포 대기)
...
- **미배포:** `b97db0d`+`f7d3cbb`+`f667308`(transfer) + `b65e5a4`+`6a3ea26`(#90 task 배정 알림) + `112ae15`(일정 수정 알림) + `3703ad7`+`da69a27`(Invoice 상태이력) + `9920d9d`(Project 상태이력) + `3e47769`(BillEvent writer fix) + `5345053`(ReportsTab 대화형 통계) **16건**(+P1 4건 + `5e0eab3` Q Mail발송 보강). 다음 /배포 시 함께 반영.

### Q Mail 직접발송 — 설계→실태규명→보강 (2026-06-28)
**/기능설계 호출했으나 조사 결과 Q Mail 발송은 이미 라이브**(M3-A, N+76): `services/emailSend.js sendMail`(OAuth/SMTP transport·첨부·RFC스레딩) + 엔드포인트 2개(`email-compose` 새메일·`email-threads/:id/messages` 답장) + 프론트 MailPage 컴포즈모달/답장컴포저. EmailSettings "준비 중"은 **stale 오안내**(BillEvent 패턴). → greenfield 설계 아닌 **완성/보강**으로 전환.
...
- ✅ **작성 중 메일 임시저장(Draft)** (`394f4d4`, 미배포) — EmailDraft(기존·미연결) 연결. GET/PUT/DELETE /:biz/email-drafts (business+user+thread_id 키, thread_id 없음=새메일·있음=답장). MailPage 새메일모달+답장컴포저 열 때 본인 초안 복원→1.5s 디바운스 자동저장→발송 시 삭제. forward 제외. E2E 9/9(생성·복원·재PUT무중복·thread격리·삭제·null). 빌드 EXIT0/TS0. Irene 선택 항목.
...
- **P0 통계 제대로 구성** ("파일로 말고"):
  - ✅ **#4 ReportsTab 대화형 통합보고서** (`5345053`) — 기존 PDF 카드 유지 + 위에 대화형 섹션: 주간/월간 토글 + 기간칩(확정뱃지) → summary KPI 카드 + 프로젝트별/멤버별 롤업 테이블 + 경영요약 narrative. service insights.ts +2(getReportPeriods/getIntegratedReport), Insights 공통컴포넌트 재사용, owner/admin 외 best-effort 숨김. E2E 5/5(periods/integrated 실데이터 프로젝트4·멤버6·진행16 / 비멤버403). 빌드 EXIT0/TS0. **"통계 파일로 말고 제대로" 달성.** (설계 상세 ↓)
...
- **P1 mock/완성도 (진행):**
  ...
  - ✅ **QTalk PM 배지 정식화** (`37d0269`) — `project_members.is_pm`(이미 존재)가 멤버 직렬화/매핑에서 누락돼 RightPanel 배지가 죽고 default_assignee 임시 프록시만 남아있던 것 정식화. loadProjectDetail 직렬화 + QTalkPage 매핑에 is_pm 추가, RightPanel 실 is_pm만 사용·프록시/stale주석 제거. E2E 3/3(is_pm=true→projectMembers 흐름·정확히 1 PM·원복). **함정: GET /:id 는 `members`가 아니라 `projectMembers`(raw) 반환. 레거시 프로젝트는 is_pm 미설정(생성 시 creator=true 로직 이후 생성분만).**
  - ✅ **`/signatures/received` 메뉴 진입로** (`3e97d5c`) — 고아 라우트 해소. 사이드바 '개인' 섹션(개인보관함·내문의 옆)에 "받은 서명" NavItem 추가(개인·수동 성격). i18n layout ko/en. 빌드EXIT0/dev 200. (향후 인박스 통합은 `project_qdocs_restructure_brief_plan` 별도 대형작업으로 유지)
