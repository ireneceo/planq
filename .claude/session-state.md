...
- **구현 진행(미배포):**
  ...
  - ✅ **§6-B 이월 배너** (`5922fd3`, **미배포**) — 드로어 상단 '이월 · 지난주부터 진행 중 · 누적 Nh 투입 — 이력·대화·메모 모두 남아있음'. 단일 엔티티 연속성 인지. 프론트만.
  - ⏳ **§6-B 시간 토글**(이번주/전체) — 백엔드 task별 주간 focus 집계 필요 (후속)
  - ⏳ **§6-C 델타** 주별 델타(carry-in 분리, 월요일 0 시작) — daily-progress 주시작 baseline 차감
  - ⏳ **U4** 단조완화(되돌림 ↓마커) — 프론트 SVG, chart 회귀주의
- **검증:** A1·A2·U5 배포 라이브. **미배포 2건: `9381d53`(스코핑 fix·운영 153.6→8.8 증명) + `5922fd3`(이월 배너)**. 둘 다 빌드EXIT0·TS0·헬스29/29·서빙200·패리티814.
