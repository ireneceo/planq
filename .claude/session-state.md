...
## 현재 작업 상태
**마지막 업데이트:** 2026-08-17 (Opus 5, 1M) — 접속 끊김 복구 → 배포 3회 → 삭제 → cron 수리
**작업 상태:** 🟡 월간보고 cron 수리 + 관리자 조작 안내 = Fable 재검증 중. Irene 지시: **통과하면 커밋·배포**.
...
**"코드가 참조하는 컬럼이 실제 스키마에 있는가" 가드를 만들 것.**
이번 사이클에만 **없는 컬럼 참조가 2건** 나왔고 둘 다 조용히 죽는 계열이었다:
- `server.js runMonthlyReportsIfDay1` 의 `businesses.status` → **월간 보고서가 한 번도 생성된 적 없음**(reports 0행)
- 내가 새로 짠 `Notification.tag` 멱등 키 → 존재하지 않아 매 실행 throw (실호출에서 발견)

둘 다 `catch` 가 삼켜서 로그에도 안 남는다. 정적 검사로만 잡힌다.
임시로 56컬럼 수동 대조는 했지만(전부 실존) 재발 방지책이 없다.
