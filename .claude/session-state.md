...
- **구현 진행(미배포):**
  ...
  - ✅ **§6-A2** (`6d0af8b`, **배포완료** `20260625_221735`) — 주간 그래프 판정 칩(EVM→일상어 4판정: 순조/앞섬/시간대비더딤/뒤처짐, SPI·CPI, 전문용어 비노출).
  - ✅ **§6-C/U5** (`c610604`, **미배포**) — 실작업률 자동 제안. GET `/by-business/:biz/participation-suggestion`(최근28일 포커스÷명목). **신뢰성 게이트:** 포커스가 명목의 40% 미만이면 '포커스 사용률'이라 오안내 → 제안 안 함(Irene 8h/120h=7% → null). 가용 패널 '최근 N주 실측 X%·[적용]' 칩.
  - ✅ **§6-C 스코핑 fix** (`9381d53`, **미배포**) — 운영 워프로랩 체크에서 발견한 **진척선 153h 비현실 버그** 수정. daily-progress 가 전체 업무 합산하던 것을 `task_ids` 로 이번주 집합 스코핑(보안 교집합). focus 도 그 집합만. focusSessions 쿼리에 last_activity_at(#94 캡)·task_id 보완. 프론트 chartTaskIdsKey 전달. **운영 read-only 증명: EV 153.6h→8.8h.**
  - ✅ **§6-B 이월 배너 + 시간** (`5922fd3` 배포 + `3f0673e` 미배포) — 드로어 '이월 · 이번주 Xh · 누적 Nh 투입 — 이력·대화·메모 모두 남아있음'. 백엔드 GET `/api/focus/task-time`(본인세션 집계, 전체/이번주). 연속성 인지.
  - ⏳ **§6-C 델타** 주별 델타(carry-in 분리, 월요일 0 시작) — daily-progress 주시작 baseline 차감. **차트 SVG 수술 → Playwright 시각검증 필요한 다음 세션 작업.** (스코핑 fix로 차트는 이미 현실값이라 델타는 선택적 폴리시)
  - ⏳ **U4** 단조완화(되돌림 ↓마커) — 차트 SVG, 되돌림은 희귀 케이스라 저우선
- **이번 세션 완료(6개):** A1·A2·U5·스코핑fix·이월배너 **배포 라이브** + §6-B 시간(`3f0673e` 미배포). 모두 빌드EXIT0·TS0·헬스29/29·패리티815·서빙200.
- **미배포: `3f0673e`(§6-B 시간 토글, 백엔드+프론트).** 다음 `/배포`.
- **운영 워프로랩 체크 결과(2026-06-25):** 잔여 29.2h(이월7.2+신규22)·활용률97%·이월 인지 작동·씨댄스 실제4h→잔여0.8h(왜곡제거 확인)·참여율 게이트 null(focus3.8h)·#94 캡 라이브(투입17.2h). 단 진척선 153h 버그 발견→`9381d53` 수정.
- **⚠️ U5 신뢰성 한계(Irene 결정 필요):** focus 기반 참여율은 "포커스를 모든 업무에 쓸 때만" 정확. 게이트로 오안내는 막았으나, 포커스 비사용자는 영영 제안 안 뜸. 향후 actual_hours 등 다른 신호 보강 검토 가능.
- **다음:** U5 배포(B와 묶거나 단독) + §6-B/그래프델타/U4. Irene 지시.
