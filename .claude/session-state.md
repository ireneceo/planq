...
## 현재 작업 상태
**마지막 업데이트:** 2026-07-31 17:20 (Opus 5, 1M)
**작업 상태:** **운영 배포 3회 완료** (최종 `b06e261`). 운영 피드백 4건 수정 + 22건 상태 정리.

### 🚀 2026-07-31 배포 3회 (누적 미배포 0)
1차 `cbee3ac` — 핀 홀더창·이월결함5·시간엔진·KB FK (+actual_hours 백필 6건·위키 시드)
2차 `6d33587` — #242 Meet
3차 `b06e261` — #216 팝업·#226 메일레이아웃·#223 진척그래프 (`Deployment Complete 226s`)

### 이번 세션 운영 피드백 수정 4건 (전부 Fable 게이트 PASS)
- **#242** Meet 링크 실패가 **일정 생성을 죽이던** 구조. 같은 파일의 일반 push 는 이미 best-effort 였는데
  Meet 만 rollback 했다. 3중 결함(운영 토큰 write scope 없음 · `gcal_connected` 가 토큰 존재만 봄 ·
  실패 시 tx 롤백) + Fable 이 2건 추가 발견(meeting 라우트 scope 미검사 · submitting 고착·영문 raw 코드).
  1차 FAIL — 경고 토스트가 자기 socket 에코의 `fetchRange` 에 300ms 만에 지워짐 → 별도 state 분리.
  ⚠️ **운영 Meet 복구는 Irene 의 Google 재연동 필요**(동의 화면 캘린더 체크). 재연동 전엔 옵션 숨김+안내.
- **#216** 팝업 잘림·스크롤 불가 9곳. 공용 Modal 을 안 쓰는 자체 styled 에 `max-height`/`overflow` 부재.
  ⚠️ 이 중 일부가 **배포 중 프론트 빌드 시간대에 소스를 고쳐 검증 전 운영에 실렸다** — 사후 Fable PASS·롤백 불요.
  **배포 중 소스 수정 금지.**
- **#226** 메일이 왼쪽으로 쏠리고 박스가 깨짐. 원인은 정화기 허용목록이 아니라 **`ALLOWED_URI_REGEXP`** —
  DOMPurify 는 **모든 속성 값**을 이 정규식으로 검사한다. 스킴 목록만 나열해 `align="center"`·`width="600"`
  이 전멸(운영 실측 align 5·width 11). **리치텍스트의 colspan/rowspan 도 같은 이유로 죽고 있었다.**
  보안 경계라 Fable 이 XSS 벡터 61종 + 27,619 probe 퍼즈 + 실행 여부 실측으로 "표면 확대 0" 확인.
- **#223** 주간 진척 그래프가 평평. **내 1차 진단 절반 오류** — 내가 지목한 `WeeklyReviewView` 는 죽은 코드였고
  실화면은 report_units. 실원인 2겹: 주 스코프 부재 + TDP 가 업무별 **일생 누적**이라 이월분이 첫날부터 유입.
  **스코핑만으로는 안 풀린다**(내가 운영·dev 실측으로 반증 → Fable 이 자기 판정 ③ 철회).
  → 업무별 기준선 차감 + **업무별** 클램프(집계 후 클램프는 오답) + 미래 요일 컷.
  운영 실측: **196.7h 평평 → 0→3.7h 상승** (est 0→7.0). `/my-week` 회귀 diff 0(36케이스).

### 📋 피드백 상태 정리
미처리 **54 → 32건**. 22건은 이미 구현돼 있는데 상태만 pending 이었다(표본 전수 대조 후 done 처리).
알림 22개가 보고자에게 쏟아지지 않게 이력만 남기고 알림은 미발생.

### 다음 후보 (코드 참조 0 = 미착수 확실)
#215 첨부파일 · #217 증빙 숫자 미갱신 · #218 셀렉트 길이 · #219/#234 에디터 엔터 · #221 분류 기준 ·
#222 새 메일 폼 이탈 · #224 연동 모달 안 닫힘 · #225 문서 다운로드 · #227~241 신규 기능군

### Fable 이 남긴 후속 결정 2건
(a) project 보고서 progress_series 도 같은 스코핑 적용 여부
(b) 라이브 그래프·SPI 를 같은 Δ 정의로 정렬할지 (현재 과도기 비대칭: 보고서=주간Δ / 라이브=누적)
