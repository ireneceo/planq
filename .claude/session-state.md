...
**남은 그룹 — 전부 대형 설계/인프라 (각각 별도 사이클 권장):**
- #49 SNS OG 메타 — ⚠️ 크롤러용 **서버 OG 주입 + nginx 라우팅(운영 인프라)** 필요. 설계+승인.
- #53 개인 외부연동 정의 — 개인 vs 팀 연동 명확화 + 전수 검증 (대형).
- #55 Q Mail 멀티계정 인박스 — All/주소별 (대형).
- #52 개선부분(초대일시 시간·재발송 이력·동기화·멤버초대).
- #50 보류(Irene 노트북 겹침).
