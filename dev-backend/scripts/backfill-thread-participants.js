...
//
// 술어: 라이브 쓰기측(services/emailAddress.mergeParticipants)과 **같은 함수**를 쓴다.
//   - inbound 메시지의 from + cc
//   - outbound 메시지의 to + cc
//   - 스레드 계정(email_accounts.email + 별칭) 주소는 제외
//
...
// 한 스레드의 메시지들 → 참여자 후보 목록 (라이브와 같은 술어)
...
(async () => {
  ...
  for (let offset = 0; offset < total; offset += BATCH) {
    ...
    for (const t of threads) {
      ...
      const next = mergeParticipants(t.participants, participantsFromMessages(ms), { excludeEmails });

      if (participantsEqual(t.participants, next)) { unchanged++; continue; }
      if (next.length === 0) { noExternal++; }
