  ...
  async (req, res, next) => {
    try {
      ...
      const outMsg = await EmailMessage.create({
        ...
        // 실제 나간 주소를 기록한다 — account.email 하드코딩이면 별칭으로 보낸 메일도
        //   보낸메일함엔 계정 주소로 남아 "표시 ≠ 실발신" 이 된다.
        from_email: sendResult.fromEmail || account.email,
        from_name: sendResult.fromName || account.display_name || null,
        to_emails: toList,
  ...
  async (req, res, next) => {
    try {
      ...
      const outMsg = await EmailMessage.create({
        ...
        from_email: sendResult.fromEmail || account.email, from_name: sendResult.fromName || account.display_name || null,
        to_emails: toList,
        cc_emails: (Array.isArray(cc) && cc.length) ? cc : null,
        bcc_emails: (Array.isArray(bcc) && bcc.length) ? bcc : null,
