 ...
 */
function buildReceiptRows(invoices) {
  const rows = [];
  for (const inv of invoices || []) {
    if (inv.status === 'draft' || inv.status === 'canceled') continue; // 취소/초안은 증빙 의무 없음 (#5)
  ...
  // 발행대기 먼저(긴급한 순=due_at 오름차순), 그 다음 발행완료(최근순)
  const rank = { overdue: 0, soon: 1, normal: 2, done: 3 };
  rows.sort((a, b) => {
...
async function fetchReceiptRows(models, where) {
  ...
  const invoices = await Invoice.findAll({
    where: { ...where, status: { [Op.notIn]: ['draft', 'canceled'] } },
    include: [
      {
        model: Client,
  ...
  return buildReceiptRows(invoices);
}
