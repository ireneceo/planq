...
const ARTICLES = [
  ...
  { cat: 'qmail', slug: 'qmail-sender-rules', visibility: 'authenticated', linked_route: '/business/settings/mail-accounts', est: 2,
    ...
    summary: t('같은 발신자를 두 번 "답변 완료" 하면 앞으로 묻지 않습니다', 'Mark the same sender "Replied" twice and we stop asking'),
    body: [
      ...
      s('같은 발신자의 메일을 두 번 "답변 완료" 하면, 그 발신자는 앞으로 "답변 필요" 에 나타나지 않습니다. 이미 쌓여 있던 그 발신자의 메일도 함께 정리됩니다.',
        'Mark two emails from the same sender as "Replied" and that sender stops appearing in "Needs reply" — including the ones already piled up.'),
  ...
  { cat: 'qmail', slug: 'qmail-reply-needed', visibility: 'authenticated', linked_route: '/mail', est: 2,
    ...
    body: [
      p('"답변 필요" 에는 확실히 답해야 하는 메일만 모읍니다 — ①고객·팀원처럼 아는 상대가 보낸 메일 ②우리가 주고받던 대화에 대한 회신 ③우리 주소로 직접 온 명확한 요청(문의·견적·회신 부탁). 애매한 메일은 "확인 권장" 으로 갑니다. 사이드바 Q Mail 옆 숫자가 답변 필요 건수입니다.',
        'The "Needs reply" tab collects only mail you clearly have to answer: (1) mail from someone you know — a client or teammate, (2) a reply in a thread you were already in, and (3) a clear request sent directly to your address (an inquiry, a quote, a request to reply). Anything ambiguous goes to "Review suggested". The number next to Q Mail in the sidebar is the needs-reply count.'),
