...
const ARTICLES = [
  ...
  { cat: 'qsale', slug: 'customer-entry-booking', visibility: 'authenticated', linked_route: '/business/settings/permissions', est: 4,
    ...
    body: [
      s('설정 › 권한의 **«고객 창구»** 카드에서 주소를 만듭니다. 명함·메일 서명·홈페이지에 붙이고, 인쇄용 QR 도 같은 카드에 있습니다. 주소를 교체하면 옛 주소는 즉시 닫힙니다.',
        'Create the link from the «Customer entry» card in Settings › Permissions. Put it on business cards, email signatures or your website — a printable QR code is on the same card. Replacing the link closes the old one immediately.'),
      s('고객은 그 주소에서 **이메일만 확인하면** 바로 문의를 남깁니다. 방문자마다 자기 대화방이 생기고, 그 대화는 Q Talk·Q Sale 에 그대로 들어옵니다.',
        'Customers verify their email on that page and can leave an inquiry right away. Each visitor gets their own conversation, which shows up in Q Talk and Q Sale.'),
      s('**상담 예약**을 켜면 창구에 «상담 예약» 탭이 생깁니다. 담당 멤버·한 번 길이·최소 예약 시점·하루 최대·예약 받는 시간을 같은 카드에서 정합니다. 담당자의 기존 일정과 겹치는 시간은 고객에게 보이지 않습니다.',
        'Turn on booking to add a «Book» tab to the entry page. Set the host, length, minimum notice, daily limit and bookable hours on the same card. Times that overlap the host\'s existing events are hidden from customers.'),
      s('신청은 담당자의 **확인필요**에 «상담 신청» 으로 뜨고, 캘린더에는 점선과 «신청» 표시로 보입니다. 일정 상세에서 [승인] [다른 시간 제안] [거절] 을 누릅니다. 누를 때마다 고객에게 메일이 가므로 받는 주소를 확인창이 한 번 보여 줍니다.',
        'Requests appear in the host\'s To-do as «Consultation request» and on the calendar with a dashed outline and a «Request» tag. Approve, propose another time, or decline from the event details. Each action emails the customer, so a dialog first shows you the address.'),
      s('확정되면 고객에게 **일정 파일(.ics)** 이 담긴 메일이 갑니다. 상담이 끝나면 고객 상세의 상담 기록에 한 줄이 자동으로 남습니다.',
        'Once confirmed, the customer receives an email with a calendar file (.ics). After the consultation ends, a line is added to the customer\'s consultation history automatically.'),
      s('로그인한 고객은 **«홈»** 에서 같은 안내·상담 예약·내 문의를 보고, 프로젝트와 청구로 이어서 갑니다.',
        'Signed-in customers see the same intro, booking and requests on their «Home», with links to their projects and bills.'),
  ...
  { cat: 'qfile', slug: 'folder-upload-and-delete', visibility: 'authenticated', linked_route: '/files', est: 3,
    ...
    body: [
      s('폴더를 Q File 화면에 끌어다 놓거나 "폴더 올리기" 를 누르면, 안의 하위 폴더 구조를 그대로 만들면서 올립니다.', 'Drag a folder onto the Q File screen or press "Upload folder" — the subfolder structure is recreated as the files upload.'),
      s('같은 이름의 폴더가 이미 있으면 새로 만들지 않고 그 폴더에 합쳐집니다.', 'If a folder with the same name already exists, the files join it instead of creating a duplicate.'),
      s('폴더를 삭제하면 "폴더만 삭제" 와 "파일도 함께 삭제" 중에 고를 수 있습니다. 확인창이 그 폴더와 하위 폴더에 파일이 몇 개 있는지 먼저 알려 줍니다.', 'Deleting a folder lets you choose "Delete folder only" or "Delete files too". The confirmation tells you how many files the folder and its subfolders hold.'),
      s('"폴더만 삭제" 를 고르면 파일은 바로 위 폴더로 옮겨지고 그대로 남습니다.', 'Delete folder only moves the files up one level, where they stay.'),
      s('"파일도 함께 삭제" 를 고르면 파일도 휴지통으로 갑니다. 30일 안에 되돌릴 수 있습니다.', 'Delete files too sends the files to the trash, where you can restore them within 30 days.'),
  ...
  { cat: 'qfile', slug: 'same-name-file', visibility: 'authenticated', linked_route: '/files', est: 2,
    ...
    body: [
      s('같은 폴더에 같은 이름의 파일이 이미 있으면 올리기 전에 물어봅니다.', 'If a file of the same name already sits in the same folder, you are asked before it uploads.'),
      s('"덮어쓰기" 는 새 파일을 올리고 이전 파일을 휴지통으로 보냅니다. 30일 안에 되돌릴 수 있습니다.', 'Overwrite uploads the new file and sends the previous one to the trash, recoverable within 30 days.'),
      s('"이름 바꿔 저장" 은 이름 뒤에 (1) 을 붙여 저장합니다. 두 파일이 모두 남습니다.', 'Save with a new name appends (1) to the file name, so both files remain.'),
      s('"건너뛰기" 는 겹치는 파일만 빼고 나머지를 올립니다.', 'Skip uploads everything except the files that clash.'),
      note('고른 답은 **그 번에 올리는 파일 전부에** 같이 적용됩니다. 수십 장을 올릴 때 파일마다 묻지 않습니다.', 'Your choice applies to every file in that upload, so you are not asked file by file when uploading dozens.'),
      note('다음에 다시 올릴 때는 **새로 물어봅니다.** 앞서 고른 답이 남지 않습니다.', 'The next upload asks again — your earlier choice is not remembered.'),
  ...
  { cat: 'settings', slug: 'trial-options', visibility: 'authenticated', linked_route: '/business/settings/plan', est: 2,
    ...
    body: [
      p('가입하면 14일 무료 체험으로 시작합니다. 첫 결제를 하기 전이라면 설정 → 플랜에서 «지금 결제하면 1개월을 더 드립니다» 를 고를 수 있어요.',
        'You start on a 14-day free trial. Before your first payment, Settings → Plan offers a second option: pay now and get an extra month.'),
      h('두 방식의 차이', 'How the two differ'),
      s('무료 14일 — 결제 없이 14일. 끝나기 전에 결제하지 않으면 워크스페이스가 잠깁니다(7일 유예 후).',
        '14 days free — no payment for 14 days. If you do not pay before it ends, the workspace locks after a 7-day grace period.'),
      s('지금 결제 · 1개월 추가 — 1개월 요금으로 2개월을 씁니다. 다음 결제는 2개월 뒤예요.',
        'Pay now, get a month — two months for the price of one. Your next charge is two months later.'),
      h('결제 수단', 'Payment methods'),
      p('카드와 계좌이체 모두 됩니다. 선불 상품이라 자동으로 빠져나가지 않고, 2개월 뒤에 결제 안내를 다시 받습니다.',
        'Card and bank transfer both work. It is prepaid, so nothing is charged automatically — you receive a payment notice again in two months.'),
      note('이미 결제하신 1개월 요금은 해지하셔도 환불되지 않습니다. 다음 결제일 전에 해지하시면 추가 청구는 없습니다. 이 안내는 결제 화면에도 표시됩니다.',
        'The month you pay for is not refunded if you cancel. Cancel before the next charge date and nothing further is billed. This is also stated on the payment screen.'),
