...
Task.init({
  ...
  status: {
    ...
    //  done_feedback   — **승인완료 · 마무리 대기** (2026-09-12 부활. 2026-04-25~09-12 은 폐지 상태였다).
    //                     컨펌 정책이 충족되면 여기로 온다 — 완료가 아니다. `completed_at`·진행률 100 을
    //                     찍지 않으며, 담당자가 "최종 완료" 를 눌러야 completed 로 간다.
    //                     Irene: *"컨펌 다 받으면 완료 상태가 되는데 그러면 담당자가 확인을 못하잖아.
    //                     승인완료함 << 이렇게 단계 하나 완료 앞에 있어야 하지 않아?"*
    //                     ★ 폐지값을 되살린 것이라 **운영 ENUM ALTER 가 없다**(값이 계속 살아 있었다).
    //                     옛 축(요청자 피드백)과 다르다 — 지금은 **담당자가 닫는다**.
    //                     전이 정본: services/actions/task_actions.js recalcStatusFromReviewers
