...
> **최종 업데이트:** 2026-09-03 (Opus 5, 1M) — **v1.48.7 운영 배포 · 커밋 2건 · Fable 게이트 2회(설계 1 · 구현 1)** — 주제는 **"스위치를 내렸는데 메일은 계속 나갔다"** 였다. ①링크로 들어온 고객이 **답이 온 것을 알 방법이 없던 것**을 고쳤다(#259 A안) — 원하면 이름·이메일을 남기고 답글을 메일로 받는다. ②Fable 설계 게이트가 **네 가지를 바꿨다**: 알림 트리거를 라우트 두 곳에 달았으면 청구서·문서·서명 카드와 Cue 자동응답, 초안 승인까지 **여섯 경로에서 알림이 안 갔다**(Message 훅 한 곳으로 옮김) · 링크만 아는 익명이 글을 써서 등록자 전원에게 **하루 96통씩 강제로 쏠 수 있었다** · 한도가 한 축이라 카톡방 다섯 명이 등록하면 넷째부터 막혔다 · me·수신거부·삭제를 shared 토큰으로 받으면 **아무나 남의 등록을 지웠다**. ③내가 쓰던 "고객에게 보이는 메시지" 술어는 정본의 **사본**이었고 이미 갈라져 있었다 — `!== true` 비교라 MySQL 이 1/0 을 주는 경로에서 **승인된 초안을 숨겼다**. ④토큰을 저장하지 않고 **파생**한다 — 회전시키면 지난 메일 링크가 다 죽고, 원문을 저장하면 DB 사본 하나로 모든 대화가 열린다. 키만 새도, DB 만 새도 만들어지지 않는다. ⑤**내 검증이 두 번 거짓 통과했다** — 파생 토큰이 `created_at` 을 잘못 읽어(속성명은 `createdAt`) 조용히 null 이 됐는데 **fail-closed 라 에러가 없었고**, "키 없으면 null" 검사는 이미 null 이라 통과였다. 보관기간 cron 도 같은 이유로 아무것도 안 지우면서 "0건 정리" 를 냈다. ⑥Fable 구현 게이트가 배포를 세우고 둘을 잡았다 — **플랫폼 킬스위치를 내려도 알림 메일은 나갔고**(링크는 안 열리니 "알림은 오는데 눌러도 안 열린다"), **본인이 지워도 발송 기록에 주소와 IP 가 남았다**(방침에 "즉시 삭제 가능" 이라 써 놓고).
