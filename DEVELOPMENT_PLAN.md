...
> **최종 업데이트:** 2026-06-13 — **v1.36.0 회차별 현금영수증 운영 라이브 (deploy `20260613_155838`, 138초, commit `1478c7f` · 운영 DB cash_receipt 3컬럼 자동추가 확인 · 헬스 200·PM2 prod online).** 버전 1.35.0→1.36.0. 분할 결제 회차마다 입금 시점 현금영수증 발급(세금계산서 회차 패턴 미러). DB `invoice_installments` + cash_receipt_no/at/marked_by 3컬럼(sync 자동) · `POST /:biz/:id/installments/:instId/mark-cash-receipt`(owner_only+audit+broadcast+통지) · `receiptsDue` 분할 시 세금/현금 모두 회차별 산출 · 프론트 `markInstallmentCashReceipt`+IssueModal 4-way 라우팅. E2E 10/10(회차별 산출·발행·status 전이·고객메일·owner_only). **미배포 → 다음 `/배포`(sync_database 자동 컬럼).**
>
