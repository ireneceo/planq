...
> **최종 업데이트:** 2026-07-03 (Opus) — **C1 q-note STT 실분 과금 (트랙B 코드완료+검증) + 보안 하드닝(트랙A 미완).** 헬스 29/29(2회, q-note 재시작 후 재검) · 프론트 빌드 EXIT0/TS0.
...
> - **🔴 보안 노출 발견(실측):** ufw inactive + Node `:3003`/uvicorn `:8000` 0.0.0.0 바인드 → nginx 우회 외부IP 직결 도달(공유시크릿만 방어). **트랙A 하드닝 = 다음 섹션:** Node `listen(PORT,'127.0.0.1')` + uvicorn `--host 127.0.0.1`(ecosystem.config.js에 planq-qnote 등록) + nginx `location /api/internal{deny all}`(dev+scripts/nginx-planq.kr.conf).
> - **미완(다음 섹션):** 트랙A 보안 하드닝 · live WS E2E(mock, 4030/4031·flush) · 운영 배포(Irene /배포 대기). **미푸시분 다수 누적(이전 세션 포함).**
