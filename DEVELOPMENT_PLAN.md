...
> **최종 업데이트:** 2026-07-03 (Opus, 2차) — **C1 q-note STT 실분 과금 완결 — 트랙B(과금) + 트랙A(보안 하드닝) + live WS E2E(mock) 전부 완료·검증.** 외부IP :3003/:8000 직결 000(차단) · WS 진입 hard-block 5/5 PASS · 헬스 29/29 · 프론트 빌드 EXIT0/TS0. 남은 것: cost-guard H-f 잔여 · 운영 배포 후 실녹음 샘플.
...
> - **미완(다음 섹션):** cost-guard H-f 잔여(q-note generate-keywords LLM rate-limit + create_session rate-limit, slowapi 신규패턴 승인 후) · 운영 배포(Irene /배포 대기 — 운영 nginx 3002/8001·PLANQ_NODE_BASE_URL :3002 체크). **미푸시분 다수 누적(이전 세션 포함).**
