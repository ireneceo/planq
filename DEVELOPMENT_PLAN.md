...
> **최종 업데이트:** 2026-08-22 (Opus 5, 1M) — **운영 배포 5회 · 장부 pending 25→20 · Fable 게이트 3회(예산 소진)** — 이번 사이클의 주제는 **"안 부르는 가드는 없는 가드"** 였다. ①**근태 메뉴를 눌러도 빈 화면**이었는데 원인은 라우트 표가 두 벌이라는 것 — `App.tsx` 에만 등록하고 **탭 모드가 쓰는 `routes/appRoutes.tsx`** 를 몰라 `path="*"` 로 떨어졌다. 그걸 잡을 `guard-app-routes.js` 는 **이미 있었는데 어느 게이트에도 붙어 있지 않아 아무도 실행하지 않았다** → guard-invariants 에 `routedrift` 로 묶고(26/26), 곧바로 같은 상태였던 `/whats-new` 도 찾아 고쳤다. ②**내 검사가 세 번 연속 거짓 결론**을 냈다 — 브라우저 로그인 순서를 틀려 "페이지가 죽었다" 로 읽었고(대조군 `/tasks` 도 같아 하니스 문제로 판명), `<main>` 이 없는 앱에서 그걸로 빈 화면을 재려 했고, 버튼 목록을 12개에서 자른 채 "팀 관리 탭이 없다" 고 했다. ③**출퇴근·휴가 신규 구축**(4테이블·원장/롤업·정정·휴가 승인·가용시간 연동) 후 배치를 세 번 옮겼다 — Q시리즈→개인→Q시리즈→**개인(내 것) + 설정(관리)로 분리**가 최종. "내 출퇴근"과 "남의 근태 승인"은 보는 사람도 권한도 달라 한 화면에 탭으로 묶으면 둘 다 어정쩡해진다. ④**업무 진행중 = 근무중** 이 안 이어져 있었다 — 자동 출근이 `focus_enabled` 안쪽에 갇혀 있었고, 화면의 '진행 시작'은 훅을 건 라우트를 아예 거치지 않았다. 업무 상태 전이 자체에 걸고, 시스템이 대신 바꾼 것은 **안내 + 30분 내 취소**를 붙였다. ⑤**#371 핫픽스** — 내가 오전에 배포한 코드가 `let` 을 try 안에 선언하고 finally 밖에서 읽어 **매 메일 동기화 끝에 예외**(운영 16회). 메일은 저장된 뒤 터져서 기능은 정상으로 보였다. ⑥**"안 들어온 메일"은 들어와 있었다** — 본문까지 같은 메일이 보관함에 있었고, 사람 기준으로 되찾을 길이 없었을 뿐이다(#261 → 주소 기준 보기).
