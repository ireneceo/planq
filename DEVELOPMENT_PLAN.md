...
> **최종 업데이트:** 2026-08-21 (Opus 5, 1M · 야간 사이클) — **운영 배포 2회 · 장부 pending 86→30 · Fable 게이트 2회** — 이번 사이클의 주제는 **"측정하지 않고 단정하지 않기"** 였다. ①**진척 그래프의 기준선이 예측 정정을 못 견뎠다** — 기준선을 스냅샷에 박제된 옛 예측으로 만들어, 예측을 16h→1h 로 고치면 55%p 올린 실제 진척이 `max(0, 0.9−5.6)=0` 으로 사라지고, 반대로 올리면 일하지 않아도 부풀었다(운영 스냅샷 보유 189건 중 **48건이 예측 변경 이력** 보유). 기준선도 지금 예측으로 환산해 `Δ = est_now × (p − p_base)` 로 통일 — Fable 이 대안 축(스냅샷 시점 예측)을 running-max 와 결합하면 #300 병리가 그대로 보존된다고 반증하며 PASS. ②**#364 는 "문제 없음" 이 아니었다** — 맥이 파일명을 분해형(NFD)으로 저장해 **"스크린샷" 검색이 33건을 0건으로** 내주고 있었다(운영 1010건 중 41건). 저장·검색·백필 3축을 맞추고 운영 56건 정리, 검색 복구 실측. Fable 이 **6번째 쓰기 경로(메일 첨부)와 첨부 이름 사본 15건 누락**을 잡아 FAIL → 보완. ③**폐지된 Q record 가 검색에만 살아남아** 같은 항목을 두 번 띄우고 고아 1건은 죽은 링크였다. ④**배포가 장부를 닫도록 내장** — 그런데 커밋 본문의 `#숫자` 를 긁게 했더니 근거로 인용한 업무 번호(#123 #167 #180)까지 잡혀 **고치지도 않은 신고를 닫을 뻔했다**(실데이터 테스트로 발견 → 명시 트레일러 `Feedback-Closes:` 로 교정). ⑤**주간 가용시간 바 라벨 통일**(진척→진척(예상시간) / 투입→실제 업무시간) + **여유 행 삭제**(이미 쓴 시간을 안 빼 "20h 일하고도 여유 11h" 였다. 계산 자체는 Irene 지시로 Fable 이관). **내가 세 번 오진했고 세 번 다 측정이 갈랐다** — `exit 143` 을 OOM 으로 단정(실측: 79초·RSS 3.93GB 에서 SIGTERM, 여유 메모리 따라 갈림) · #255 결손을 18건으로 오독(실제 4건, 나머지는 task 0건 프로젝트의 정상 빈 상태) · 워크트리에서 돌린 가드가 사실은 본 체크아웃만 검사(스크립트가 `ROOT='/opt/planq'` 하드코딩)했다.
