...
> **최종 업데이트:** 2026-09-14 (Opus 5, 1M) — **운영 배포 `55c90dfa`(v1.52.1, 09:49) · 앞선 `362f33c1`(v1.52.0, 06:49) — Fable 미검증(자체 검증): 호출 5회 전부 한도 초과(HTTP 429), Irene 지시로 자체 검증 진행.** 오늘의 주제는 **"설정이 거짓말을 하고 있었다, 그리고 0건 픽스처는 증명이 아니다"** 다. ①어제 넣은 [푸시 실패 시 메일] 을 **개별 설정과 별개 스위치**로 둔 것이 곧 "메일 다 껐는데 계속 온다" 였다 — 운영 실측: irene 계정은 메일 10종이 전부 0 인데 그 항목만 1(기본값). 이제 **두 겹 다** 통과해야 나가고, 크론 본체를 운영과 같은 설정으로 돌려 0통을 확인했다(양성 대조군 포함 7/7, 실제 발송 0통). ②공개 링크가 **보는 사람의 언어를 반만** 따랐다 — 감지는 됐는데 문구 **95건**이 ko/en 어느 쪽에도 키가 없어 한국어 기본값으로 떨어졌다(패리티 가드가 못 잡는 계열: 한쪽에만 없는 게 아니라 **양쪽 다 없다**). ③업로드 실패에 이유가 없었다 — **화면이 사유를 이미 들고 있으면서 안 그렸다**. 그리고 50MB 하드코딩이 Drive 로 갈 파일까지 미리 막고는 "Drive 를 연결하세요" 라고 안내했다(이미 연결돼 있는데). ★ 전날 내가 "운영 nginx 상한 10MB" 라고 보고한 것은 **틀렸다** — 실측 2GB. ④Q sale 을 Q task 배치로(탭 우상단 연회색 알약·검색과 필터 한 줄·축 이름을 셀렉트 밖에), 메모를 **댓글**로 (채팅방 메모와 같은 컴포넌트·같은 표), Cue 바를 Q task 와 같은 3단계로(Enter 전송·정리 확인 후 추가). ⑤이름 앞 아이콘이 원형 12곳/라운드박스 30곳으로 갈려 있던 것을 통일하고 **가드 `avatarshape`** 신설. 프로필 사진은 `users.avatar_url` 이 **이미 있어** 스키마 변경 0. ⑥**자체 검증이 결함 3건을 잡았다** — 보관함 숫자가 목록과 갈라짐(0건 픽스처로는 통과했다), 단계 칸이 행마다 20px 어긋남([보기] 라벨 길이 탓), 사진이 **mimetype 만 보고** 위장 파일을 저장. 셋 다 양성 대조군이 뒤집어 준 것이다. 이전 기록: 2026-09-13 (Opus 5, 1M) — **운영 배포 2회: `d02021bb`(v1.49.0, 17:55) · `1f57a4e6`(v1.50.0, 20:06) — Fable 미검증(자체 검증): 오늘 호출 전부 한도 초과(HTTP 429).** 저녁의 주제는 **"어제 내가 넣은 라우트가 남의 라우트를 가리고 있었다 — 에러 없이"** 다. ①`routes/projects.js` 에 `GET /:id/notes` 를 **두 번** 선언했다. Express 는 먼저 만난 것만 부르므로 뒤에 있던 **프로젝트 메모 조회가 그날부터 죽은 코드**가 됐고, 그것을 읽던 화면 4곳(Q talk 우측패널·프로젝트 상세·Q task)이 **500 도 404 도 없이** 엉뚱한 목록을 받았다 — 그래서 어제 검증을 그대로 통과했다. 경로를 옮기고 **가드 `--category=duproute`**(한 라우터 파일 안 method+path 중복, 873개 경로)를 새로 붙여 깨뜨려 확인했다. ②**노트 탭을 Q Note 본체로** 바꿨다 — 문서 탭이 `PostsPage` 를 scope 로 쓰는 것과 같은 방식(`QNotePage scope`). 목록만 베낀 화면으로는 프로젝트 안에서 회의록을 만들 수도 고칠 수도 없었다. 녹음 중에는 다른 탭으로 옮겨도 **언마운트하지 않는다**(언마운트가 곧 녹음 중단이다). ③그 과정에서 **q-note 쓰기 경로의 조용한 결손 둘**이 드러났다 — 노트를 만들 때 보낸 `project_id`·`client_id` 가 **스키마에 없어서 pydantic 이 버리고** 있었고(화면은 사이클 N+17 부터 보냈고 주석엔 "반영" 이라 적혀 있었다 — 실측한 세션의 project_id 는 언제나 NULL), 공개 범위를 L2 아닌 값으로 바꾸면 `project_id = None` 으로 덮어써 **프로젝트 노트를 워크스페이스 공개로 바꾸는 순간 프로젝트에서 사라졌다.** 한 컬럼에 뜻이 둘 섞여 있던 것이다 — 연결과 공개 범위를 갈랐다. ④**정보 탭과 Q info 를 한 벌로** — 정보 탭이 Q info 를 베껴 5열 그리드 vs 2칸 단순 행으로 갈라져 있었다(커스텀 항목·보안등급·상태·삭제가 통째로 없었다). 행 자체를 `components/Knowledge/kbListShell` 로 빼서 두 화면이 같이 쓴다. AI 버튼도 표준 `AiActionButton` 으로. ⑤**고객 프로필에 [노트]·[정보]**(읽기 전용) — 연결하는 자리는 각자의 정본이 따로 있다. Q docs 글에는 고객 축이 없어 넣지 않았다(대상 없는 버튼은 눌러도 아무 일이 없다). ⑥**문서 표가 두 겹으로 틀려 있었다** — [열 균등]이 열마다 160px 고정을 넣었고(3열=480px, 본문 872px), 더 뿌리는 **prosemirror-tables 가 표에 인라인 `min-width` 를 써서 우리 CSS 의 `min-width:100%` 를 덮고 있던 것**이다. 그래서 그 100% 는 **한 번도 적용된 적이 없었고** 표가 `max-content`(290px)에 머물러 "테두리가 내용까지만" 이 됐다. 스타일시트를 `!important` 로 올렸다 — 좁은 표는 본문 폭까지 늘어나고 넓은 표는 자연폭을 지켜 래퍼가 스크롤한다 (여기에 `width:100%` 를 쓰면 옛 우측 잘림이 돌아온다). ⑦**내 카나리가 두 번 거짓말했다** — 같은 계정으로 Node 쪽에서 또 로그인해 브라우저 세션이 401 로 떨어졌고, 로그인 직후 `gotoSPA` 로 옮겨 앱이 아직 미인증이라 /login 으로 튕겼다. 둘 다 판정이 "0건" 으로 나와 **제품 고장처럼** 보였다. 그리고 styled 템플릿 주석에 백틱을 넣어 또 템플릿을 끊었다(TS 101건). 이전 기록: **운영 배포 `c5cfef04` (10:38, v1.48.22) — Fable 미검증(자체 검증): 오늘 17회 전부 한도 초과(HTTP 429).** 오늘 늦게의 주제는 **"서버는 계속 최신을 주고 있었다 — 멈춰 있던 것은 화면이다"** 다. ①Irene: *"문서를 수정했는데도 링크에서 이전 내용으로 나와."* 나는 캐시를 의심했는데 아니었다 — API 는 `no-store` 였고 OG 는 DB 직독이었다. **공개 링크 페이지 12종이 전부 mount 때 한 번 읽고 그걸로 끝**이었다. 링크를 열어 둔 채 원본을 고치면 보는 사람은 새로고침 전까지 옛 내용을 본다. `hooks/usePublicRevalidate` 한 벌(탭 복귀 재조회 + 보이는 동안 60초 폴링, 숨으면 정지)로 전부 되살렸다. 공개 라우트는 소켓에 JWT 가 필요해 **로그인 화면의 계약(socket+visibility)을 그대로 못 가져온다** — 그래서 폴링이다. **입력 중에는 멈춘다**: 서명은 `phase==='review'` 일 때만(OTP·캔버스 중 갱신이 끼어들면 **그리던 서명이 사라진다** — 실측으로 review 1건 / 서명 단계 0건 확인), 청구서는 모달 열림 중 정지, 비밀번호로 연 페이지는 `pwRef` 로 **같은 자격**으로 다시 읽는다(갱신이 곧 잠금이 되지 않게). ②Irene: *"헤더가 q docs에서 위로 자꾸 올라가. 서브헤더까지 고정이 되어야지."* 밴드1+밴드2 를 **한 묶음**(`StickyBands`)으로 감쌌다 — `top: 60px` 같은 숫자를 주지 않는다. 폰에서 제목이 길면 밴드1 이 `height:auto` 로 자라 그 숫자가 곧 거짓이 되기 때문이다. 2026-09-08 의 "제목 밴드까지만"(본문 세로공간을 아끼려고) 결정을 **Irene 지시로 되돌린 것**이다. ③확인 필요의 영업 문의를 누르면 **Q sale 상담 탭과 같은 패널**이 열린다 — 서버가 `drawer.inquiry` 를 통째로 싣고, 변환(`utils/saleInquiryView`)과 등록+초대 호출(`services/saleRegister`)을 추출해 두 화면이 **같은 함수**를 부른다. ④[고객으로 등록]이 초대 메일을 **확인 없이** 보내던 것을 닫았다(대기열 24번의 "Fable 이 봐야 할 것 4번"). ⑤**내 측정이 두 번 거짓말했다** — 짧은 문서로 재서 바깥 셸 스크롤을 "헤더가 올라간다" 로 읽었고(긴 문서 픽스처로 다시 잡음), 패널과 확인창이 **둘 다 `[aria-modal="true"]`** 라 확인창 판정이 패널을 집어 거짓 통과 직전이었다. ⑥배포 게이트가 **내 변경과 무관한 데이터**로 막혔다 — 워크스페이스 3 에 `help@gitconsulting.group` 이 90일간 10건 도착했는데 "우리 주소" 에 없었다(그 주소로 온 명백한 요청이 '확인 권장' 으로 강등된다, #221 계열). **Irene 승인 후** 별칭 등록 → health-check 44/44. 이전 기록: **운영 배포 `c1c1b180` (00:14, v1.48.21 유지) — Fable 15회 전부 429.** 그때의 주제는 **"되돌릴 수 없는 일에는 확인이 있어야 하고, 검사기가 눈이 가려지면 제품을 고장으로 보고한다"** 다. ①Irene: *"확인을 받아."* [초대]·[초대 재발송] 은 누르는 즉시 고객에게 메일이 나갔다 — 이제 둘 다 **보낼 주소를 문구에 보여주고** 묻는다. 재발송은 주소가 없으면 비활성 + 이유(종전엔 눌리고 서버가 400 으로 조용히 거절해 사용자에겐 "아무 일도 안 일어남" 이었다). ②그 확인창을 **재려다** 공용 모달(`components/UI/Modal.tsx`)에 `role="dialog"`·`aria-modal` 이 **아예 없다**는 것을 알았다 — 하니스가 확인창을 **스코핑조차 못 했고**(§17 위반 상태), Esc 로 닫히지 않고 배경이 스크롤되고 Tab 이 뒤로 샜다. 훅 3종을 early return **위**에 넣어 ConfirmDialog 포함 **15곳**이 같이 고쳐졌다. ③같은 검증 중에 **`invite_token` 이 고객 목록·상세 응답에 실려 나가는 것**을 발견했다 — `c.toJSON()` 을 그대로 내보낸 탓이고, 그 토큰은 `register` 가 **"그 고객으로" 계정을 붙이는 자격증명**이다(고객 목록 읽기 권한만으로 남의 초대 링크를 만들 수 있었다). 7개 응답 지점에서 내리고 **health-check 새 카테고리 `secrets`** 로 게이트에 붙였다(응답 **원문 문자열** 판정 + 빈 응답 거짓통과 대조군). 배포 후 운영 실측 — 고객 7명 중 토큰 보유 **1건 · 미수락 0건** = 살아 있는 초대 링크가 없어 **회전 불필요**. ④**내 검사기가 거짓 사유로 빨간불이었다** — `toggles` 의 일정 "하루 종일" 실패는 **제품 정상 + 눈가림**이었다. 출근 자동기록 알림이 전면 백드롭으로 클릭을 먹었는데 판정은 "저장이 10초 안에 안 갔다"(= 제품 고장과 같은 말)고 보고했다. 이제 누르기 전에 **그 좌표가 내 것인지** 보고, `lib/browser.dismissBlockers()` 로 방해물을 먼저 닫는다 — `goto()` 에서 자동으로 부르지 않는다(`canary-modal-top` 은 그 알림 자체가 검사 대상이다). 좌표 판정은 **클릭과 같은 렌즈**로(puppeteer 는 누르기 전 스크롤한다) — 안 맞춰서 3건을 오탐했다. 이전 기록: **운영 배포 2회: `8c642085`(08:31) · `81cd75db`(09:47), v1.48.21 유지 — Fable 5회 모두 한도 초과(HTTP 429)라 두 배포 다 독립 검증 0회.** 오늘의 주제는 **"기준이 없으면 화면은 아무거나 보여준다"** 다. ①업무는 컨펌이 다 끝나면 곧바로 닫혔다 — 정작 일한 담당자가 결과를 못 보고 지나갔다(Irene: *"그러면 담당자가 확인을 못하잖아"*). `done_feedback`(승인완료·마무리 대기)을 완료 **앞**에 되살렸다. 이것은 2026-04-25 의 "자동완료 유지 + 알림만" 결정을 **되돌린 것**이다 — 같은 신고가 두 번 왔고(#282) **알림은 인지를 보장하지 못했다**. 값이 살아 있고 코드 경로만 0 이던 폐지값이라 **운영 ENUM ALTER 0건**이고 ko/en 4관점 라벨도 이미 있었다. 막다른 길을 막는 예외(컨펌자가 있어도 `done_feedback` 이면 담당자가 닫는다)가 핵심이다. ②Q sale 상담 목록은 **무엇이 상담인지 기준이 아예 없었다** — 고객이 한 마디도 안 해도 방이 있으면 올라왔다. "외부(비멤버) 발화 ≥ 1" 로 못 박았고, dev 미연결 대화방 9개가 **전부 외부 발화 0건**임을 DB 로 반증해 0건이 과잉 차단이 아님을 확인했다. ③같은 대화가 게스트·채팅 **두 줄**로 세어져 합계가 부풀어 있었다(914→912) — 행의 단위를 **대화방**으로 접고, 등록 가능 판정을 `source` → **`ref.kind`** 로 바꿨다. "채팅에서 고객이 이메일 넣으면?" 이 정확한 급소였다: 서버(`sale_save`)는 이미 그 링크로 등록할 수 있었고 **내 목록이 이메일을 버리고** 있었다 — 내가 "서버가 지원 안 한다" 고 보고한 것이 틀렸다. ④고객 정보를 **한 벌로** — 목록 직렬화가 이메일 셋을 하나로 접어 "어떤 주소인지" 를 말할 수 없었다. 상세에서 초대·계정·청구·세금계산서로 펴고 사업자 정보를 실어 **우측 패널·전체보기·설정**이 같은 응답을 나눠 쓴다(관리는 설정에만 — Irene 지시). 고객 탭은 페이지 전환을 버리고 우측 패널로, 이름은 `ClientLink` 공통 컴포넌트로. ⑤업무 추가가 메일 제목·본문을 통째로 베끼던 것을 **빈 입력**으로 바꾸고, 그 입력에 **실제 임시저장**을 붙였다(예외 선언으로 넘기지 않았다 — 래칫 89→88 복구). ⑥필터를 헤더에서 **탭 아래**로, "원본 열기" → **[보기]**. ⑦**내가 만든 검사기가 세 번 거짓말했다** — 게스트 행 0건을 `0===0` 으로 통과시킨 **빈 데이터 초록**, 헐거운 정규식이 앱 크롬의 "이번 주 마무리" 를 잡아 **드로어가 안 열려도 통과**, 앞 단계가 켜둔 "답변 필요만" 필터를 물려받은 **거짓 FAIL**. 셋 다 고쳐 다시 쟀고, 지금 카나리는 잴 대상이 없으면 통과시키지 않고 **판정 불가**로 떨어진다. ⑧일부러 **뺀 것**: 프로젝트 만들기·청구하기 액션 — `/projects`·`/bill` 이 고객 지정 쿼리를 읽지 않아(실측 0건) 지금 달면 죽은 링크가 된다. 이전 기록: **Q sale 사이클 1a "문의가 보인다" (자체 검증 · Fable 게이트 대기, 미배포).** 게스트 채팅·메일·전화로 들어온 문의를 한 곳에서 단계별로 보고 기록한다. 영업은 새 고객 테이블이 아니라 **`clients` 의 축**이고(허브 키가 이미 하나다), 단계를 바꾸는 문은 `setStage` 하나, 접근 종류·한도 술어도 각각 한 곳이다. 설계를 구현 직전에 코드와 대조해 **이후 계약과 부딪히는 6건**을 먼저 고쳤다(§16) — 배지 합산 규칙·NULL 보존·ENUM 3건(알림 두 테이블은 순서가 다르다)·온보딩 계수·상태 라벨 가드·Q Note 쓰기 권한. 자세한 것은 바로 아래. 이전 기록: **입력 임시저장 라운드 1A (자체 검증 · Fable 미검증, 미배포).** 댓글·수정요청·승인·확인요청·보류 사유·메모처럼 서버에 바로 저장하지 않는 글이 **닫거나 다른 업무로 가도 남는다.** 카나리를 돌리다 실제 버그 둘을 잡았다 — ①업무 전환 이펙트가 `revisionDraft.clear()` 를 불러 **드로어를 연 채 다른 업무를 누르면 쓰던 수정요청 메모가 지워졌고**(그 순간 초안 키는 아직 떠나는 업무 것이다) ②`NoteThread` 가 `onAdd` 뒤 **무조건** 비웠는데 넘기는 핸들러 셋이 모두 실패를 삼켜 **저장이 실패해도 메모가 사라졌다**(→ `onAdd: Promise<boolean>` 타입 계약). 그리고 내 양성 대조군 하나가 거짓이었다 — window 에 capture 리스너로 막는 차단은 at-target 이라 먼저 붙은 훅 리스너가 받는다(차단 3회가 찍혔는데 안 뒤집혔다). 이전 기록: **워크스페이스 단일 정본 단계 3~5 (`01d3c207`, Fable PASS, 운영 배포 `e6f2aab0`).** 창이 `X-Workspace-Id` 로 자기 워크스페이스를 싣고, 서버는 범위 인자가 없을 때 **정본(마지막 전환값)을 추측해 채우지 않는다** — 헤더가 정본과 다르면 409 `workspace_stale` 로 알리고 창이 따라간다. 설계는 관찰 1~2일 뒤 409·추측 제거를 넣게 되어 있었는데, 그 대기의 목적(옛 번들을 깨지 않기)을 **헤더 없는 요청만 종전 동작**으로 남기는 분기 하나로 지키고 한 번에 넣었다. 옛 번들 소진은 운영 로그 `[wsctx] summary` 의 `legacy` 로 판정한다. 이전 기록: **운영 배포 3회째 `1aa8f4c7`(13:19, v1.48.21 유지) — 워크스페이스 격리 2차.** 설계는 "id 로 여는 상세는 서버가 남의 항목도 200 을 준다" 를 전제했는데, 감사해 보니 캘린더·메일·Q info·파일·청구·고객 6곳은 URL 에 **현재** 워크스페이스를 넣어 불러 **새지 않는 대신 404** 였다 — 그래서 `isOtherWorkspace` 두 줄을 붙여도 영영 발화하지 않았을 것이고, 사용자에게는 "찾을 수 없음"/무반응으로 보였다. 문이 두 종류라는 것을 계약에 박고, 404 쪽은 서버 한 곳(`/api/entity-workspace` — business_id 만)에 묻게 했다. 반대로 URL 에 워크스페이스가 없는 자료정리 뷰어는 **남의 글을 실제로 그리고 있었다.** 같은 라운드에 알림 종 범위(Q7)와 검색 비밀 칸 매칭을 닫았다(Fable PASS). 이전 기록: 2026-09-11 **운영 배포 2회: `4aa971ef`(06:06) · `85261888`(08:42), v1.48.21 유지** — 오늘의 주제는 **"워크스페이스는 한 창의 사정이 아니다"** 다. 전환은 누른 창만 리로드했고(팝아웃·다른 탭은 옛 워크스페이스에 남았다), 서버는 범위를 모르면 첫 소속으로 **추측**했고, id 로 여는 상세는 **그 항목 자기** 워크스페이스 권한만 봤다 — 그래서 빈 워크스페이스 확인필요에 다른 워크스페이스 지연 업무가, 테스트 워크스페이스 탭에 워프로랩 노트가 떴다. 정본 자가치유 → 서버 추측 제거(+가드 wsscope) → 전 창 전파(WorkspaceSyncGuard) → 상세 "전환해서 열기" 까지 구조로 닫았다. 같은 날 신고 8건(메일 답변필요·채팅 알림 점프·외부컨펌 리스트업·새 소식 읽음·검색 강조 등)을 한 라운드로 Fable PASS. 이전 기록: 2026-09-11 (Opus 5) — **모바일 채팅이 카톡처럼 붙는다 (#409·#410) + iOS 키보드 위 ▲▼✓ 줄 제거(앱 재빌드 필요)** — **운영 배포 `4aa971ef` (2026-09-11 06:06, v1.48.21 유지)** · Fable PASS ×3(미검증이던 커밋 12건 + 경고 4건 + W1~W3) · 릴리즈노트 v1.48.21 운영 발행 · 개발 현황 id=78. 이전 기록: 2026-09-10 (Opus 5) — **v1.48.21 운영 배포 + 워크스페이스 격리 수리** — 오늘 늦게의 주제는 **"워크스페이스를 하나 더 만들자 원래 있던 결함이 드러났다"** 다. 확인필요·대시보드·Q Bill 이 `/api/dashboard/todo` 를 **범위 없이** 불러 소속 워크스페이스를 전부 합치고 있었고(사이드바 배지만 범위를 지켜 불러 **같은 화면에 두 숫자가 갈라져** 있었다), 범위를 줘도 새는 수집기가 하나 있었다 — `collectSignatures` 가 `businessId` 를 **받아놓고 안 썼다**(합산 모드에선 같은 행이 워크스페이스 수만큼 중복 계수). 관리자 탭은 `/admin` 이 kind 표에 없어 15개 화면이 전부 `other` 였고 그래서 ①이름이 언제나 "설정" ②identity 가 같아 **서로를 덮어썼다**. 관리자에서 `+` 를 누르면 워크스페이스 메뉴가 섞여 나와 누르는 순간 **직전 워크스페이스의 탭 목록이 통째로 복원**됐다. 기존 `scopetabs` 가 이걸 못 잡은 이유가 구조적이다 — 스냅샷이 탭의 `path` 만 담고 **`title` 을 버리며** 탭을 여는 문을 한 번도 안 누른다(신규 `--suite admintabs` 17/17, 양성 대조군 포함). 그리고 **워크스페이스 만들기 자체는 정상**이다(운영 #9 는 다른 워크스페이스와 같은 구조). 이전 기록: 2026-09-10 (Opus 5) — **v1.48.20 운영 배포** (오늘 배포 4회: v1.48.18·19·20 + OAuth 저장소) — ★ 오늘의 주제는 **"Fable 이 네 번 FAIL 을 냈고 네 번 다 진짜였다"** 다. Picker 배포 절차 누락(운영에 nginx 스크립트가 아예 없었다) · 메일 폭 맞춤의 높이/데스크탑 회귀(내가 주석에 적은 근거 둘이 모두 틀렸다) · 메일 제목 진단 오류(Gmail 도착 전에 이미 깨져 온다 — 우리가 못 고친다) · **OAuth 상태 DB 이관이 오히려 더 약했던 것**(동시 요청이 1회용·시도상한·replay 차단을 전부 뚫었다). 그리고 게이트가 **6일간 한 번도 안 울린** 두 구멍을 막았다 — 커밋하면 침묵하던 훅, 18일 켜진 채 잊힌 스킵 파일. 이전 기록: **v1.48.18 운영 배포 완료** (`aafd1192`) — ⓪**서버가 만든 글자도 사용자의 언어다** — 통계 인사이트·Cue 카드 문구를 서버가 한국어 문자열로 완성해 내려보내고 있었다(i18n 가드는 프론트만 훑어 **처음부터 검사 밖**). 코드+원시값 → 응답 경계에서 한 번 해석(알림 제목 선례). 곁가지로 **통화 '원' 하드코딩**(USD 워크스페이스도 '원')·**타임존 Asia/Seoul 고정**·**`req.user.language` 가 아예 안 실려 있던 것**·지출 카테고리 내부 코드 노출·**죽은 링크 1건**(`/insights?tab=people` 은 앱 화면이 아니라 공개 마케팅 블로그였다)·영어 화면의 "0원" 까지 닫았다. 신규 하드 게이트 `--category=statstext`(반증 12건). 이전 기록: **미배포 커밋 2건** — ①아이폰 알림 소리(배지 플러그인이 권한을 가로챘다)·채팅 바닥 고정·결과물 회차 라운드 ②메일 규칙 소급 적용·임시답변·통계 카드 줄·완료 수 공식·팝아웃 시작/중지·검색 선택행. 자세한 것은 바로 아래. 이전 기록: 2026-09-09 (Opus 5) — **v1.48.17 배포** — 종류별 휴가 · 자동저장 ✓ 통일 · 채팅 비밀값 마스킹 · 모바일 모달 15곳 상단 잘림. 자세한 것은 바로 아래 섹션. 이전 기록: **v1.48.15 배포** — **지운 자료는 처음부터 사라지지 않고 있었다(#408).** 09-08 에 "운영 DB·휴지통·백업 어디에도 없다" 고 결론냈는데 틀렸다 — 저장 라우트가 `custom_values` 를 **머지**해서 프론트가 키를 빼고 보내도 DB 에는 남는다. `custom_columns` 만 보고 `custom_values` 를 안 봐서 못 찾은 것이다(실측: doc#36 "TIME Internet" 의 링크·아이디 2건이 고아 키로 그대로 있었다). **없었던 것은 저장이 아니라 그 값을 다시 꺼낼 화면이다** → 상세에 "삭제된 항목"(되돌리기 / 완전 삭제), 옛 데이터는 이름을 못 살려 "이름 추정" 으로 표시. 곁가지로 ①**완전 삭제가 표현 불가능**했고(머지라 키를 빼도 안 지워짐) ②**공개 무인증 응답이 지운 값을 통째로 내보냈으며** ③화이트리스트를 씌우고도 카나리가 빨간불이라 찍어 보니 **`body`** 로도 샜다 — 항목으로 합성한 색인 본문이 항목이 바뀌어도 재계산되지 않아 공유 링크·검색에 계속 살아 있었다. 기계가 만든 본문일 때만 다시 계산하고(사람이 쓴 본문은 안 덮는다) 색인도 갱신한다. 신규 카나리 `--suite qinfoundo` 9/9(양성 대조군: 기능 무력화 시 5건 빨간불). **가드가 내 회귀를 잡았다** — 버튼 28px·배지 18px 로 UISPEC 래칫 702→705, 베이스라인을 올리지 않고 규격 36px 로 고쳤다. 곁들여 확인만 하고 손대지 않은 것 셋: emailIdle 재연결(2.6회/시간, 정상) · Drive 이관 대화방 축(이미 c21fc208 에 반영) · React #185(크래시 8건 전부 v1.48.14 이전, 이후 0건 — 다만 그 경로를 다시 밟은 양성 증거는 없어 "고쳤다" 가 아니라 "더 안 보인다"). 2026-09-08 (Opus 5, 1M) — **v1.48.14 배포** — **탭이 워크스페이스를 넘어 따라오던 것.** 저장 키는 이미 워크스페이스별로 갈려 있었는데 **기록이 전환보다 앞서** 있었다 — 경로가 먼저 바뀌면 그 순간의 범위(=옛 범위)에 새 경로가 탭으로 박히고 그 다음에 effect 가 범위를 갈아끼웠다. 키만 보면 갈라져 있어 정상으로 보이고, 새는 곳은 **키 안의 내용**이었다(실측 `::b5=["/talk","/tasks","/admin/dashboard"]` · `::admin=["/files"]`). 그 상태의 플랫폼 관리자 화면은 **에러 화면**이었다 — "탭이 섞인다" 와 "플랫폼관리자는 에러다" 는 같은 하나였다. 그리고 순서를 고쳐도 **이미 섞여 저장된 것은 코드가 안 지운다**(Irene: "여전히 리셋 안돼") — 읽는 순간 버리게 했다. 2026-09-08 (Opus 5, 1M) — **v1.48.13 배포** — Cue 가 파일을 **내용으로** 찾아 답한다(운영 59건 색인·청크 158) · 근태·휴가·주간보고까지 Cue 범위 · 메일 요약·검증(믿어도 되는지는 LLM 아닌 결정적 신호로만) · 평문 메일 링크·내용 복사·본문만 ⌘A · 반복업무 수정 범위가 태그·컨펌자까지 · 업무 팝아웃 정렬 · 주소별 메일 429(무한스크롤이 폴더 목록을 불렀다) · **렌더 크래시 자동 보고 + 관리자 16화면 크롤 카나리**(여태 /admin 은 게이트에 한 번도 안 들어와 있었다). 2026-09-07 (Opus 5, 1M) — **6차: 채팅 첨부가 안 열린 이유는 두 곳이었다** — ①매퍼가 `preview_url` 을 버려서 이미지가 전부 파일 카드로 떨어졌고(서버는 잘 보내고 있었다), ②첨부 다운로드 라우트가 **로컬 디스크만** 봐서 Drive 저장분은 언제나 404 `file_missing` 이었다. 미리보기도 같은 라우트로 본문을 받으므로 화면에서는 "눌러도 아무 일이 없다" 로 보인다. 그리고 파일 목록의 **출처는 배타적 폴더가 아니라 태그**다 — 접으면서 채팅·업무 분류를 통째로 잃고 있었다. 카나리 2종(양방향 반증 완료) 신설. 2026-09-07 (Opus 5, 1M) — **5차: 확인 필요 숫자가 목록 상한에 잘리고 있었다** — `total` 이 배열 길이라 수집기의 30건 상한이 **숫자까지** 잘랐다. 실측 확인 필요 35 인데 답장 필요 메일만 46건. 아침 신고와 같은 계열이다. 숫자는 참으로, 목록만 자르고 "외 N건" 을 말한다(35→51, 메뉴 배지 합과 정확히 일치). 팀 드라이브는 **옮기면 되고 연결 시 고르는 것은 권한상 불가**임을 실측으로 확정했다. — 4차: Fable 이 내 코드를 깼다(VERDICT: FAIL)** — 게이트를 커밋 뒤에 돌려 조용히 초록으로 넘기고 운영에 두 번 배포한 뒤에야 올렸다. Fable 은 그날 내가 만든 `ensureRootFolder` 가 **전이 오류(500·타임아웃·rate limit)에서 고객 Drive 에 두 번째 루트 폴더를 만들고 토큰을 덮어쓴다**는 것, **검색 키가 생성 키와 달라 남의 워크스페이스 폴더를 채간다**는 것, 새 문이 형제 라우트의 소유권 검사를 빠뜨렸다는 것을 실측으로 깨뜨렸다. 내 자체 검증은 전부 초록이었다 — 나는 잘 도는 경로만 쟀다. 핫픽스 완료. — 3차: 드라이브는 끊긴 적이 없었다** — 내가 "연동 끊김" 이라고 보고한 것이 틀렸다. 토큰도 폴더도 멀쩡했고 **DB 가 옛 폴더 id 를 가리키고** 있었을 뿐이다(404 → 자가복구 신설). 그리고 Drive API 는 `supportsAllDrives` 없이는 **공유(팀) 드라이브를 무조건 404** 로 주는데 그 파라미터가 코드에 하나도 없었다 — 팀 드라이브 중심 요구가 구조적으로 성립하지 않고 있었다. 컨펌 요청 알림도 컨펌자에게만 가서 요청자는 모르고 지나갔다. — 2차: 배지 두 숫자 · 파일 드래그&드롭 · 다운로드 % · Drive 첨부** — 1차에서 신고만 하고 못 고친 것들을 닫았다. 세 건 모두 원인이 같은 모양이었다: **서버는 되는데 화면에 문이 없었다**(폴더 이동 API 있음·드롭 존 없음 / 진행률 훅 있음·목록에 버튼 없음 / Drive 목록 있음·가져오기 없음). 그리고 확인필요 3건이 2건으로 보인 것은 집계에 '보낸 업무요청' 수집기가 **아예 없어서**였다. 이미지가 느린 것은 리사이즈가 안 돼서가 아니라(1.37MB→31KB 잘 됨) **라이트박스가 원본을 그대로 열어서**였다. — 1차: **운영 배포 3회 · 커밋 3건 · 신규 카나리 5종** — 오늘의 주제는 **"보이는 것이 곧 있는 것은 아니다"** 였다. ①출근 알림이 우측 패널 뒤로 깔리고 태블릿·폰에선 아예 안 보였다 — 원인은 z-index 가 아니라 **층**이었다. 모달을 그리는 위젯이 사이드바(`position:fixed; z-index:100`, 태블릿 이하는 `transform`) 안에 있어 1100 이 페이지 기준 100 이었다. 숫자를 올려서 될 일이 아니라 **층 밖으로 나가야** 했다. 전면 백드롭 74개 중 66개가 포털을 안 쓰고 있어 래칫으로 동결했다. ②메일 본문 아래 130px 빈 여백은 첨부 탓이 아니었다 — `srcdoc` iframe 이 **부모 CSP 를 물려받아** 9월 2일에 넣은 `script-src 'self'` 가 높이 보고 스크립트를 **조용히 죽이고** 있었다. 높이가 한 번도 안 나가니 636px 내용이 200px 추정치 상자에 담겼다. CSP 를 풀지 않고 스크립트를 파일로 뺐다. ③업무 책임선에서 직급 예외를 걷었다(Irene: "owner든 admin이든 누구든 그 기준이 맞는 거 아냐?") — 직급으로 남의 글을 덮으면 "누가 왜 고쳤는지" 가 원장에 남지 않는다. ④업무 재개가 다른 기기에 1분 늦던 것은 **포커스 위젯에만 소켓 리스너가 없어서**였다(근태는 305ms). ⑤**내 측정이 다섯 번 거짓말했다** — 밴드2 대신 본문을 집고, 근무중일 때도 있는 "휴게" 버튼 때문에 판정이 영영 참이 못 되고, 사이드바 시계가 포커스 타이머 정규식에 걸리고, 서버가 `custom_columns` 로 내보내는 값을 옛 키로 보고, 빌드 산출물만 고치고 재빌드를 안 해 대조군이 안 뒤집혔다. ⑥**내가 만든 검사기가 다른 검사를 네 번 죽였다** — 남긴 알림이 클릭을 가로채고, 그 원복이 rate-limit 429 에 막히고, early return 이 러너 계약을 어겨 FATAL 이 나고, 공유 DB 풀을 닫았다. ⑦styled 주석에 백틱을 넣어 빌드를 깨뜨렸고, 고치려던 일괄 치환이 중첩 템플릿 1,429곳을 손상시켰다 — HEAD 와 줄 단위 대조로 822줄만 되돌리고 편집 24개를 표식으로 전수 확인했다(유실 0).
