...
> **최종 업데이트:** 2026-09-04 (Opus 5, 1M) — **운영 배포 5회 · 커밋 6건** — 주제는 **"한 변경이 남긴 회귀 둘, 그리고 내 검증이 실사용 조건이 아니었다"** 였다. ①안드로이드를 **링크로 앱이 열리는 상태**까지 올렸다(앱 서명 SHA-256 반영). Play Console 개편으로 서명 키가 메뉴에서 사라져 세 번 헛짚은 끝에 `/keymanagement` 주소로 직행해 찾았고, 그 경로를 런북에 박았다. ②**#405(탭 워크스페이스별 보관)에서 회귀가 둘 나왔다.** 저장 키를 가르면서 `state` 를 통째로 갈아끼웠는데, 그 안에 **부팅 경로**와 **실행 모드(mirror)** 두 가지가 같이 실려 있었다. 둘 다 저장소에서 읽으면 안 되는 값이다. 첫 번째는 주소로 들어온 경로가 마지막 탭으로 튕기는 것(알림·공유·딥링크 전부), 두 번째는 **데스크탑 앱 좌측 메뉴 전멸**이었다. ③**그 두 번째를 내 검증이 세 번 놓쳤다.** `/login` 에서 시작하면 앱이 `shell` 모드로 굳어 사이드바가 멀쩡하다 — **PWA 는 로그인된 채 앱 경로로 부팅**해 `tab` 모드로 들어간다. Irene 의 *"탭에서 열면 가지는데 좌측 메뉴가 다 안눌려"* 한 마디가 범인을 지목했다(탭 막대는 `setActive`, 사이드바는 `navigateActive` — 다른 함수다). ④**#211 세 갈래를 열었다** — ①도달(고객 링크 진입 안내), ②시간→청구, ④첫 응답 시간. 셋 다 설계를 **운영 실데이터가 정했다**: 이 팀의 청구는 월정액이라 시간은 덧붙이는 항목이고, 실제시간이 든 업무가 26%뿐이라 숫자만 주면 적게 청구하게 되므로 **기록률을 함께** 띄운다. ⑤**외부 프로젝트 열람 설계 결정 3건이 승인**됐다(보안등급 축 · 멤버 이상 발급 · 담당자 표시명 노출).
