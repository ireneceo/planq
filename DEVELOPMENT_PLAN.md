...
> **최종 업데이트:** 2026-08-29 (Opus 5, 1M) — **운영 배포 3회 + 파일 휴지통 신설 · #390 · #284** — 오늘의 주제는 **"검사기가 빨간 이유를 끝까지 따라갔더니 제품 버그였다"** 였다. 파일 휴지통 카나리가 계속 실패했는데 원인이 하니스가 아니라 **`/files` 한 번 로드가 API 요청 1,099건(한도 600/분)을 쏘던 것**이었다 — 그래서 다음 부팅의 refresh 가 429 를 받아 **로그인 화면으로 튕겼고**, 카나리는 그걸 "화면이 안 떴다" 로 읽었다. 실패 메시지에 경로(`=/login`)를 찍게 만든 뒤에야 드러났다. **파일 화면을 한 번 여는 것만으로 사용자가 1분간 잠기는** 상태였고 실사용자도 그대로 닿았다. "판정 불가를 통과로 넘기지 않은 것" 이 이 버그를 살렸다. 그 밖에 ①**삭제가 되돌려지지 않던 것** — `deleted_at` 은 찍혔지만 **같은 함수가 바이트까지 지우고 있어서** 복구 화면만 붙였으면 복구 버튼이 거짓말을 할 뻔했다. 삭제를 휴지통행/영구삭제 2단계로 갈랐다. ②**#390 공유 링크가 아예 안 만들어지던 것** — `Number('direct-4525')` = NaN 이 URL 에 그대로 실려 없는 경로를 때렸고, 404 가 HTML 이라 화면엔 아무 말도 안 떴다. ③**#284 문서→Info** — AI 항목 분리 엔진은 이미 있었는데 **그 경로만 안 쓰고 있었다**(없는 기능이 아니라 연결이 끊긴 것). ④모바일 체감 3건 — 앱 콜드스타트가 지난 화면으로 납치되던 것, 부팅 왕복 2→1, 메뉴 재진입 즉시 표시, Cue 입력이 키보드 뒤로 들어가던 것.
