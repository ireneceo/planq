...
> **최종 업데이트:** 2026-07-25 (Opus 5, 1M) — **#200(b') 메일 참여자 백필 + PortOne 걷어내기 + 결제 설정 재구성 + 약관·랜딩 사실 정정 (Fable 게이트 설계 2 + 구현 4라운드 PASS, 운영 배포 완료 `644ae95`)** — ①**#200(b') email_threads.participants**: 지난 사이클에 근본원인(in-place push)은 고쳤으나 옛 데이터 954행이 `[]` 그대로. 실측 중 **쓰기측이 아직 반쪽**임이 드러남 — 답장·작성·전달 3경로가 participants 를 아예 안 써서 백필해도 재축적되는 구조였다. `services/emailAddress.js` 신설(문자열/`{email,name}`/`{address,name}` 3형태 흡수 단일 원천) + 술어 통일("계정 주소·별칭이 아닌 모든 from/to/cc", bcc 는 은닉 수신자라 제외). **Fable 설계 게이트가 최초 술어(inbound to 를 버리는 비대칭)를 실측으로 반려** → 대칭 술어 B 로 개정(참여자0 이 운영 235→17, dev 912→29). 부수 해소 3건: `email_threads.js:696` `x?.address` 파싱 오류로 **별칭 답장 발신주소 선택이 죽어 있던 잠복 버그**(별칭 등록 즉시 발현할 지뢰) · step3 후보 조회 무순서(백필이 여는 매칭 표면 → `last_message_at DESC` 착지 고정) · `participantsEqual` 의 JSON.stringify 비교가 **MySQL 의 JSON 키 길이순 재정렬** 때문에 매 실행 전건 UPDATE 되던 멱등 파괴. ②**PortOne 걷어내기**: 입력 경로만 절단(관리자 섹션·GET 응답 필드·PUT 저장 4필드·i18n 키), **`Payment.method` ENUM `'portone'`·결제내역 라벨·DB 컬럼은 보존**(과거 이력 표시 보호 — 실측 결과 portone row 는 dev·운영 0건이라 이력 리스크조차 없었으나 파괴적 마이그레이션 회피). ③**결제 설정 화면 재구성**("넣어야 하는 것만 딱" — Irene): 결과→입력 순서로 뒤집어 현황 카드(켜짐/꺼짐 + **부족한 항목 명시**) → 2단계 체크리스트(Secret·Webhook, 각 단계에 출처·Stripe 딥링크·엔드포인트 URL 복사) → 계좌이체 → 정책. **Publishable Key 강등** — Fable 이 "결제가 전부 Stripe 호스티드라 `loadStripe` 호출 0건 = 이 값을 쓰는 코드가 없다"를 실증, 운영에 사람 이름(`irene`)이 저장돼도 무해했던 진짜 이유. 웹훅 URL 은 `window.location.origin` 파생(dev 화면이 운영 URL 안내하던 사고 차단). 입력란 `width:100%` 누락으로 브라우저 기본 폭(~177px)에 렌더되던 것 정렬(실측 15개 전부 630px). ④**Stripe 키 형식 검증**: `pk_`/`sk_`·`rk_`(restricted 허용)/`whsec_`, 빈 문자열(삭제)은 통과. 프론트+백엔드 양쪽(프론트만 막으면 API 직호출로 재발). 활성 판정을 **서버 계산 `stripe_enabled` 단일 원천**으로(암호화 키 회전 시 거짓 "켜짐" 차단 + 전용 경고). ⑤**약관·랜딩 사실 정정**: `legal.json` ko/en 3곳 교체(계좌이체 경로 신규 기술 · 워크스페이스 Q Bill 결제 구조 = 개인정보처리자가 워크스페이스 운영자임 명시 · 미제공 수단 카카오페이·네이버페이 삭제), 시행일 2026-08-01, **버전 bump 없음 = 재동의 미트리거**(제3자 제공의 축소·정정이라 §10 '중요한 변경' 아님 + 뒤집히는 조건 4가지 박제). 랜딩 4곳 — "팝빌 세금계산서 자동"(실제는 외부 발행 후 마킹) 정정, **연혁의 "PortOne 라이브"는 그 시점에도 사실이 아니었음이 실증돼 제거**(Opus 의 "연혁=사실 기록" 전제를 Fable 이 반증). `docs/LEGAL_UPDATE_2026-08-01_ROLLOUT.md` 신설(공지 절차 + 기한 컨틴전시). **Fable 게이트 6회**(설계 CONDITIONAL PASS ×2 → 구현 FAIL ×2 → PASS): FAIL 사유가 전부 실제 결함이었다 — i18n 래칫 427/426(영어 UI 에 `sk_ 또는 rk_` 한·영 혼합) · 약관 공지 절차 리포 미문서화(법정 기한 작업 누락 위험) · 연혁 거짓 기재. **운영 배포**(`644ae95`, Complete 222s): 3점 실측(PM2 uptime 24s · 청크 해시 dev=운영 `index-DTlvEw73.js` · 백엔드 코드 착지) + **운영 백필 완주**(dry-run 952 → 스냅샷 → apply 6s → 멱등 재실행 변경 0 → 빈 participants 954→17 · `updated_at` 990/990 보존). **다음: 약관 개정 공지(Irene, 오늘이 7일-전 기한 마지막 날) · Stripe 키 입력 · #206 Q Task 보류(ENUM) · #208 출퇴근·휴가**.
