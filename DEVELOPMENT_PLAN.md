...
> **최종 업데이트:** 2026-09-19 (Opus 5, 1M) — **운영 배포 3회: v1.52.13 · v1.52.14 · v1.52.15.** 이번 구간의 주제는 **"신고된 한 곳은 대개 계열의 한 곳이다"** 와 **"내 검사가 초록이면 정말 잰 것인지부터 의심한다"** 다. ①**상담의 축을 정했다** — Irene: *"업무처럼 취급되면 될지 프로젝트나 고객처럼 연결이 되어야 할지 모르겠는데."* 상담은 **고객 축의 기록**(`client_interactions`)이다. 업무는 «누가 언제까지» 라 상태가 변하고 상담은 «언제 무슨 접점» 이라 변하지 않는다. 고객이 이미 허브이므로 상담을 옆에 세우면 허브가 둘이 된다. 메일·채팅·Q note·수기 **네 입구가 한 원장**에 착지한다. 출처(`source_kind='qnote'`)는 화면이 고르지 않고 **서버가 노트 소유를 확인한 뒤** 박는다(fail-closed). ②★ **«저장 응답이 화면을 지운다» — 신고 1곳에서 시작해 8곳을 찾았다.** 신고는 *"Q note에서 프로젝트 연결했더니 갑자기 아무 내용도 없다고 나왔어"* 였다. `PUT` 응답에는 `utterances`·`documents`·`speakers` **키가 아예 없는데**(`GET` 에만 붙는다) 화면이 그것으로 상태를 통째로 갈아끼웠다. **서버는 멀쩡하고 화면만 잃어** 새로고침하면 돌아오므로 오래 안 보였다. 같은 모양이 Q note 공개범위 변경 · Q docs 문서↔표(연결 문서 목록이 사라진다) · 프로젝트 멤버 저장(권한 표시가 바뀐다) · 메일 설정 · Q sale 요약 · 보고 단위 ×2 · 확인필요 일정 수정에 있었다. → `applySessionPatch` · 호출부 병합 · **가드 `--category=savemerge` 신설**. ★ **그 가드가 처음엔 정작 신고된 형태를 못 잡았다** — 저장 함수 추출기가 여러 줄 시그니처에서 **매개변수 타입의 `{`** 를 본문 시작으로 오인해 `linkSessionEntities` 가 목록에서 빠졌다. 경계를 «다음 export» 로 고치니 추적 25 → **59개**, 그 자리에서 6곳이 더 나왔다. ③**공용 팝오버에 왼쪽 클램프가 없었다** — 트리거가 화면 왼쪽이면 패널이 `x=-111` 로 나가 **«눌러도 아무 일 없는 버튼»** 이 된다. 훅 한 곳을 고쳐 전 팝오버가 같이 해결(오른쪽 팝오버는 안 움직이는 것을 실측 확인). ④**고객 이름 라벨이 통째로 비어 있었다** — 응답의 `name` 은 null 이고 정본은 `display_name`. 옵션 17개가 전부 공백이라 고를 수가 없었다(기능 사망). `SessionLinkBar` 도 같은 결함. ⑤★ **Q note 업무추출이 서버만 있고 화면이 0곳이었다** — 라우트 4개·서비스 4개가 **2026-06-05 부터** 있었는데 부르는 화면이 없었다(3개월 반). 신고 #382 의 «업무추출되게 하고» 가 여기서 막혀 있었다. 붙이고 나니 후보가 `project_id: null` **고정**이라 프로젝트 안에서 뽑아도 워크스페이스 업무로 떨어지던 것, [다시 추출] 이 **LLM 의 다른 표현** 때문에 쌓이던 것도 드러났다(이미 뽑힌 제목을 프롬프트에 실어 해결). ⑥**연결 입력 문구 13곳을 정본 하나로** — 같은 뜻에 5가지가 돌았다(「프로젝트 없음」·「연결 없음」·「프로젝트를 선택하세요」…). 축은 둘뿐: **NONE**(연결 바) / **PICK**(생성 폼). ⑦★ **내 검사기가 하루에 아홉 번 거짓 초록이었다** — 선택자 오집(메일 뱃지를 행으로, 업무를 일정으로) · 픽스처 미생성(`last_insert_rowid()`=0) · keep-alive 로 **같은 화면을 5번** 읽음 · API 가 401 인데 통과 · 모달 저장이 막혔는데 통과. 전부 «아무것도 재지 않아서» 초록이었다. → 판정 **앞에** «전제가 성립했는가» 를 따로 판정하고, 잰 것이 없으면 **⬜ 미측정**으로 찍는다. memory `feedback_green_without_measuring`. ⑧**Fable 토큰이 v1.52.14 도중 소진**됐다 — 이후 전부 **자체 검증**이고 커밋 메시지·게이트 마커(`unavailable`)·`docs/FABLE_GATE_QUEUE.md` 에 **미검증으로 남겼다.** «통과» 라고 쓰지 않았다. ⑨**운영 피드백 14 → 8건** — #407·#413·#414·#416·#420·#421 답글 달고 닫음. 남은 8건은 `docs/WORK_TODO_2026-09-19.md` 에 «왜 안 됐나» 까지 적었다(★ 그 문서 맨 위에 **내 오보 정정**이 있다 — #417 의 «안 된 것» 을 커밋 메시지만 보고 추측해 틀리게 보고했다).
