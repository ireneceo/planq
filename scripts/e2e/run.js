...
const SUITES = {
 ...
 // 탭 이름이 내용으로 유지되는가 — LRU 정지(alive:false)로 pane 이 언마운트돼도 되돌아가면 안 된다
  ...
  // 업로드 배치가 조용히 잘리지 않는가 (2026-09-24). 429 를 «대기» 로 읽고 스스로 이어서 올리는가 ·
  //   끝내 실패했을 때 [전체 재시도]·[실패 목록 지우기] 로 **한 번에** 조치할 수 있는가.
  //   ★ 429 는 network 계층에서 주입한다 — 실제로 상한을 때리면 느리고 잔여가 남는다.
  uploadretry: () => require('./canary-upload-retry'),
  // 폴더 삭제가 «묻는가» · 폴더째 업로드 손잡이 · 같은 이름이면 «묻는가» (2026-09-24).
  //   서버 문은 실 API 로 따로 확인했다 — 여기서 재는 것은 «그 문이 화면에서 열리는가» 다.
  folderops: () => require('./canary-folder-ops'),
  // 같은 이름 재업로드 — **폴더 계열과 분리한다**(2026-09-24). 한 브라우저에서 이어 붙이면
  //   앞 구간이 화면을 옮기고 트리를 접어 뒤 구간이 무너졌다(실측: 같은 코드로 17/4 ↔ 15/타임아웃).
  dupname: () => require('./canary-dupname'),
