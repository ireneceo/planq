// 파일 휴지통 카나리 — 2026-08-29 신설. run.js `--suite trash`.
//
//   ★ 이 카나리가 계속 실패했고, 그게 **제품 버그를 찾아냈다.** 실패 원인은 하니스가 아니라
//   `/files` 가 한 번 로드에 API 요청 1,099건(한도 600/분)을 쏘던 것이었다 — 그래서 다음
//   부팅의 refresh 가 429 를 받아 로그인 화면으로 튕겼고, 카나리는 그것을 "화면이 안 떴다"
//   로 읽었다. 실패 메시지에 경로(=/login)를 같이 찍게 만든 뒤에야 드러났다.
//   요청 폭주를 고치자(commit fc01170b) 3/3 통과했다.
//
//   교훈: "판정 불가" 를 통과로 넘기지 않은 것이 이 버그를 살렸다. 빨간 카나리를 끄고
//   넘어갔으면 사용자가 파일 화면 한 번 열고 로그아웃되는 것을 아무도 몰랐다.
//
