...
## Fable 검증 게이트 (전 설계·검증 무조건 — Irene 지시 2026-07-21)

**정책: 모든 기획/개발 설계, 그 설계에 대한 검증, 구현 검증, 테스트 검증은 무조건 Fable 이 수행한다.**
개발=Opus / 설계·검증=Fable(별도 모델, `claude-fable-5`) 완전 분리. Opus 는 설계·구현하고, **Fable 이 독립적으로 검증**한다. **Opus 가 "스스로 검증 완료" 로 완료 보고하는 것 금지** — 반드시 Fable 게이트 통과가 완료 조건. (실행: 검증 시점에 `model: 'fable'` 서브에이전트를 띄워 독립 검증시키고 그 판정을 근거로 보고.)

### 무조건 적용 — 3 게이트 (예외는 아래 경량 1건만)
1. **설계 게이트** — 기획/기능/기술 설계는 Fable 이 타당성·절단면·리스크·누락을 독립 검토한 뒤 구현 착수
2. **구현 검증 게이트** — 구현 완료 후 Fable 이 아래 ①~④ 로 검증. Opus 자체 검증만으로 완료 보고 금지
3. **테스트 검증 게이트** — 테스트(단위·실HTTP·e2e)는 Fable 이 실행/판정. "코드상 맞다" 로 대체 불가

### 특히 필수 — 고위험(설계·구현·배포 3단 전부 Fable, 절대 생략 불가)
...
### 유일한 경량 예외
오타·주석·로그 문구 등 **사용자 로직/데이터/화면에 영향 0 인 순수 텍스트 한 줄 변경**만 `/검증` 경량 절차 허용(설계도 테스트도 없는 편집). **판단이 애매하면 무조건 Fable 로 올린다 — 의심되면 게이트.**
...
### 3) 상세 액션이 많으면 — 헤더 아래 **액션 줄** (2026-09-05 박제)

패널 헤더(60px)는 **제목 자리**다. 액션이 서너 개를 넘으면 헤더에 밀어넣지 말고 헤더 **아래
한 줄**로 내린다. 그 줄의 세로 밴드는 좌측 리스트의 **검색줄과 같게** 둔다 —
`padding: 12px 20px 8px` + 내용 높이 36px(= `min-height: 56px`) + `border-bottom: 1px solid #F1F5F9`.
그래야 좌우 패널의 옅은 밑줄이 같은 y 에서 이어진다(헤더 60px 계약과 같은 이유).

- 왼쪽 = 자주 쓰는 액션 **3개 이내** (글자 버튼)
- 오른쪽 = 나머지는 `components/Common/OverflowMenu.tsx` 의 **⋯ 하나**로 접는다
- 같은 화면의 보기/편집 모드는 **둘 다** 이 구조로 — 한쪽만 두 줄이면 모드 전환 때 실선이 튄다

> Irene 2026-09-05: *"서브헤더에 버튼이 너무 많아. 제목이 보이지도 않아."*
> Q docs 상세 실측(1440px) — 헤더 920px 중 액션이 534px, 제목 칸은 304px(33%)였다.
> **아이콘 전용 버튼을 늘리는 것은 자리를 먹으면서 뜻은 안 알려주는 최악의 조합**이다.
> 메뉴로 접으면 자리를 돌려주고 글자 라벨이 붙어 오히려 읽힌다.

적용처: Q docs 상세(`components/Docs/PostsPage.tsx`). 신규 상세 화면도 이 구조를 쓴다.

> **⏭ 다음 사이클 예정 — 두 밴드로 더 줄인다 (Irene 2026-09-05 후속, 미구현)**
> *"제목은 다 나와야겠다. ⋯ 이 점점점 메뉴만 위로 올리면 어때? 그리고 2, 3째줄 한줄로 맞추고
> 버튼들은 우측 정렬이 맞겠는데."*
>
> 버튼을 ⋯ 로 접은 뒤 액션이 **534px → 254px** 로 줄어(편집 71·공유 69·서명 98·⋯ 36),
> 이제 별도 액션 줄이 없어도 된다. 목표 구조:
>
> | 밴드 | 왼쪽 | 오른쪽 |
> |---|---|---|
...
>
> - 제목 옆에 ⋯(36px)만 두면 1440px 에서 제목 몫이 **872px** 이다(액션을 전부 올리면 640px).
> - 지금의 `DetailActionBar` 밴드는 사라진다. **보기·편집 두 모드를 같이** 바꿔야 한다 —
>   한쪽만 두 밴드면 모드를 오갈 때 가로 실선이 튄다.
> - 폰(390px)에서는 버튼 묶음이 메타 아래로 감기게 둔다. 한 줄에 우겨넣으면 둘 다 못 읽는다.
