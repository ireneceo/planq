...
## 0-A. 통합 기준 페이지와 우선순위 (2026-09-16)

PlanQ 프론트엔드의 시각적 통일 기준 페이지는 **Q Task, Q Talk, Q Docs**다. 신규 화면이나 기존 화면을 정비할 때 이 세 영역에서 실제 사용량이 높고 완성도가 높은 구현을 기준으로 삼는다.

기준이 서로 다르면 다음 순서로 결정한다.

1. 같은 목적의 공통 컴포넌트 중 **가장 많이 사용되는 구현**
2. 같은 사용 목적이라면 **완성도 높은 페이지(Q Task·Q Talk·Q Docs)의 구현**
3. 그래도 다르면 PlanQ 공통 컴포넌트와 이 가이드의 토큰
4. 그래도 판단되지 않으면 새 변형을 만들지 말고 사용자 또는 Fable의 결정을 받는다

### 통일 대상

- 페이지 셸, 패널 헤더, 목록·상세 레이아웃과 반응형 전환
- 버튼 톤·크기·높이·간격·로딩 상태
- 모달, ConfirmDialog, 드로어, 모바일 bottom sheet 등 오버레이
- 입력·셀렉트·탭·필터·검색·빈 상태·에러 상태
- 제목·본문·메타 텍스트, 배지, 아이콘, 여백과 정렬
- hover/focus/pressed/disabled 상태, 키보드 조작, 닫기 방식

### 구현 원칙

- 시각적 차이만 있는 로컬 styled 컴포넌트·팝업·버튼을 새로 만들지 않는다. 기존 공통 컴포넌트와 토큰을 우선 사용한다.
- 같은 역할의 구현이 여러 개면 새 구현을 기준으로 삼지 말고, 기준 구현에 맞춰 통일한다.
- 공통 컴포넌트의 동작·접근성·권한 계약은 바꾸지 않고 시각 규칙만 통일한다.
- `alert`, `window.confirm`, 임의 토스트, 페이지별 커스텀 팝업, 임의 버튼 색상·높이는 금지한다.
- 데스크톱만 맞추지 말고 모바일·태블릿에서도 같은 기준이 유지되는지 확인한다.
- 디자인 통일 작업은 소스·DB·API·배포 설정을 변경하지 않는다.
### 디자인 작업의 자동 마무리

디자인 범위로 수행한 작업은 Codex가 자기 변경을 직접 검증한 뒤 저장·개발완료까지 자동 처리한다. 개발 기능이나 구조 변경이 없으면 별도 승인 대기로 작업 섹션을 열어 두지 않는다.

- 프론트엔드 빌드와 변경 범위 검증을 실행한다.
- 작업 주체별 변경 기록과 검증 결과를 남긴다.
- 커밋·푸시·백업까지 처리한다.
- 배포는 하지 않는다. 배포는 Claude Code가 별도 지시와 절차에 따라 수행한다.
- 검증에서 발견된 기존 시스템 문제는 임의 수정하지 않고 완료 보고에 분리 기록한다.

### 검토 체크

각 화면을 검토할 때 기준 페이지와 나란히 비교하고 다음을 기록한다.

- 기준 컴포넌트와 다른 점
- 기준으로 선택한 근거(사용량 또는 완성도)
- 수정한 시각 요소
- 모바일·태블릿·데스크톱 확인 결과
- 동작 변경 없음 여부
...
#### 예외 — **채팅형 한 칸 입력은 Enter 로 보낸다** (2026-09-14, Irene 지시)

> Irene: *"입력란 + Enter 전송. 모든 곳 동일한 형태야. 다른 곳도 찾아서 수정해."*

위 금지는 **여러 칸을 채워 한 번 제출하는 폼**의 규칙이다. 칸이 하나뿐이고, 짧은 글을 여러 번
보내며, **보낸 뒤 본인이 바로 지울 수 있는** 입력은 성격이 다르다 — 거기서 ⌘+Enter 를 요구하면
"보내는 방법을 모르는 칸" 이 된다. 해당하는 것: **메모·이슈 스레드**(`components/Common/NoteThread`)
· Cue 바(`CuePanelSection`) · 채팅 입력.

이 예외를 쓸 때 지켜야 할 것:
- **줄바꿈은 Shift+Enter** (채팅 표준). 줄바꿈 길이 아예 없으면 안 된다.
- **IME 가드는 필수** — `utils/imeKey.isEnterAction`. 한글은 Enter 가 마지막 음절을 확정하는
  키이기도 해서, 가드가 없으면 "안녕하세" 까지 쓴 글이 전송된다
  (memory `feedback_ime_enter_needs_guard`).
- **안내 문구가 동작과 같아야 한다** — 칸의 placeholder 에 "Enter 저장 · Shift+Enter 줄바꿈".
  2026-09-03 에 동작은 Enter 인데 안내만 ⌘/Ctrl+Enter 로 남아 있던 전례가 있다.
- **폼의 제출 버튼에는 적용하지 않는다.** 되돌리기 어려운 것(생성·승인·발송)은 종전대로.
...
### 1.11 문서 본문의 **표는 기본으로 폭을 채운다** (2026-09-13)

> Irene: *"문서에 표가 왜 내용까지만 테두리가 생기고 좌우 열이 자동 맞춤이 안되는 거야?
> 기본 반응형 안잡아줘?"* · *"표를 좌우로 맞추면 열이 잘 나눠져야 하는데 우측이 빈 여백이 되어 버리는데?"*

| 상황 | 계약 |
|---|---|
| 표를 넣었을 때 | **본문 폭을 채우고 열이 고르게** 나뉜다. 아무것도 안 눌러도 그래야 한다 |
| 열이 많아 다 못 들어갈 때 | 눌러 넣지 않고 **래퍼 안에서 가로 스크롤**(글자가 잘리는 것보다 낫다) |
| [폭 맞춤] 을 눌렀을 때 | 지금 **본문 폭**을 열 수로 나눈다. 고정 px 을 넣지 않는다 |

- **`width: 100%` 를 쓰지 않는다** — 넓은 표를 강제로 끼워 넣어 **우측이 잘린다**(옛 회귀).
  `width: max-content` + `min-width: 100%` 가 맞는 조합이다.
- ★ **그 `min-width: 100%` 는 `!important` 여야 한다** — prosemirror-tables 가 표 DOM 에
  **인라인으로** min-width(열 합계 px)를 쓴다. 인라인이 스타일시트를 이기므로, 그것 없이는
  우리가 적은 100% 가 **한 번도 적용되지 않는다**(실측: 새 3열 표가 872px 본문 안에서 290px).
...
### 2.3 상세 액션 줄 — 헤더 아래 한 줄 (2026-09-05)

> Irene: *"서브헤더에 버튼이 너무 많아. 제목이 보이지도 않아. 좌측 검색창 가로라인 맞춰서 헤더 2줄로 가야 하는 거 아니야?"*

패널 헤더(60px)는 **제목 자리**다. 상세 화면의 액션이 서너 개를 넘으면 헤더에 밀어넣지 않는다.

**실측(Q docs 상세, 1440px) — 고치기 전:** 헤더 920px 중 액션 10개가 534px, 제목 칸 304px(33%).
1440px 에서도 제목이 3분의 1이라, 노트북 폭이나 긴 제목에서는 바로 잘렸다.

```
┌ PanelHeader 60px ────────────┬  좌측 리스트 헤더와 같은 줄
│ 제목                          │
├ DetailActionBar ─────────────┼  좌측 **검색줄과 같은 밴드**
│ [편집][공유][서명 받기]    ⋯  │
└──────────────────────────────┴
```

**밴드 값(좌측 `SearchWrap` 과 한 벌 — 한쪽만 바꾸면 밑줄이 어긋난다):**
- `padding: 12px 20px 8px` · 내용 높이 36px → `min-height: 56px` (box-sizing: border-box)
- `border-bottom: 1px solid #F1F5F9` · 배경 `#FFFFFF`

**규칙:**
- 왼쪽 = 자주 쓰는 액션 **3개 이내**, 글자 버튼(3톤 §1.7)
- 오른쪽 = 나머지는 `components/Common/OverflowMenu.tsx` **⋯ 하나**로. 메뉴 항목은 **글자 라벨**
- 보기/편집 모드 **둘 다** 같은 구조 — 한쪽만 2줄이면 모드 전환 때 가로 실선이 튄다
- **아이콘 전용 버튼 4개 이상 나열 금지** — 자리는 먹으면서 뜻은 hover 해야 아는 최악의 조합
