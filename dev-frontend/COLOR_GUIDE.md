...
## 2.5 Point 컬러 — Coral/Rose (포인트 컬러, 절제 사용)

> **PlanQ 브랜드 포인트 컬러.** Primary 딥틸과 보색 페어링.
> **사용 빈도: 한 화면에 1-2번 정도로 절제. 남발 금지.**

| 단계 | HEX | 용도 |
|------|-----|------|
| Point 50 | `#FFF1F2` | 포인트 라이트 배경 (질문 카드 배경 등) |
| Point 100 | `#FFE4E6` | 포인트 뱃지/태그 배경 |
| Point 200 | `#FECDD3` | 포인트 보더, 강조 라인 |
| **Point 500** | **`#F43F5E`** | **포인트 액션 버튼, 강조 아이콘, AI 감지 표시** |
| Point 600 | `#E11D48` | 포인트 버튼 호버 |
| Point 700 | `#BE123C` | 포인트 버튼 프레스 |
| Point 800 | `#9F1239` | 포인트 뱃지 텍스트 |

### Point 적용 규칙 (엄격)

| 용도 | 색상 | 사용 시점 |
|------|------|----------|
| 핵심 CTA 1순위 (예: 답변 찾기, 결제, 가입 완료) | Point 500 배경 + 흰 텍스트 | 화면당 최대 1-2개 |
| AI 감지 알림 (질문 감지, 마감 임박, 새 알림) | Point 500 아이콘 + Point 800 텍스트 | 정말 주목해야 할 때만 |
| 강조 카드 좌측 보더 | Point 500 (3px) | 질문 카드 등 |
| 뱃지 배경 | Point 100 + Point 800 텍스트 | "NEW", "AI" 등 |

### 절대 금지

- ❌ 일반 액션/링크에 Point 컬러 사용 (그건 Primary 딥틸)
- ❌ 한 화면에 Point 색 3개 이상
- ❌ Status 의미로 Point 사용 (에러는 Error 빨강, 경고는 Warning 앰버)
...
| 상태 | 배경 (뱃지) | 텍스트 (뱃지) | 비고 |
|------|------|------|------|
| 미진행/대기 (not_started) | `#F1F5F9` | `#475569` | — |
| 업무요청 (task_requested, 가상) | `#FFE4E6` | `#BE123C` | 로즈 — AI 감지/요청 |
| 진행대기 (waiting) | `#E0E7FF` | `#3730A3` | 인디고 |
| 진행중 (in_progress) | `#CCFBF1` | `#0F766E` | 티일 |
| 확인요청중 (reviewing) | `#FEF3C7` | `#92400E` | 앰버 |
| 수정필요 (revision_requested) | `#FCE7F3` | `#9F1239` | 로즈/다크 |
| 마무리 대기 (done_feedback) | `#DBEAFE` | `#1E40AF` | 블루 |
| 완료 (completed) | `#E2E8F0` | `#475569` | 그레이 (Irene 승인 2026-04-19) |
| 취소 (canceled) | `#F1F5F9` | `#94A3B8` | 취소선 병행 |

> **버튼 색 매핑 원칙:** 워크플로우 액션 버튼은 "도착 상태 텍스트 색" 을 fill 로 사용 → 클릭 시 어느 상태로 가는지 즉시 인지. 예: `[진행 시작]` = `in_progress` 텍스트색 (#0F766E) / `[승인]` = `done_feedback` (#1E40AF).
...
## 업무 상태 색 — 보류 / 외부컨펌 (#206, 2026-07-26)

| 상태 | bg | fg | 뱃지 보더(배너) | 의미 |
|------|----|----|-----------------|------|
| `on_hold` (보류) | `#FFEDD5` | `#9A3412` | `#FDBA74` | orange — 프로젝트 레벨 `hold`(`#F59E0B`) 와 같은 계열. reviewing amber(`#FEF3C7`) 와 구분 |
| `external_review` (외부컨펌중) | `#E0F2FE` | `#075985` | `#7DD3FC` | sky — "우리 밖에 나가 있음". waiting indigo·in_progress teal 과 구분 |

**색만으로 구분하지 않는다** — 두 상태는 `components/Common/Icons.tsx` 의 `StatusGlyph` 로
아이콘을 병행한다(보류 `‖` / 외부컨펌 `↗`). 색각 이상 사용자에게 amber↔orange 는 구분되지 않는다.

진행률 바는 두 상태에서 `#94A3B8` 로 탈색한다(값은 보존). teal 로 두면 "지금 흐르는 중"으로 읽혀 모순.
