// 좌측 트리의 아이콘 — 폴더(닫힘/열림) · 전체 · 내 파일 · 시스템 폴더 · [+].
//
// ★ 2026-09-20 에 DocsTab.tsx 에서 **빼냈다.** 그 파일은 god-file 래칫에 동결돼 있는데
//   트리를 손볼 때마다 커졌다(2,251 → 2,611줄). 가드가 «동결 초과 성장» 으로 막아 세웠고,
//   맞는 지적이라 구조로 답한다 — 주석을 줄여 숫자를 맞추는 것은 같은 문제를 미루는 것뿐이다.
import React from 'react';
import type { FileSource } from '../../../services/files';

export const FolderSvg: React.FC<{ size?: number }> = ({ size = 14 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" />
  </svg>
);
export const FolderOpenSvg: React.FC<{ size?: number }> = ({ size = 14 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export const AllSvg: React.FC<{ size?: number }> = ({ size = 14 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" />
    <rect x="14" y="14" width="7" height="7" /><rect x="3" y="14" width="7" height="7" />
  </svg>
);
export const MyFilesSvg: React.FC<{ size?: number }> = ({ size = 14 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>
    <circle cx="12" cy="7" r="4"/>
  </svg>
);
export const PlusSvg: React.FC<{ size?: number }> = ({ size = 12 }) => (
...
export const SystemFolderIcon: React.FC<{ src: FileSource; size?: number }> = ({ src, size = 14 }) => {
