...
type InboxTab = 'all' | 'work' | 'mail' | 'sale' | 'chat' | 'signature' | 'billing';
const TYPE_TO_TAB: Record<string, Exclude<InboxTab, 'all'>> = {
  task: 'work', event: 'work', invite: 'work', mention: 'work', task_candidate: 'work',
  // 메일은 별도 탭 — "답장하면 끝" 인 1회성 액션이라 확인 필요에 들어온다
  email: 'mail',
  // 채팅 (2026-09-15, Irene: "Talk 안에 안읽은 메시지도 확인필요에 나와야지").
  //   ★ 옛 주석은 "채팅과 달리" 라며 채팅을 뺐었다 — 뒤집힌 결정이다. 메일이 들어오는데
  //     채팅이 안 들어올 이유가 없다(둘 다 "누가 말을 걸었고 내가 답해야 한다").
  //     노이즈는 **방 단위로 세서** 막는다 — 서버 collectChats 가 방 1개 = 항목 1개로 만든다.
  chat: 'chat',
...
const TAB_LIST: InboxTab[] = ['all', 'work', 'mail', 'sale', 'chat', 'signature', 'billing'];
  ...
  return (
    ...
    >
      ...
      {(() => {
        ...
        const counts: Record<InboxTab, number> = { all: items.length, work: 0, mail: 0, sale: 0, chat: 0, signature: 0, billing: 0 };
        items.forEach((it) => {
            ...
            <TabBar role="tablist">
              {TAB_LIST.map((tab) => (
                <TabBtn key={tab} role="tab" type="button" aria-selected={activeTab === tab}
                  ...
                  <span>{t(`todo.tab.${tab}`, {
                    all: '전체', work: '업무', mail: '메일', sale: '영업', chat: '채팅', signature: '서명', billing: '청구',
                  }[tab])}</span>
