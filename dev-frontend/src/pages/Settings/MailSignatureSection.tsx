...
export default function MailSignatureSection({ businessId, accountId, initialHtml, initialEnabled }: Props) {
  ...
  //   (Irene: "저장된 거 바로 바로 알게 아이콘 우측에 표시되는 거 다 통일해줘").
  //   여기는 ✓ 가 없던 게 아니라 **자기만의 다른 배지**("저장됨" 글자 알약)를 헤더에 그렸다 —
  //   같은 뜻을 두 벌로 만들면 반드시 갈라진다(위치·모양·사라지는 시간이 이미 달랐다).
  //   자체 debounce·타이머·상태도 AutoSaveField 가 하는 일과 겹쳐 있어 통째로 걷어낸다.
  ...
  return (
    <Wrap>
      <Head>
        ...
        <Right>
          <AutoSaveField type="toggle" onSave={() => save({ signature_enabled: enabledRef.current })}>
            <Toggle
              type="button"
              role="switch"
