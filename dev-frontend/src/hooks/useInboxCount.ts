...
export interface InboxCounts { total: number; task: number; bill: number; mail: number; /** 서버에서 실제로 받았는가 — 앱 배지가 0 의 뜻을 값으로 추측하지 않게 한다. */ loaded: boolean }
export function useInboxCount(businessId: number | null | undefined): InboxCounts {
  const [count, setCount] = useState<InboxCounts>({ total: 0, task: 0, bill: 0, mail: 0, loaded: false });
  const localCleanupRef = useRef<(() => void) | null>(null);
  const { user } = useAuth();

  useEffect(() => {
    if (!businessId || !user) { setCount({ total: 0, task: 0, bill: 0, mail: 0, loaded: false }); return; }
    let cancelled = false;
    ...
    const refresh = async () => {
      try {
        ...
        if (!cancelled) setCount({ total: r.total || 0, task: r.taskCount || 0, bill: r.billCount || 0, mail: r.mailReplyCount || 0, loaded: true });
