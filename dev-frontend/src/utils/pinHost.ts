...
export function usePinHost({ tool, title }: UsePinHostOptions): PinHost {
  ...
  const pin = useCallback(async () => {
    ...
    // ★ 고정창의 **자리는 어떤 방법으로도 우리가 못 정한다** — 두 번 실측했다.
    //   ① 권한 없이 `win.moveTo()` → 에러 없이 무시 (2026-08-20 1차)
    //   ② **창 관리 권한 granted + getScreenDetails 호출 후에도 무시** (2026-08-20 2차, 대조군 포함:
    //      같은 조건의 일반 window.open 팝업은 다른 모니터까지 정확히 이동한다 → 차이는 권한이 아니라
    //      **창 종류(Document PiP)** 다. 손으로 끄는 것만 된다.)
    //   그래서 "팝아웃을 옮긴 자리에 고정창을 띄운다"(Irene 요구)는 **구조적으로 불가능**하다.
    //   시도 코드를 남기지 말 것 — 되는 것처럼 보이는 죽은 코드가 다음 사람을 또 헤매게 한다.
    //   할 수 있는 최선은 반대 방향이다: 고정창이 뜬 자리를 학습해 **팝아웃을 그 자리에서 연다**
    //   (rememberOrigin). 창을 옮기지 않으면 핀을 눌러도 그대로다.
