...
//   unread = 안 읽은 메시지. 읽으면 사라지지 우리가 처리하는 게 아니다 → 브랜드 민트(Primary 300).
//   ★ 2026-09-15 2차: 처음엔 중립 슬레이트(#64748B)로 낮췄다가 되돌렸다. 사이드바 배경이
//     딥 틸(#115E59→#134E4A)이라 회색이 **묻혀서 안 보였고**, 무엇보다 이 제품의 색 규칙에서
//     회색은 "없음"이다(2026-09-14 메모 개수: 0=회색 / 1 이상=녹색). 안 읽은 메시지가 **있는데**
//     회색이면 뜻이 거꾸로다. Irene: "토크 알림 색상이 왜 회색이야?"
//     민트는 어두운 사이드바에서 잘 보이고 Coral 과 헷갈리지 않는다 — 둘 다 만족한다.
...
const InboxBadge = styled.span<{ $collapsed?: boolean; $tone?: 'work' | 'unread' }>`
  ...
  background: ${p => (p.$tone === 'unread' ? '#5EEAD4' : '#F43F5E')};
  color: ${p => (p.$tone === 'unread' ? '#134E4A' : '#FFFFFF')};   /* 민트 위 흰 글씨는 안 읽힌다 */
...
const MobileHeaderBadge = styled.span`
  ...
  border-radius: 8px; background: #5EEAD4; color: #134E4A;   /* 안읽음 톤 — 사이드바 Q talk 배지와 같은 뜻·같은 색 */
...
// Q sale — 영업(고객을 단계로 끌어올린다). 고객 아이콘과 구별되게 사람 + 상승선.
const IconSale = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 21V10"/><path d="M9 21V6"/><path d="M15 21v-9"/><path d="M21 21V3"/></svg>);
  ...
  return (
    <LayoutContainer>
      ...
      <MobileHeader $tabMode={tabMode}>
        ...
        {/* ★ 뒤로 가기 버튼은 뺐다 (Irene 2026-09-02: "모바일에서는 헤더에 뒤로가기 화살표
            없어도 돼. 어차피 뒤로가게 다 헤더에 나와." / "화살표 때문에 로고도 위치 바뀌고
            가운데 정렬 안되잖아.").
            실측: 좌측이 햄버거 44 + 화살표 44 = 88, 우측이 44 라 `space-between` 에서
            로고가 22px 오른쪽으로 밀려 있었다. 빼면 좌우가 44:44 로 맞아 정확히 가운데다.
            (2026-08-31 에 "갇힌다" 는 호소로 넣었던 것을 되돌린 것 — 그 사이 패널 헤더의
            돌아가기가 자리를 잡았다는 Irene 판단.) */}
