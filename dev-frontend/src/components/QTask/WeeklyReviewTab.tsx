...
const WsBtn = styled.button<{ $on: boolean }>`padding:9px 16px;background:transparent;border:none;cursor:pointer;font-size:0.875rem;font-weight:${(p) => (p.$on ? 700 : 500)};color:${(p) => (p.$on ? '#0F766E' : '#64748B')};border-bottom:2px solid ${(p) => (p.$on ? '#14B8A6' : 'transparent')};margin-bottom:-1px;&:hover{color:#0F766E;}`;
...
const PBtn = styled.button<{ $on: boolean }>`padding:8px 18px;border:none;background:${(p) => (p.$on ? '#fff' : 'transparent')};color:${(p) => (p.$on ? '#0F766E' : '#64748B')};border-radius:8px;font-size:0.8125rem;font-weight:700;cursor:pointer;box-shadow:${(p) => (p.$on ? '0 1px 2px rgba(0,0,0,.06)' : 'none')};`;
