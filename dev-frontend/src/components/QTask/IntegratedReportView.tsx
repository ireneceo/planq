...
const NavBtn = styled.button`width:30px;height:30px;border:1px solid #E2E8F0;background:#fff;border-radius:8px;font-size:1rem;color:#475569;cursor:pointer;&:hover{background:#F0FDFA;border-color:#99F6E4;color:#0F766E;}`;
const PeriodText = styled.span`font-size:0.9375rem;font-weight:700;color:#0F172A;min-width:104px;text-align:center;`;
const Flex1 = styled.div`flex:1;`;
const DoneChip = styled.span`font-size:0.75rem;font-weight:700;color:#15803D;background:#DCFCE7;border-radius:999px;padding:4px 12px;`;
const ShareMsg = styled.span`font-size:0.75rem;font-weight:700;color:#0F766E;background:#F0FDFA;border-radius:999px;padding:4px 12px;`;
const PendChip = styled.span`font-size:0.75rem;font-weight:700;color:#A16207;background:#FEF9C3;border-radius:999px;padding:4px 12px;`;

const SecRow = styled.div`display:flex;align-items:center;gap:8px;margin-bottom:8px;h2{margin:0;}`;
const GenBtn = styled.button`margin-left:auto;font-size:0.75rem;font-weight:600;color:#0F766E;background:#F0FDFA;border:1px solid #99F6E4;border-radius:8px;padding:5px 10px;cursor:pointer;transition:background .15s,border-color .15s;&:hover:not(:disabled){background:#CCFBF1;border-color:#5EEAD4;}&:disabled{opacity:.6;cursor:default;}`;
const ExecArea = styled.textarea`width:100%;min-height:84px;resize:vertical;border:1px solid #E2E8F0;border-radius:10px;padding:12px 14px;font-size:0.875rem;line-height:1.7;color:#334155;font-family:inherit;&:focus{outline:none;border-color:#14B8A6;box-shadow:0 0 0 3px rgba(20,184,166,.15);}&::placeholder{color:#94A3B8;}`;
const ExecRead = styled.p`font-size:0.875rem;line-height:1.75;color:#334155;white-space:pre-wrap;margin:0;`;
const ExecMuted = styled.p`font-size:0.8125rem;color:#94A3B8;margin:0;`;

const ViewToggle = styled.div`display:inline-flex;background:#F1F5F9;padding:3px;border-radius:8px;gap:2px;align-self:flex-start;@media print{display:none;}`;
const ViewTab = styled.button<{ $on: boolean }>`display:inline-flex;align-items:center;gap:6px;padding:8px 18px;border:none;background:${(p) => (p.$on ? '#fff' : 'transparent')};color:${(p) => (p.$on ? '#0F766E' : '#64748B')};border-radius:8px;font-size:0.8125rem;font-weight:700;cursor:pointer;box-shadow:${(p) => (p.$on ? '0 1px 2px rgba(0,0,0,.06)' : 'none')};`;
const ViewN = styled.span`font-size:0.625rem;font-weight:700;color:#94A3B8;`;
...
const UnitName = styled.h3<{ $link?: boolean }>`font-size:1.0625rem;font-weight:800;color:#0F172A;margin:0;min-width:0;word-break:break-word;${(p) => p.$link && 'cursor:pointer;&:hover{color:#0F766E;}'}`;
const DeptTag = styled.span`font-size:0.6875rem;font-weight:700;color:#475569;background:#E2E8F0;border-radius:999px;padding:2px 9px;`;
const Pct = styled.span`font-size:0.9375rem;font-weight:800;color:#0F172A;`;
const HBadge = styled.span`font-size:0.6875rem;font-weight:700;border-radius:999px;padding:3px 10px;`;
const UStatus = styled.span<{ $on: boolean }>`font-size:0.6875rem;font-weight:700;border-radius:999px;padding:3px 10px;background:${(p) => (p.$on ? '#CCFBF1' : '#FEF9C3')};color:${(p) => (p.$on ? '#0F766E' : '#A16207')};`;
...
const ComLabel = styled.div`font-size:0.6875rem;font-weight:800;color:#0F766E;letter-spacing:.03em;margin-bottom:4px;`;
const ComBody = styled.p`font-size:0.875rem;line-height:1.7;color:#334155;white-space:pre-wrap;margin:0;`;
const EmptyUnit = styled.div`font-size:0.875rem;color:#94A3B8;padding:32px 20px;text-align:center;`;
...
const SetT = styled.div`font-size:0.8125rem;font-weight:700;color:#0F172A;`;
const SetH = styled.div`font-size:0.6875rem;color:#94A3B8;margin-top:2px;`;
...
const ErrBox = styled.div`display:flex;align-items:center;gap:10px;padding:40px;color:#92400E;font-size:0.875rem;justify-content:center;`;
