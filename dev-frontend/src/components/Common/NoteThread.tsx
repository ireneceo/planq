...
import DraftRestoredNote from './DraftRestoredNote';
...
interface Props {
  ...
  busy?: boolean;
  /** 성공하면 true — 쓰던 글(초안)은 true 일 때만 비운다. 실패를 삼키는 핸들러가 글을 지우지 못하게 타입으로 강제 */
  onAdd: (body: string, visibility: 'internal' | 'personal') => Promise<boolean>;
  ...
  /**
   * 운영 #367 — 쓰다 만 메모의 보존. 키는 hooks/useDraftText.useDraftKey 한 곳이 만든다(사용자·워크스페이스·대상).
   * draftKind 를 넘기지 않으면 보존하지 않는다(기존 동작). draftLegacyKey 는 옛 키 한 번 이관용.
   */
  draftKind?: DraftKind;
  draftEntityId?: string | number | null;
  draftBizId?: number | null;
  draftLegacyKey?: string | null;
}
...
export default function NoteThread({
  ...
  draftKind, draftEntityId, draftBizId, draftLegacyKey,
}: Props) {
  const { t } = useTranslation('common');
  // 운영 #367 — 대상이 바뀌어도(다른 대화·다른 스레드) 쓰던 글은 그 대상에 남아 있다가 돌아오면 되살아난다.
  // ★ 훅은 조건 없이 **항상** 부른다. 보존 여부로 훅 호출을 가르면 그 prop 이 바뀌는 순간
  //   훅 순서가 어긋나 상세 전체가 죽는다(React #310 — 정적 검사로는 안 잡힌다).
  const draftStorage = useDraftKey(draftKind ?? 'qtalk-note', draftKind ? draftEntityId : null, draftBizId);
  const draft = useDraftText(draftKind ? draftStorage : null, { legacyKey: draftLegacyKey });
  const [localText, setLocalText] = useState('');
  const draftKey = !!draftKind;
  ...
  const submit = async () => {
    ...
    if (!body || busy) return;
    // ★ 성공했을 때만 비운다 — 저장이 실패했는데 입력칸이 비면 사용자는 쓰던 글을 잃는다(초안 계약 ②).
    const ok = await onAdd(body, shared ? 'internal' : 'personal');
    if (ok !== true) return;
  ...
  return (
    <Wrap>
      ...
      {draftKey && <DraftRestoredNote draft={draft} />}
    </Wrap>
  );
}
