...
const Handle = styled.button<{ $open: boolean; $pulse: boolean; $side: 'left' | 'right'; $openDist: string; $hideBelow?: number }>`
  ...
  /* ★ 2026-08-24 (Irene: "제발 모든 팝업과 우측패널이 세로 화살표보다 위로 올라오게 해")
     ...
     ★ 2026-09-03 — 값을 index.css 의 --pq-z-panel-handle 로 옮겼다. 이 핸들과 위아래를 다투는
     것들(에디터 버블 95 · 앱 크롬 100 · 오버레이 130)이 각자 숫자를 들고 있으면 서로를 모른다.
     실제로 표 편집 바가 이 핸들에 가려 있었다(Irene: "메뉴닫는 세로 화살표 아래로 보여").
     층을 바꾸려면 이 파일이 아니라 index.css 의 층 표를 고친다. */
  z-index: var(--pq-z-panel-handle, 90);
