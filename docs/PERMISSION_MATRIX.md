...
> - §2 — Cue AI 팀원 role `ai` 추가. `admin` 워크스페이스 역할은 미들웨어 분기만 존재 — ENUM·역할변경 라우트 미지원으로 현재 부여 불가(스캐폴드) 명시
...
> **`admin` (워크스페이스 관리자) 역할의 현재 상태 (2026-07-10 실측):**
> 미들웨어·스코프 코드에 admin 분기가 존재하지만 (`middleware/menu_permission.js:85-86` owner_only 외 전권, `middleware/access_scope.js:31,54` `scope.isAdmin`, `routes/tasks.js:1121-1122` FIELD_RULES owner 급), **`business_members.role` ENUM 은 `('owner','member','ai')`** (`models/BusinessMember.js:46`, 운영 DB 실측 동일) 이고 역할 변경 라우트도 `['owner','member']` 만 허용 (`routes/businesses.js:1139`) — **admin 은 현재 부여할 수 없는 전방 호환 스캐폴드**다. 아래 매트릭스의 admin 열은 이 스캐폴드 코드 기준. (별개로 `attachWorkspaceScope` 는 platform_admin 의 `businessRole` 문자열을 `'admin'` 으로 세팅 — `access_scope.js:40` — 워크스페이스 admin role 과 다른 개념이니 혼동 주의)
...
- **reviewers add/remove** — 담당자/요청자(작성자 포함)/owner (`task_workflow.js:573,650`). 워크플로우의 `isOwner` 헬퍼는 `role === 'owner'` 만 인정 — admin 스캐폴드·platform_admin 미포함 (`task_workflow.js:65-68`).
