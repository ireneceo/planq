...
## §4 — 판단이 필요해 Opus 가 손대지 않은 것 (Irene 지시 2026-08-24 "판단이 필요한 건 fable에게")

### 4-A. 운영 nginx `client_max_body_size` 상향 — **값 결정 필요**
- 현재 50M. 업로드는 브라우저 → nginx → Node → Drive 로 흐르므로 Drive 경유 파일도 nginx 를 통과한다.
  즉 지금은 Drive 연동이 멀쩡해도 영상(수백MB~GB)은 못 올린다.
- 코드는 외부(Drive) 경유 시 **5GB** 까지 허용하도록 이미 설계됨(`services/plan.js upload_file`).
  플랜 파일당 한도는 100MB(워프로랩 실측).
- **판단 필요:** 상한값(100M / 2G / 5G). Opus 권고는 2G.
- **리스크(측정 필요):** ①nginx·multer 가 본문을 **디스크 버퍼링** → 큰 업로드 1건이 일시적으로 파일 크기의
  2배 디스크 사용. 운영 디스크 여유 확인이 선행 ②`proxy_read_timeout`/`client_body_timeout` 을 같이
  올리지 않으면 상한만 커지고 전송 중 끊긴다 ③Drive 경유는 플랜 스토리지 쿼터를 우회 — 남용 경로
- nginx 는 배포 스크립트가 건드리지 않는다(별도 작업). `nginx -t` 후 reload 라 실패 시 기존 설정 유지.

### 4-B. 배포 중 사용자에게 "새 버전" 알리기 — **UX 설계 필요**
- 2026-08-24 사고: 배포가 옛 청크를 지워(`rsync --delete`) 옛 번들을 든 브라우저의 모든 요청이
  `Failed to fetch` 로 죽었다(메일 전송·AI 초안·목록 갱신 전부). 사용자는 원인을 알 방법이 없었다.
- 즉시 조치는 완료: 프론트 배포에서 `--delete` 제거(옛 청크 보존).
- **남은 판단:** `BuildVersionGuard.isReloadSafe()` 가 입력 중이면 reload 를 **조용히** 미룬다.
  작업 중일수록 옛 번들에 오래 머문다. "새 버전이 있습니다 — 저장 후 새로고침" 배너가 필요한데,
  옛 배너 시스템은 사이클 N+3 에 의도적으로 제거됐다(잔존 파일도 2026-08-14 삭제).
  되살릴지/어떤 형태일지가 설계 판단.
- 부수: 옛 청크를 안 지우므로 **주기 정리 작업**(예: 30일 경과 assets 삭제)이 필요하다 — 아직 없음.

### 4-C. 쓰기 실패 침묵 — 남은 지점 전수 (P0-1 후속)
- Q Task 의 `saveField`/`changeStatus` 는 배너+재조회로 고쳤다(#228 의 직접 원인).
- **남은 판단:** 같은 관용구(`if(!r.ok) return;` / `catch {}`)가 다른 페이지에 얼마나 있는지 전수하고,
  `apiFetch` 를 쓰기 전용 래퍼로 감쌀지(구조 변경) 아니면 호출부마다 고칠지.
  가드 래칫으로 신규 발생을 막을지도 함께.

### 4-D. 세션/401 — 사실관계는 정리됨, 판단만 남음
- 오늘 운영 401 7건은 **전부 외부 스캐너**(`ua=RootEvidence/1.0`)였고 실사용자 갱신 16건은
  모두 `grace_reissue` 로 정상 회복. **세션 갱신 실패는 없었다.**
- 즉 #228 저장 실패의 원인은 아직 미상. P0-1 배너가 다음 발생 시 사유를 노출한다.
- **판단 필요:** 원인 미상인 채로 둘지, 쓰기 실패 로깅(클라 → 서버)을 붙일지.

### 4-E. 메일 첨부 — 남은 것
- 완료: 인라인 이미지 즉시 표시 + `?w=1024` 리사이즈(실측 2369KB → 68KB, 97% 감소, 디스크 캐시 8ms).
- **판단 필요:** 첨부 **칩/라이트박스**(원본 미리보기)도 리사이즈본을 쓸지. 라이트박스는 확대가 목적이라
  원본이 맞을 수도 있다. 그리고 997건 중 **571건이 `file_id` 없음** — 저장 경로가 반쪽인지 확인 필요.

---

## §5 — 메일 전달(Forward) **기능 차단 중** — Fable 이 설계·검증 후 해제 (Irene 지시 2026-08-24)

> "전달기능은 너가 제대로 못해? 그럼 fable이 하게 지금은 작동이라도 막아놔.
>  엉망인 기능들 고객이 보면 안되지. 우리 이제 오픈 좀 하자"

### 차단 위치 (해제 시 둘 다 풀어야 한다)
- 프론트: `pages/QMail/ThreadMessages.tsx` 의 `const FORWARD_ENABLED = false` → 전달 버튼 미표시
- 서버: `routes/email_threads.js` forward 라우트 — `QMAIL_FORWARD_ENABLED !== '1'` 이면 **503**
  (버튼만 숨기면 옛 번들·직접 호출이 남으므로 서버에서도 거절)

### 무엇이 문제인가 (확인된 사실)
1. **원문이 리치 에디터를 통과한다** — 옛 `startForward` 가 원문 `body_html` 을 TipTap 컴포저에 넣었다.
   에디터는 자기 스키마에 없는 것을 이해하지 못해 메일의 `<table>`·인라인 스타일을 자기 노드로
   재해석한다 → 표가 라운드 박스로 변형, `align/width/colspan` 유실로 레이아웃 붕괴,
   그 과정에서 `cid:` 참조까지 사라져 이미지가 빠진다. (운영: "이상한 라운드 박스가 죄다 생기네")
2. Opus 가 서버 `include_original` 경로를 넣었다 — 컴포저엔 덧붙일 말만, 원문은 서버가
   `srcMsg.body_html` 을 가공 없이 이어붙이고 인용 헤더도 서버가 만든다.
   **그러나 실사용 검증이 안 됐다** — 신고 시점까지 운영 발송 **0건**(요청이 서버에 도달하지 않았음.
   사용자가 옛 번들에 머물러 있었다). 즉 이 경로는 **한 번도 실행된 적이 없다.**

### Fable 이 판단·검증할 것
- 원문 보존 방식: 서버 이어붙이기(현재 구현)가 맞는지, 아니면 **원본 MIME 을 그대로 첨부(message/rfc822)**
  하는 정공법이 맞는지. 후자가 메일 클라이언트 표준에 가깝다.
- 인용 헤더 형식(로케일/시간대), 원본 첨부 중복 여부(서버가 `srcMsg` 첨부를 자동으로 붙인다).
