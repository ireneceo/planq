...
> ## ✅ 종료 — 전 항목 처리 완료 (2026-08-22 07:50 UTC, 운영 검증)
>
> dev 가 #370~#374 를 전부 수정·배포했고 운영에서 확인했다. **이 문서는 기록용이다. 추가 작업 없음.**
>
> | # | 배포된 수정 | 운영 검증 |
> |---|---|---|
> | #373 | `middleware/ogMeta.js:334` 봇 분기에 `/insights/:slug` 케이스 | 크롤러 → "워크스페이스 만들기 · PlanQ" ✅ |
> | #374 | `routes/og.js:28` `INDEX_CANDIDATES` 후보 배열 | `/insights/:slug` 200 ✅ |
> | #370 | `emailImapCron.js:212` 로고 저장 게이트 + `:268` content_hash 중복제거 | 로고 재유입 **0건** ✅ |
> | #371 | `emailImapCron.js:455` `auto-submitted` 처리 | — |
> | #372 | 카운터 재계산 백필 | biz1 444건/163MB — 실제와 일치 ✅ |
>
> **#362 도 done 처리 + 사용자 답글 완료.**
>
> 남은 것: 격리 파일 524개(21MB) `/opt/planq/backups/logo-cleanup-20260822_024124/` 며칠 관찰 후 수동 삭제 (운영 측 작업).
> `/public/docs/:token` 크롤러 미리보기는 운영에 공유된 documents 가 0건이라 미검증.
