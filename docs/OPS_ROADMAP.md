...
## 운영 프론트 청크 정리 (2026-09-06 박제)

배포마다 해시가 바뀐 청크가 쌓인다. 실측: **36,761개 / 636MB** (2주치 · 현재 빌드는 480개).

```bash
scripts/prune-prod-assets.sh --days 7            # dry-run (기본)
scripts/prune-prod-assets.sh --days 7 --apply    # 격리(mv) — 삭제 아님
scripts/prune-prod-assets.sh --purge-quarantine /opt/planq/backups/stale-assets-<STAMP>
```

**★ mtime 으로 지우면 안 된다.** 내용이 안 바뀐 청크는 **해시가 같아 파일명이 그대로**다 —
지금도 살아 있는데 오래돼 보인다. 나이만 보고 지우면 살아 있는 청크를 지운다.
그래서 "살아있는 목록" 을 로컬 빌드 산출물에서 계산하고, 그 밖의 것만 후보로 둔다.
로컬 빌드 파일이 운영에 하나라도 없으면(= 배포와 어긋남) 스크립트가 **중단**한다.

**★ 삭제가 아니라 격리(mv)다.** 같은 파일시스템이라 즉시·무비용이고 되돌릴 수 있다.
보존 기간(기본 7일)은 오래 열어둔 탭·옛 서비스워커가 옛 청크를 참조할 여지를 남기는 값이다.

2026-09-06 1회차: 16,160개 / 244MB 격리 → 636MB → **358MB**.
검증 — 살아있는 271개(js·css) 전부 200 · 실브라우저 랜딩·로그인 정상 · 자산 4xx 0 · JS 예외 0.
