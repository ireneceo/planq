...
## ★ 표준 배포 절차 (2026-09-18 확정 — 앞으로 모든 Android 업데이트는 이 순서)

> Irene 2026-09-18: *"앞으로 Android 앱을 수정해서 Google Play에 업데이트할 때 사용할 표준 배포 절차를 정리해."*
>
> ★ **먼저 가르는 질문 — 네이티브 껍데기를 건드렸는가?**
> 앱은 **원격 껍데기**(`server.url = https://planq.kr`)라 화면·기능 코드는 **웹 배포로 이미 도달**한다.
> 새 AAB 가 필요한 것은 **번들 안에 박히는 것**을 바꿨을 때뿐이다:
> `android/` 전체 · `www-placeholder/`(오프라인 폴백 화면) · `capacitor.config.ts` ·
> `google-services.json` · 권한 · 딥링크 필터 · 아이콘/스플래시.
> 그 밖의 변경에 AAB 를 새로 올리는 것은 심사 대기만 늘린다.

| # | 단계 | 확인 |
|---|---|---|
| 1 | 코드 수정 | |
| 2 | **QA** — dev.planq.kr 에서 기능 검증 | `node scripts/health-check.js` · `guard-invariants.js` · 관련 카나리 |
| 3 | **웹 배포**(`/배포`) | 화면 코드는 여기서 사용자에게 도달한다 |
| 4 | **여기서 갈린다** — 네이티브 껍데기를 안 건드렸으면 **끝**. 건드렸으면 5번부터 | |
| 5 | 버전 | **손대지 않는다.** CI 가 `versionCode`=Codemagic 빌드번호, `versionName`=`dev-frontend/package.json` 으로 넣는다 |
| 6 | **`git push` → GitHub** | ★ **CI 는 GitHub 에서 당겨간다.** 푸시를 빠뜨리면 **옛 코드로 빌드**된다 |
| 7 | **Codemagic `PlanQ Android — Play`** 실행 | `cap:beta:android`(운영 고정·백업·FCM·targetSdk 사전검사) → `bundleRelease` → **`android-packaged-backup-check.js`**(**Play 에 올라가는 AAB 자체**를 aapt2 로 읽어 백업 잠금 단언 — 소스도 중간 산출물도 아니다. 그 둘을 읽는 검사는 다섯 번 뚫렸고, 마지막은 **패키징 뒤 AAB 안 매니페스트를 바꿔치기**하는 것이었다)에 백업 잠금 단언 — 소스가 아니라 빌드가 만든 것을 본다. `bundleRelease` 는 aapt2 link 를 **두 번** 하고 AAB 에 실리는 건 뒤엣것이라, 앞엣것만 보면 번들 쪽만 바꿔치기해도 초록이다) → `jarsigner` 서명 검증 |
| 8 | AAB 내려받기 + **그 빌드의 commit hash 기록** | 어느 커밋의 AAB 인지 남기지 않으면 네이티브 회귀를 추적할 수 없다 |
| 9 | Play Console → **Test and release → Production → 새 버전 만들기** → AAB 업로드 | |
| 10 | **릴리즈 노트**(ko/en) | 사용자에게 보이는 변경만 |
| 11 | **검토 → 출시 시작(Start rollout)** | ★ **초안 저장만 하면 아무 일도 일어나지 않는다** — 2026-09-15 에 실제로 릴리스 28 이 **Draft 로 3일간 방치**돼 심사 통과 상태인데 스토어에 앱이 없었다 |
| 12 | 심사·게시 | 스토어 URL 이 **200** 이 되는지로 판정한다 |
| 13 | **운영 다운로드 링크** | 관리자 > 플랫폼 설정 > 앱 다운로드(Android) 에 스토어 URL. **재배포 불필요** · dev 에 넣으면 운영에 안 간다 |
| 14 | **Play 설치본 smoke test** | 아래 목록 |

### 14번 — Play 설치본에서만 확인되는 것 (코드 점검으로 대체 불가)
딥링크 자동검증(Play App Signing 재서명 키) · 알림 클릭 후 화면 이동 · 포그라운드/백그라운드 푸시 표시 ·
알림 권한 거부 후 동작 · Google 로그인 왕복 · 파일 업로드/첨부 · 다운로드(공유 시트) ·
네트워크 끊김→복구 · 키보드 표시 · 앱 종료 후 재실행 · 업데이트 후 로그인 유지 · Back 버튼

### 기록 — 새 표를 만들지 않는다
Android 출시도 **기존 `/admin/dev-status`(개발 현황)** 에 남긴다. `commit_to` = AAB 를 만든 커밋,
`version` = `1.52.11 (code 29)` 형태. 서술 11섹션에 완료·진행·이슈·변경현상·확인영역·다음작업이 들어간다.
별도 «앱 배포» 화면을 만들지 않는다 — 같은 일의 기록이 두 곳이 되면 반드시 갈라진다.
