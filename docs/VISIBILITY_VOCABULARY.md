...
## 6. 사이클 N+9 작업 매트릭스 (구현 — 2026-05-11)

| # | 영역 | 작업 | 상태 |
|:-:|---|---|:-:|
| 1 | 권한 정책 | `access_scope.js` 옵션 A 헬퍼 (canAccessFileByLevel / fileListWhereByLevel 등 6종) | ✅ 청크 1 |
| 2 | Visibility model | L1~L4 vocabulary 통일 + 컬럼 (`files.visibility`, `posts.vlevel`, `kb_documents.scope` 'private' 추가) | ✅ 청크 1 |
| 3 | 마이그레이션 스크립트 | `scripts/migrate-visibility-l-levels.js` — project_id NULL → L3 백필 (운영 files 5, posts 3) | ✅ 청크 1 |
| 4 | Visibility 배지 | `VisibilityBadge` 컴포넌트 + `VisibilityChangeModal` + `PUT /:id/visibility` API | ⚠️ 청크 4 (컴포넌트만, 카드/행 적용은 청크 5) |
| 5 | 개인 보관함 메뉴 | `/personal-vault` 라우트 + `PersonalVaultPage` 4 탭 + `/api/personal-vault/*` 4 라우트 | ✅ 청크 2 (4 탭 — 노트·메모 input_type 분리는 v1.1) |
| 6 | 사이드바 섹션 | "기능" → "협업" + 신규 "개인" 섹션 + 개인 보관함 NavItem | ✅ 청크 2 |
| 7 | Invoice owner_user_id | DB 컬럼 + 백필 (운영 14건은 N+8 fix 에서 이미 처리) + 생성 시 default | ✅ 청크 1 |
| 8 | Signature 알림 통일 | requester_user_id 기준 | 📋 잔여 (다음 사이클) |
| 9 | 5중 시그널 | 사이드바·헤더·탭·popup·tour | 📋 청크 5 (잔여) |
| 10 | 첫 사용 explainer | 개인 보관함 1회 안내 (localStorage dismiss) | ✅ 청크 2 |

### 잔여 청크 5 (다음 사이클)

- VisibilityBadge 카드/행 적용 (Q file `DocsTab`, Q docs `PostsPage`, Q info 카드)
- VisibilityChangeModal 진입점 (카드의 배지 클릭 → 모달)
- 헤더 sub-line (`PageShell` subtitle 또는 별도) — Q note, 개인 보관함 외 적용
- Q note 페이지 자물쇠 + dismiss 안내 박스
- 메모 popup 우상단 자물쇠
- FirstVisitTour (Q note 첫 진입)
