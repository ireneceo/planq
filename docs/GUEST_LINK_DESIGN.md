...
> **알려진 한계 — LLM 은 게스트를 구별하지 못한다.** `services/cue_context.js` 와
> `task_extractor.js` 는 sender 이름을 그림자 User 에서 읽으므로 **모든 게스트가 "게스트"** 로
> 보인다. 표시명은 메시지에 박제돼 있어 화면은 정확하지만 LLM 입력은 아니다.
> v1 은 게스트 방에서 Cue 가 발화하지 않아 무해하다. 다만 **자동추출이 켜진 고객 방에서는
> 두 게스트가 한 사람으로 요약된다.** 게스트 방에 Cue 를 열거나(§10) 자동추출을 기본으로
> 켜기 전에 이 두 곳이 `meta.guest.name` 을 읽게 고쳐야 한다.
...
## 13. 답글 알림 — 게스트가 이름·이메일을 남긴다 (#259 A안, 2026-09-03)

> Irene: *"A로 하면 고객에게 이름이랑 이메일 등록하게 해야 하네. 그렇게 게스트가 되면
> 나중에 Q sale 메뉴에도 등록되면 되겠네."*
>
> **Fable 설계 게이트 판정: 조건부 GO (2026-09-03).** 아래는 그 조건 10건을 반영한 뒤의 설계다.
> 판정이 바꾼 것은 §13.2(필드 원천·전파 방향) · §13.6(한도·소유 증명) · §13.7(착지점) ·
> §13.9(보관기간 cron) — **처음 설계대로 갔으면 알림이 절반의 경로에서 안 갔고,
> 익명 아무나가 등록자에게 메일을 강제로 쏠 수 있었다.**

### 13.1 무엇을 고치는가

지금 게스트는 **답이 온 것을 알 방법이 없다.** 카톡 링크를 눌러 글을 남기고 나가면,
멤버가 답해도 게스트는 그 링크를 다시 눌러 봐야 안다.

**A안** — 게스트가 원하면 이름·이메일을 남기고, 답글이 오면 메일로 알린다. 등록은 **선택**이다.

### 13.2 사람 = 자기 링크 (자식 행)

메일에 **열리는 링크**를 실으려면 그 사람 몫의 토큰이 있어야 한다. 그래서 자식 행을 둔다.

| kind | 뜻 |
|---|---|
| `shared` | 카톡방에 퍼지는 링크. 익명·여럿. **지금 그대로** |
| `personal` | 이메일을 확인한 사람마다 1개. `parent_link_id` 로 shared 아래 자식 |

**필드 원천 (전부 명시 — 이것이 비면 다음 사람이 각자 다르게 채운다):**

| 필드 | 값 |
|---|---|
| `business_id`·`conversation_id`·`project_id`·`client_id` | 부모 복사 |
| `created_by` | 부모 복사 |
| `guest_user_id` | **자기 그림자 User 신규** + `conversation_participants` 행. 부모의 그림자는 그 링크로 들어온 모두가 함께 쓰는 익명 신원이라, 물려받으면 §8 승격이 무리 전체를 올리고 `is_mine` 이 남의 글을 자기 글로 보여 준다 |
| `can_write` | 부모 값을 저장하되 **판정은 resolve 시** `부모 AND 자식`. 복사값만 믿으면 나중에 부모를 열람 전용으로 바꿔도 자식은 계속 쓴다 |
| `token_hash` | 확인 **전**에는 아무도 갖지 않는 난수의 해시(컬럼이 NOT NULL·UNIQUE). 실제 토큰은 확인 후 |

**전파 방향 — 한 방향씩 다르다:**

- **부모 → 자식**: 회수(`revoked_at`)·킬스위치·`can_write`. 부모 회수 시 자식 행에도 `revoked_at` 을 찍는다(파생으로만 두면 *언제* 닫혔는지 아무도 몰라 보관기간을 셀 수 없다).
- **자식 → 부모**: 사용(`last_used_at`·`expires_at` 슬라이딩). ★ 반대로 하면 안 된다 — 부모는 카톡방에 뿌린 뒤 아무도 안 누를 수 있고, 부모가 90일 유휴로 죽으면 **활발히 쓰던 개인 링크까지 죽는다.**

### 13.3 확인 전에는 아무것도 안 나간다 (OTP)

무인증 표면이라 **남의 주소**를 적을 수 있다. 확인 전에 나가는 메일은 **코드 한 통뿐**이고
그 안에 워크스페이스 이름·대화 제목·본문은 없다.

- 코드 6자리 `crypto.randomInt` (★ `Math.random()` 금지 — 전자서명 OTP 에서 한 번 났다)
- `otp_hash` sha256, TTL 10분, 시도 5회 → 30분 잠금, 비교는 `timingSafeEqual`
- 있는 주소·없는 주소·회수된 주소 **응답이 같다** — 다르면 그것이 곧 열거 수단
- **확인 안 된 채 24시간이면 행째로 지운다** — 남의 주소를 적어 넣은 경우가 거기 남는다. 근거 없는 개인정보를 남겨 두지 않는다

### 13.4 토큰은 저장하지 않고 **다시 만든다**

알림 메일에는 그 사람의 링크가 들어가야 하는데 우리는 해시만 갖는다. 길은 셋이었다.

| 안 | 문제 |
|---|---|
| 메일마다 회전 | 지난 메일의 링크가 전부 죽는다 — 사용자에게는 "만료" 로 보인다 |
| 원문 저장 | **DB 사본 하나로 모든 대화가 열린다.** 이 시스템이 해시만 두는 이유가 그것 |
| **(택함) 파생** | `HMAC(GUEST_LINK_SECRET, id:created_at:contact_email)` 로 매번 계산 |

파생의 성질: **비밀키만 새도 못 만들고(행의 값을 모른다), DB 만 새도 못 만든다(키가 없다).**
둘 다 있어야 만들어진다 — 원문을 저장하는 것보다 엄격히 낫다. 키가 없으면 기능이 죽는다(fail-closed).
운영에도 `GUEST_LINK_SECRET` 을 **dev 와 다른 값**으로 넣어야 한다.

### 13.5 알림 메일에는 **본문을 싣지 않는다**

주소는 확인됐지만 메일함은 남이 볼 수 있고 전달도 된다. `"{워크스페이스}에서 새 답글"` +
**열기 버튼**뿐 — 보낸 사람 이름도 본문도 없다. ko/en 두 벌(게스트는 로그인 사용자가 아니라
언어 설정이 없다 — 신청한 화면의 언어를 `locale` 에 적어 둔다).

**보내지 않는 조건** (하나라도 걸리면 그 사람에게는 안 간다):

- 고객에게 안 보이는 메시지 — 삭제·내부 메모·미승인 초안. 술어는 **`utils/messageVisibility.js` 정본 하나**
- **보낸 사람이 게스트** — ★ 아니면 링크를 아는 익명 아무나가 글을 써서 등록자 전원에게 메일을 강제로 쏜다
- 미확인 / 수신거부 / 회수 / 만료 / 부모가 닫힘
- 15분 안에 이미 보냄 · **3분 안에 그 대화를 보고 있었음**(지금 보는 사람에게 메일을 쏘지 않는 가장 싼 조건)

### 13.6 라우트 (전부 무인증, `routes/guest.js`)

| 라우트 | 하는 일 | 토큰 |
|---|---|---|
| `POST /:token/notify/request` | `{name,email,consent,locale}` → 확인 코드 발송 | **shared 만**(personal 은 404) |
| `POST /:token/notify/verify` | `{email,code}` → 확인 + 첫 확인이면 원문 토큰 1회 응답 | shared |
| `GET /:token/notify/me` | 등록 상태 | **personal 만** |
| `POST /:token/notify/unsubscribe` | 알림만 끔(`{on:true}` 면 재개). 링크 회수가 아니다 | **personal 만** |
| `DELETE /:token/notify` | 이름·이메일 삭제 + 그 링크 회수 | **personal 만** |

**자기 것임을 증명하는 수단은 토큰 하나다.** 쿠키에 브라우저 라벨을 심는 안은 버렸다 —
지워야 할 개인정보가 하나 늘고, 라벨만으로 남의 등록을 지울 수 있는 문이 생긴다.
`me`·`unsubscribe`·`DELETE` 를 shared 로 받으면 그 링크를 아는 아무나가 남의 등록을 묻고 지운다.

**한도 — 한 축만 걸면 반드시 뚫리거나 정상 사용을 막는다** (링크당 3회/시간 하나였으면
카톡방 다섯 명이 같은 저녁에 등록할 때 넷째부터 막히고, 동시에 링크를 여러 개 가진 사람은
임의의 주소로 하루 72통을 때릴 수 있었다):

| 축 | 값 | 막는 것 |
|---|---|---|
| 링크 | 20 / 시간 | 이 링크로 아무 주소나 계속 두드리기 |
| 링크+이메일 | 3 / 시간 | 같은 주소 반복 |
| IP | 10 / 시간 | 링크를 여러 개 들고 돌아가며 때리기 |
| 워크스페이스 | 200 / 일 | 발신 평판은 워크스페이스 단위로 다친다 |
| **이메일(플랫폼 전체)** | 5 / 일 | 링크를 바꿔 가며 한 사람 괴롭히기 |

수신거부 주소는 **화면으로 데려가는 링크**다 — 메일 스캐너가 링크를 미리 열기 때문에
GET 한 번으로 꺼지면 안 된다. 끄는 것은 그 화면의 버튼(POST)이 한다.

### 13.7 발송 착지점 — **Message 훅 하나**

처음 설계는 "메시지를 만드는 라우트 두 곳에서 부른다" 였다. 실제로 게스트에게 보이는 메시지를
만드는 곳은 **여덟 곳이 넘는다** — 대화 라우트 둘, Cue 자동응답, 청구서 발송, 문서·공유·서명 카드.
게다가 **초안 승인은 생성이 아니라 update** 로 보이게 된다. 부르는 곳을 세는 방식은 새 경로가
생길 때마다 조용히 빠진다(이미 겪었다).

→ `models/Message.js` 의 `afterCreate` + `afterUpdate(ai_draft_approved 전이)` 한 곳.
훅은 트랜잭션 안이므로 실제 일은 **커밋 뒤**(`transaction.afterCommit`) + **응답 뒤**(`setImmediate`).
매 메시지마다 묻는 질문이라 "이 대화에 알림 받을 사람이 있나" 는 60초 캐시로 답한다
(대부분의 대화에는 게스트가 없다 — 그 "없다" 를 저장 경로에 쿼리로 얹지 않는다).

### 13.8 멤버 쪽

링크 관리 목록은 **`kind='shared'` 만**(자식까지 섞이면 등록자 수만큼 "링크" 가 늘고
각각 회수 버튼이 달려, 등록자가 50명이면 정작 부모가 목록에서 밀려난다).
자식은 그 링크에 딸린 **"등록한 사람"** 으로 붙는다 — 이름·주소·확인 여부·마지막 사용/알림·
**개별 회수**. ★ 이 이름을 메시지 옆에 붙이지 않는다: 링크는 메일로 전달될 수 있고,
전달받은 제3자의 글이 **확인된 사람의 글로 보인다**(#259 에서 이미 난 사고와 같은 모양).

### 13.9 보관기간을 지키는 코드

방침에 쓴 문장은 코드가 있어야 참이 된다 (`services/guestContactCleanup.js`, 자정 cron):

- 확인 안 된 행 — **24시간**
- 회수·만료 후 **30일** — `contact_*`·`otp_*`·`consent_*`·`last_used_ip` NULL
- 같은 시점에 **EmailLog 의 수신 주소도 마스킹**(`deleted@도메인`). 본체를 지우고 로그에 남기면 안 지운 것이다

`consent_privacy_version` 은 `platform_settings.privacy_version` 을 박제한다.
**방침 본문에 게스트 조항이 새로 들어갔으므로 버전을 올릴지는 Irene 결정** — 올리면
기존 회원에게 재동의 모달이 뜬다(`routes/auth.js`).

### 13.10 이번 범위 아님

Q sale 원장 연결(`contact_*` 를 문의로 승격) · SMS · 브라우저 push · 멤버가 먼저 초대 메일 보내기.

### 13.11 검증 (구현 게이트에서 실측)

★ dev 는 메일 발송이 꺼져 있다(`EMAIL_SENDING_ENABLED=false`) — 판정은 **EmailLog 행**으로 한다
(`status='skipped'` + `to_email` + `template`). "0통" 은 행 0건, "1통" 은 행 1건.

| 축 | 실측 |
|---|---|
| 미확인 발송 | 확인 전 상태에서 답글 여러 건 → `guest_reply_notify` 행 **0** |
| 익명 강제 발송 | shared 익명이 글 10건 → 등록자에게 **0통** (음성 대조군) |
| 정상 | 멤버가 답글 → **1통**, 15분 내 추가 답글 → **여전히 1통** |
| 경로 누락 | 카드·초안 승인·Cue auto 로 보이게 된 메시지 → 각 **1통** |
| 술어 | 삭제·내부 메모·미승인 초안 심고 → **0통**. 승인된 초안(raw 1/0) → **1통** |
| 부모 회수 | shared 회수 → personal 토큰 **404** (양성 대조군: 회수 전 200) + 자식 `revoked_at` 기록 |
| 부모 유휴 | 부모 90일 유휴 + 자식 사용 → **둘 다 생존** |
| 열거 | 있는/없는 주소 응답 동일, 5회 오답 후 잠금, personal 로 request → 404 |
| 한도 | 다섯 축 각각 초과 시 429. **다섯 명 동시 등록은 429 없음** |
| 소유 | shared 토큰으로 `me`·`unsubscribe`·`DELETE` → 전부 차단 |
| 테넌트 | 다른 워크스페이스 대화의 personal 토큰 교차 조회 404 |
| 삭제 | `DELETE` 후 `contact_*` NULL, 과거 메시지 표시명 **보존**, EmailLog 마스킹 |
| 파생키 | 키 없으면 알림 0통(fail-closed), 키 있으면 같은 토큰 재계산 |
