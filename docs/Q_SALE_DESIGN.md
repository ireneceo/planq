...
- 액션 3종(행마다): **원본 열기**(`/talk?conv=` · `/mail?thread=`) · **고객으로 등록**(기존 `POST /api/sale/:biz/save-as-client` 재사용) · **업무 추가**(`POST /api/tasks`)
  - ★ **행의 단위는 대화방이다** (2026-09-12 2차). 게스트 링크는 별개의 접점이 아니라 그 대화의 **신원**이다 —
    따로 세면 같은 대화가 두 줄로 보이고 합계가 그만큼 부푼다(실측 3건). 링크가 붙은 대화는 `source:'guest_link'`,
    링크가 없으면 `source:'chat'` 으로 **한 줄만** 만든다. 대화방이 목록에 없는 링크(보관됨 등)만 따로 붙인다.
  - ★ **등록 가능 여부는 `source` 가 아니라 `ref.kind`** 다. Irene: *"채팅할 때 고객이 이메일 넣으면?"* —
    게스트는 채팅 화면의 "답글 알림 신청"에서 이름·이메일을 넣고 OTP 로 확인한다(`routes/guest_subscribe.js`).
    그 값은 `guest_links.contact_email` + `email_verified_at` 에 남고, `sale_save.js` 의 guest_link 분기가 **이미 그것을 쓴다**.
    따라서 채팅이어도 링크가 붙어 있으면 등록된다. 버튼을 숨기는 것은 **링크가 없는 순수 대화방**(`ref.kind==='conversation'`)뿐이다.
  - ★ 한 대화에 링크가 여럿일 때 등록에 쓸 하나는 `betterLink()` — **확인된 이메일 → 이메일 있음 → 최근 사용순**.
    구독은 확인한 사람마다 personal 링크를 하나씩 만든다. 아무거나 고르면 **연락처가 빈 고객**이 생긴다.
  - ★ 신원(`linksByConv`)은 **소스 칩과 무관하게** 읽는다. 여기에 필터를 걸면 "채팅" 칩에서 링크를 못 읽어
    겹침 보정이 0 이 되고, 전체에서 7 인 채팅이 칩을 누르면 9 로 튄다(실측·수정됨).
  - ★ 게스트가 남긴 이메일은 **영구 보관이 아니다** — 미확인 24시간, 회수·만료 후 30일에 지운다
    (`services/guestContactCleanup.js`, 개인정보처리방침에 쓴 문장의 구현). 실데이터에 "인증됐는데 이메일은 빈" 링크가 있는 이유다.
...
**자체 검증(Fable 미검증)**: 실HTTP — 집계 **912**(메일 903·채팅 7·게스트 2, 겹침 2 보정 후) ·
소스 칩별 숫자가 전체와 일치(`source=chat`→7 · `source=guest_link`→2, 네 경우 모두 합계 일치) ·
`limit=5` 로도 912 불변 · 답변 필요 48 전부 참 · 대화방 중복 0(고유 9/9) · 비소속 워크스페이스 403 ·
**등록 경로 실증** — 이메일을 남긴 게스트를 합성해 목록 노출(`email_verified:true`)→`save-as-client` **201**→그 행이 목록에서 빠짐(게스트 2→1) 확인 후 **데이터 원복**(잔여 고객 0·잔여 링크 0, 고객 수 16 동일. 감사 기록은 실제 일어난 일이므로 남긴다) ·
실브라우저 6/6(상담 탭 기본 · 칩 5종 · 액션 3종 · **게스트 대화 2건 모두 등록 버튼 있음** · 답변 필요만 48 · 고객 탭 "전체 16") ·
build EXIT 0 · `error TS` 0 · health-check 43/43 · guard-invariants EXIT 0(UI 규격 739/739)
...
# §N. 우측 패널 재설계 — **고객 기준 하나** (2026-09-13 Irene 지시, 미구현)

> Irene 원문:
> *"sale 리스트에 채팅이 아닌데 채팅보기가 나와. 이상한데로 보내고. 그리고 되돌리기 고객으로 등록은 뭐야?
> 등록하면 등록하는 거지 되돌리기는 뭐고 뭐 이딴 기능이 있는 거야? 고객으로 등록은 이메일로 고객이
> 제대로 들어와야 하는거지. 그리고 q sale 우측패널은 모두 동일해야 해. 상담에서 열든 고객에서 열든
> 우측 패널은 고객에서 연 창이야. 액션버튼도 모두 동일해야 하고. 어떻게 보든 무슨 상관이야.
> 상담 종류에 따라 다르고 고객 기준으로 저장하고 고객기준으로 응대하는 거야. 전체 히스토리보기는 뭐야.
> 다 나오는데 전체 히스토리 보기 해봤자 전체 보기로 갈 뿐이잖아. 전체 프로필에 나오는 거 다 우측패널에
> 나와야해. 그리고 수정 입력 추가 등 기능 우측패널에서 모두 되는 거야. 이메일이 그 고객이면 프로젝트가
> 연결되고 그 프로젝트랑 고객이 연결되는 거야. 여러 프로젝트 여러 이메일이 연결이 되면 각각 표시하면
> 되는 거야. 이메일이든 채팅이든 전화든 우측 패널은 해당 고객의 내용을 제대로 볼 수 있어야 해.
> 그리고 상담탭에서 열면 해당 내용을 문의정보로 상단에 다른 박스로 표시를 해줘. 아래 히스토리 중 내용이
> 위에 클릭한 내용을 보여주는 거지. 그리고 우측패널제목이랑 아래 나오는 거랑 왜 겹쳐? 이름 두번 나오고
> 그러잖아. 우측 패널 액션버튼들 정리 안되서 다 레이아웃 나가는 것도 그렇고."*

## N.1 핵심 계약 — 패널은 **한 벌**이고 기준은 **고객**이다

**어디서 열든 우측 패널은 "고객에서 연 창" 하나다.** 상담에서 열었다는 사실은 **상단 문의 박스 하나**로만
표현하고, 그 아래는 고객 패널 그대로다. 액션 버튼도 같다.

- **지금은 반대로 되어 있다** — `ClientPanel` 이 `isInquiry` 로 **두 벌**로 갈라져, 문의로 열면 고객 기능이
  통째로 없다(기록·다음 연락·업무 추가·연결·단계·청구 전부). 2026-09-13 에 확인필요에서도 이 패널을
  열게 했는데, 그것은 **갈라진 쪽을 한 곳 더 늘린 것**이다.
- 저장·응대의 단위는 **고객**이다. 상담 종류(메일·채팅·전화·게스트링크)는 **표시 축**일 뿐 화면을 가르지 않는다.
- ★ 미등록 문의에는 아직 `clients` 행이 없다. **이 지점이 설계의 급소다** — 선택지:
  ① 문의를 열면 **임시 고객 뷰**(아직 저장 안 됨)로 그리고 액션은 "먼저 등록이 필요합니다" 로 유도
  ② 문의를 여는 순간 `status='prospect'` 고객을 **자동 생성**(Q sale 은 이미 prospect 축이 있다)
  ③ 등록 전에는 문의 박스 + 읽기 전용 고객 골격
  **Irene 확인 필요.** ②는 한도(`prospects_max`)·자동 생성 원장에 영향이 있어 임의로 정하지 않는다.

## N.2 항목별 (신고 ↔ 코드 위치)

| # | 신고 | 원인 위치 | 해야 할 것 |
|---|---|---|---|
| 1 | 채팅이 아닌데 **"채팅에서 보기"**, 엉뚱한 곳으로 | `SaleInboxList.tsx:266` — `it.source === 'email' ? 'mail' : 'chat'` **이분법**. `client`·`phone`·`guest_link` 가 전부 "채팅" 으로 떨어진다. 경로는 `open_path`(`/sale/:id`·`/talk?conv=`·`/mail?thread=`)라 라벨과 목적지가 어긋난다 | 라벨을 `source`(또는 `ref.kind`) **전수 분기**로. 새 값이 기본값으로 떨어지지 않게(상태값 규약). 라벨과 `open_path` 를 **같은 곳에서** 정한다 |
| 2 | **"되돌리기"** 가 왜 있나 | `SaleInboxList.tsx` `restoreItem` — 보관함(dismiss) 되돌리기. `[문의 아님]` 으로 치운 것을 복구하는 용도인데 **"고객으로 등록" 옆에 나란히** 있어 등록의 취소로 읽힌다 | 보관함 탭 안으로 격리하거나 제거. **등록에는 취소가 없다**는 것이 분명해야 한다 |
| 3 | 고객으로 등록하면 **이메일이 제대로 들어와야** | `routes/sale_save.js` — 등록 시 `clients.email` 이 문의 이메일로 채워지는지 **실측 미확인** | 등록 → 고객 상세에서 이메일·이름·회사가 그대로 보이는지 실HTTP 왕복으로 증명 |
| 4 | 패널이 상담/고객에서 다르다 | `ClientPanel.tsx:194` `isInquiry` 분기 (본문 255~·푸터 419~) | **분기 제거.** 문의 정보는 상단 박스로, 나머지는 고객 패널 하나 |
| 5 | **"전체 히스토리 보기"** 무의미 | `ClientPanel.tsx:174` "히스토리는 최근 몇 건만, 전체는 상세에서" | 패널에서 **전체를 본다**(더 보기/무한스크롤). 페이지로 보내는 버튼 제거 |
| 6 | 전체 프로필 내용이 패널에 다 있어야 · **수정·입력·추가도 패널에서** | `pages/QSale/SaleDetailPage.tsx` ↔ `ClientPanel.tsx` | 전체 프로필의 섹션을 패널이 **같은 컴포넌트로** 전부 쓴다. 읽기 전용으로 두지 않는다 |
| 7 | 이메일 ↔ 프로젝트 ↔ 고객 연결, **여러 개면 각각 표시** | `ClientLinksSection` · `clients` 이메일 축 | 이메일이 여러 개·프로젝트가 여러 개면 **각각 행으로**. 하나로 접지 않는다(2026-09-12 "이메일 셋을 하나로 접어" 와 같은 계열) |
| 8 | 메일·채팅·전화 무엇이든 그 **고객 내용**이 제대로 | 히스토리 수집 축 | 채널별로 화면을 가르지 말고 고객 타임라인 하나로 |
| 9 | 상담에서 열면 **문의 정보를 상단 별도 박스**로, 아래 히스토리 중 그 건을 보여준다 | 신규 | 상단 박스 = 클릭한 그 문의. 아래 = 고객 전체 히스토리(그 건이 선택/강조) |
| 10 | **제목과 본문 이름이 겹친다**(이름 두 번) | `ClientPanel.tsx:244-260` 헤더 제목 + `268-270`/`308-310` 본문 `LetterAvatar`+`TopName` | 하나로. 헤더에 이름을 두면 본문 Top 블록은 아바타+보조정보만 |
| 11 | 액션 버튼이 안 정리돼 **레이아웃이 나간다** | `ClientPanel.tsx` 푸터 — `ActionButton` **19곳** | CLAUDE.md 상세 밴드 계약과 같은 원칙: 자주 쓰는 3개 이내 + 나머지는 `OverflowMenu`. 푸터가 줄바꿈으로 커지지 않게 |

## N.3 확인해야 할 것 (구현 전)
- N.1 의 ①②③ 중 무엇으로 갈지 **Irene 확인** (미등록 문의에 고객 행이 없다)
- 패널이 전체 프로필 전부를 담으면 폭 440px 로 충분한지 — 폰(100vw)·태블릿(min(560,90vw)) 포함
- 2026-09-13 에 확인필요(Todo)에 붙인 문의 패널도 **같이** 바뀐다(같은 컴포넌트를 쓰므로 자동이지만,
  `drawer.inquiry` 페이로드가 고객 패널이 필요로 하는 값을 다 싣는지는 다시 봐야 한다)

## N.5 구현 (2026-09-13) — 무엇을 어떻게 했나

**Irene 결정 (N.1 의 ①②③ 중)**: **"첫 액션에서 자동 등록"**.
여는 것만으로는 아무것도 만들지 않는다 — 저장이 필요한 액션을 처음 누를 때 한 번 묻고 `prospect` 를 만든 뒤
**그 액션을 이어서** 실행한다. 목록을 훑기만 해도 원장이 쌓이는 ②를 피하면서 "액션버튼 모두 동일" 을 지킨다.

- **문은 하나** — `ClientPanel` 의 `withClient(run)`. 액션마다 이 흐름을 베끼지 않는다.
- ★ **자동 등록은 초대 메일을 보내지 않는다.** `services/saleRegister.registerInquiryAsClient` 에
  `{ invite }` 를 냈다. 기록을 남겼을 뿐인데 고객에게 메일이 나가면 안 된다(CLAUDE.md "외부 발송은 확인을 받는다").
  명시적 [고객으로 등록] 만 `invite: true` 이고, 지금처럼 **보낼 주소를 문구에 적어** 묻는다.
- 등록되면 패널은 **그 자리에서** 고객 쪽으로 채워진다(`autoId`). 대상이 바뀌면 잊는다 — 안 잊으면
  다음 문의에 앞 고객의 원장이 붙는다.

| 신고 | 한 것 |
|---|---|
| 1 채팅 아닌데 "채팅 보기" | `viewInLabel()` — `ref.kind` **전수 분기**. 라벨과 `open_path` 를 같은 곳에서. 모르는 kind 는 kind 이름 그대로 보여 **기본값으로 숨지 않는다** |
| 2 "되돌리기" | 보관함 행에만 있던 것은 그대로, 문구를 **"상담으로 되돌리기"** 로. 등록에는 취소가 없다 |
| 3 등록 시 이메일 | 실HTTP 왕복 **10/10** — `invite_email`·상세 `email`·목록 행 `email` 이 전부 문의 주소. 등록자·시각도 남는다 |
| 4 패널이 두 벌 | `isInquiry` 분기 **제거**. 문의는 상단 박스, 아래는 고객 패널 그대로 |
| 5 "전체 히스토리 보기" | 패널에서 **전체를 본다**(`before` 커서 + 더 보기). 페이지로 보내는 링크 제거. `balanced` 도 뺐다 — 커서에서는 건너뛴 항목이 커서를 거짓으로 만든다 |
| 6 전체 프로필이 패널에 · 수정도 | `ProfileForm` 을 `components/QSale/ClientProfileForm.tsx` 로 **추출**해 전체 프로필과 패널이 같은 폼을 쓴다(AutoSaveField 그대로, `key` 로 대상 구분) |
| 7 이메일·프로젝트 각각 | 이메일은 종류별 행, 프로젝트는 `ClientLinksSection`(전체 프로필과 같은 컴포넌트) |
| 8 채널 무관 고객 내용 | 타임라인 하나. 채널로 화면을 가르지 않는다 |
| 9 문의 정보 상단 박스 | `client-panel-inquiry-box` — 종류·답변필요 칩 · 제목 · 내용 · 주소(확인여부) · 시각 · [보기] |
| 10 이름 두 번 | 이름은 **헤더에만**. 본문 Top 은 아바타 + 회사·단계만 |
| 11 액션 레이아웃 | `ActionButton` **19 → 4**(기록·다음 연락·업무 + `OverflowMenu`). 푸터 실측 **65px 한 줄** |

**회귀 카나리 신설**: `node scripts/e2e/run.js --suite salepanel` — 문의/고객 양쪽 액션 동수 · 이름 중복 0 ·
푸터 높이 · 확인창 게이트(확인 전 POST 0건) · **패널 열린 채 뒤 목록 스크롤** + 측정 전제(패널 열림·백드롭 덮음).

## N.4 (추가 지시 2026-09-13) — **우측 패널이 열려도 뒤 화면은 스크롤된다**

> Irene: *"우측패널들 열려있어도 좌측에 나오는 화면은 스크롤 반응하게 해. 고정할 필요 있어?
> 어둡게 했으면 됐지. 리스트들이든 뭐든 스크롤 반응은 막지마"*

**이것은 Q sale 만의 이야기가 아니라 우측 드로어 전체의 계약 변경이다.**

- 지금 계약: CLAUDE.md "드로어 접근성" 이 `useBodyScrollLock(open)` 을 **필수**로 못 박고 있고,
  프리미티브 `components/Common/DetailDrawer` 가 내장한다. **적용처 18개 화면.**
- 바뀔 계약: **우측 곁패널(side drawer)은 뒤 화면 스크롤을 잠그지 않는다.** 백드롭으로 어둡게 하는 것으로 족하다.
  목록을 보면서 패널을 함께 보는 것이 이 패널의 용도다 — 잠그면 "옆 목록에서 다음 것을 찾는" 동선이 끊긴다.
- **가운데 모달(`UI/Modal`)은 이 변경에서 제외한다** — 전면을 덮는 창은 뒤가 움직이면 안 된다.
  Irene 이 말한 것은 "우측패널들" 이다. 모달까지 풀면 2026-09-13 에 고친 접근성(배경 스크롤 차단)이 되돌아간다.
- 손댈 곳: `components/Common/DetailDrawer.tsx`(프리미티브 한 곳) + 직접 그린 드로어들.
  `CLAUDE.md` 의 "body 스크롤 잠금 필수" 문장도 같이 고쳐야 한다 — **문서를 안 고치면 다음 사람이 되돌린다.**
- 주의: 스크롤 잠금을 풀면 **백드롭이 스크롤을 먹는지** 확인해야 한다(백드롭에 `overflow:hidden`·
  `touch-action:none` 이 있으면 폰에서 여전히 안 움직인다). 그리고 패널 자체의 스크롤과 뒤 화면 스크롤이
  **서로 먹지 않는지**(scroll chaining) 실기기 폭에서 잰다.
- 검증: 패널 연 상태에서 뒤 목록 `scrollTop` 이 실제로 변하는가(음성 대조군 = 닫힌 상태) · 폰·태블릿·데스크탑 3폭.
