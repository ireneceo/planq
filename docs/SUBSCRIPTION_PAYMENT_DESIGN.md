# 구독결제 — 카드 정기결제 연동 설계 (PG 메커니즘)

> ⚠️ **2026-07-07 갱신: PortOne 폐기 → 토스페이먼츠 직접 연동으로 교체 예정, 착수 보류.** 아래 "PortOne" 서술은 향후 토스 기준으로 읽을 것(빌링키·webhook·markPaymentPaid 단일 착지 원칙은 동일). 보류 사유: 토스 연동에 가입비 220,000+연관리비 110,000(연1회)+심사 요구 → 지금 결제 안 함.
> 🔒 **SaaS 구독결제 ↔ Q Bill 분리 canonical 은 별도 문서 `SAAS_BILLING_VS_QBILL_SEPARATION.md` (혼동 방지·불변식·Fable 체크리스트).**
> 현재 라이브 수단: **송금(무통장입금) 단일.** 이 문서는 후속 카드 PG 메커니즘.

> 작성 2026-07-07 (Opus). 상태: **설계만** (PG 미착수, 배포 없음).
