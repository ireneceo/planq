...
>
> **★ Fable 설계검증(CONDITIONAL PASS) 반영 v2** — 아래 §정정 참조. FK 실태 교정(RESTRICT 39개, 78 아님 / audit_logs.user_id 는 SET NULL). 초판의 D1·D2·D5·익명화범위에 치명 결함 5건이 있었다.
...
- **유예 중 복구는 공개 라우트로만 가능** (Fable 🔴2 — auth.js:58/511 이 deleted 를 403 처리해 인증 토큰을 못 얻으므로, 인증 필요한 deletion-cancel 은 자기모순):
  - `POST /api/auth/deletion-recover` **(공개)** — 이메일+비밀번호(OAuth 는 이메일 OTP, D9) 재인증 → `status='deleted' && deletion_scheduled_at > NOW()` 이면 `status='active'`, 예약 NULL
  - 로그인 라우트(auth.js:511)에 `status='deleted' && 유예중` 특례: 403 대신 **`code:'account_deleted_pending'`** + 복구 안내(프론트가 suspended 와 구분해 "복구하시겠습니까" 표시). suspended 는 기존 `account_suspended` 유지
...
**users**: `name` → `탈퇴한 사용자`, `email` → `deleted-{id}@deleted.planq.kr`(UNIQUE 충돌 방지), `username` → `deleted_{id}`, 그리고 NULL 처리: `phone`·`avatar_url`·`bio`·`secondary_email`·`pending_email`·`pending_secondary_email`·`name_localized`·`expertise`·`organization`·`job_title`·`language_levels`·`answer_style_default`·`timezone`·`reference_timezones`. 비밀번호 해시 무효화. `refresh_token`·`*_reset_token`·`*_otp_hash` 전량 NULL. **avatar 물리 파일 삭제**(URL NULL 만으론 부족).
...
**clients** (Client 는 users row 를 가짐 — `clients.user_id` FK 실존): 탈퇴자가 Client 로 연결된 row 의 `display_name` → `탈퇴한 고객`, `display_name_localized`·`invite_email` → NULL. (Client 역할 탈퇴 시나리오 — 초판 누락)
...
1. 유일 owner + (다른 human 멤버 OR client OR 미수금 invoice OR 활성 구독) 있는 워크스페이스 → `transfer_required`
2. 유일한 활성 platform_admin → `last_platform_admin` (Fable 보완 — 관리 주인 없음 방지)
...
## 초판 대비 정정 요약 (Fable 🔴 5건)
1. 🔴1 솔로 워크스페이스 탈퇴 불가 → **D8 동반 soft-delete**
2. 🔴2 유예 복구 자기모순 → **공개 라우트 + account_deleted_pending code**
3. 🔴3 OAuth 계정 탈퇴 불가 → **D9 이메일 OTP**
4. 🔴4(=D5) 미수금 차단 리젝위험 → **경고로 강등**
5. 🔴5 익명화 구멍 → **business_members + clients + users 잔여컬럼 + avatar 물리파일**
