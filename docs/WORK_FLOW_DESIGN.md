...
> # ⛔ 동결 — 바꾸기 전 무조건 Irene 확인 (2026-08-24)
>
> **가용시간 카드와 주간 진척 그래프는 지금 상태가 정본이다.**
> Irene: *"가용시간과 진척그래프는 지금 보이는 거 제대로 된 것 같아 함부로 바꾸지 말고
> 무조건 나에게 확인하는 걸로 해."*
>
> 계산식·집합·라벨·축 무엇이든 손대기 전에 **먼저 묻는다. "고치는 김에" 도 금지.**
> 다른 기능을 고치다 이 값이 따라 바뀌는 경우(예: `loadBreakdown` / `chartWeekTasks` 를 읽는 코드)도
> 보고 대상이다.
>
> 배경: 2026-08-24 하루에 이 두 화면을 네 번 고쳤고, 그중 한 번은 "통일" 하겠다며
> `담당자=나` 조건을 임의로 제거했다가 정정 지시를 받았다. Irene 이 같은 화면을 반복해서
> 검산해야 했다. memory `feedback_capacity_graph_frozen`.
...
| 선 | 정의 | EVM 대응 |
|----|------|---------|
| **계획 페이스(대각선 점선)** | 0 → **계획(Σ예측시간 = weekTotalEst)**, 영업일 균등 페이스. **종점이 가로 가용선과 만나는 지점은 주마다 다르다** (계획이 가용보다 많을 수도 적을 수도 있고, 그 차이가 곧 읽을거리다) | — (계획선) |
| **PV (판정 전용, 비표시)** | Σ예측 × 경과영업일/총영업일 — 판정칩 SPI 계산에만 사용, 선으로 그리지 않음 | PV |
| **진척 (예측시간)** | Σ(**예측시간 × 진행률**) — 예측 기준으로 한 만큼 | EV (획득가치) |
| **실제 업무시간** | Σ(**실제시간 × 진행률**) — 실제 기록 기준으로 한 만큼 | AC (실제비용) |

> **★ 집합 규칙 (2026-08-24 Irene 확정 — 바꾸지 말 것)**
> - **그래프·카드의 집합 = `이번 주 정본(inWeekCanonical) ∩ 담당자=나`.** Irene: "담당자=나 당연하지.
>   리스트에 담당자 나 아닌 걸 왜 내 업무정보에 넣어." 내가 컨펌자로만 관여한 남의 업무는 리스트에는
>   보이지만 **내 시간·내 진척 수치에는 넣지 않는다.** (이 조건을 제거하려던 시도가 한 번 있었고 정정됐다.)
> - **계획·진척은 완료 업무를 포함한다.** 완료했다고 계획이 줄면 안 된다(2026-08-21 확정).
>   완료 제외는 **남은 일** 계산에만 적용한다. 항등식: `진척 + 남은 = 계획`.
> - **오늘 나의 업무** = 이번 주 정본 위에 날짜 필터. 완료 업무는 **어디서 눌렀든 `completed_at`(워크스페이스
>   tz)이 오늘이면 오늘 집합에 들어온다** — 완료에 다른 조건을 걸지 않는다(`utils/todayTaskSet.ts` 정본).
> - **완료 해제 = 진행률 0.** 리스트 체크박스와 팝아웃 체크가 같은 결과여야 한다
>   (단일 착지점 `task_actions.revertStatus`).
>
> **★ 2026-08-24 Irene 확정 — 두 선은 같은 축(진행률) 위에 있고, 재는 자만 예측/실제로 다르다.**
> 옛 정의는 실제선만 raw 누적이라 축이 서로 달랐고, 거기에 기준선 차감(Δ)까지 얹혀 있었다.
> **기준선 차감(Δ)은 두 선에서 걷어냈다.** 반증(운영 실측 #385, lua/워프로랩 2026-08-24):
> 실제시간을 **아래로 정정**하면(5h → 1h) `Δ = max(0, 1 − 5) = 0` 이라, 진행률 100% 인 업무가
> 그 주 내내 실제선에 **0 만** 기여했다. 정정은 과거 기록의 수정이지 노동의 취소가 아니다.
> 부수 효과로 그래프가 목록(절대값)과 같은 축을 갖는다.
> `services/progressBaseline.js` 의 `estDoneOf` / `actDoneOf` 가 **정의 정본**이고,
> 라이브 그래프(`routes/tasks.js daily-progress`) · 보고서(`reportUnitSnapshot`) · 개인 주간보고
> (`weeklyReviewSnapshot`) · 프론트 오늘 점(`QTaskPage`) **네 표면이 같은 공식**을 쓴다.
> `getProgressBaselines` 는 `estNow`(예측 정정 면역)와 되돌림 판정에만 남는다.
> 포커스 실측도 같은 축으로 환산한다(raw 시간을 더하면 오늘 점만 단위가 섞인다).

> **y축 = max(계획 페이스 종점, 가용, 그려진 누적 최댓값)** — 가용선을 반드시 포함한다. 계획이 가용보다 작은 주에 가로 가용선이 잘리면 "계획이 가용을 넘었는지" 를 눈으로 못 읽는다. 판정칩 SPI는 계획(Σ예측) 기준을 유지한다. 옛 "0→Σ잔여" 정의는 코드에 구현된 적 없어 삭제.
...
- **carry-in 진행분:** ~~이번 주 델타로 그려 월요일 0에서 시작~~ → **2026-08-24 폐기.** 델타는 하향 정정을 0 으로 클램프해 진행률 100% 인 업무를 그래프에서 지웠다(#385). 지금은 두 선 모두 `× 진행률` 절대값이라 이월 업무의 진행분도 선에 실린다 — 카드(누적)와 그래프가 **같은 기준**이 된다.
...
> **개정 이력 — 그래프 두 선 + 페이스 대각선 (2026-08-24, Irene 지시)**
> - **진척(예측시간) = 예측시간 × 진행률 / 실제 업무시간 = 실제시간 × 진행률** — 두 선이 같은 축.
> - **기준선 차감(Δ) 폐기** — 하향 정정이 영구 0 클램프를 만들던 구멍(#385).
> - **페이스 대각선 종점을 가용시간 → 계획(Σ예측시간)** 으로. Irene: "끝이 가용시간과 만나는 지점이
>   다를 수 있는 건데 왜 이렇게 고정되어 있어?" 옛 정의(2026-07-05)는 종점을 가용에 고정해,
>   계획이 가용과 다른 주에도 늘 같은 각도로 그려졌다. 범례도 `가용 페이스` → **`계획 페이스`** 로.
>
> **개정 이력 — 패널의 큰 수 (2026-08-21 → 2026-08-24, Irene 지시)**
> 아래 ① 의 원안은 `남은 일 / 가용` + `여유` 였다. 두 번 바뀌었고 **현재 코드는 아래와 다르다.**
> - 2026-08-21 — 큰 수를 **진척** 으로 뒤집고 `여유` 행 삭제. 이유: 남은 일 기준은 **완료할수록 줄어들어**
>   "이번 주에 채워야 할 24h" 라는 뜻과 반대로 움직였다(Irene: "완료하면 시간이 줄어들어버리면 안되지").
>   여유(가용−남은 일)는 이미 쓴 시간을 빼지 않아 위 줄과 서로 어긋나 삭제. 초과 경고는 상단 요약 칩이 맡는다.
> - 2026-08-24 — 진척의 기준을 **이번 주 증가분(Δ)** 에서 **그 집합의 누적(Σ예측×진행률, 이월 포함)** 으로.
>   이유: 주가 바뀌는 순간 Δ 는 구조적으로 0 이 되는데 같은 카드의 `남은 일` 은 이월을 포함한 누적이라
>   **분자만 리셋되고 분모는 안 되는** 상태였다. 운영 실측(8/24 00:22 KST): 남은 57.7h · 실제 13.5h 인데 진척 0.0h.
>   같은 시점 진행률>0 인 활성 업무 6건의 누적 진척 17.5h 가 전부 화면 밖에 있었고, 이번 주 신규 22건은
>   Σ예측 44.7h 에 진행률 전부 0 이라 그 항목만으로는 0 이 될 수밖에 없었다.
>   ~~§6.1 그래프는 그대로 Δ~~ → **2026-08-24 Irene 지시로 그래프도 Δ 를 버렸다.** 카드와 그래프가
>   이제 같은 기준(누적)이다. 카드의 `이번 주 새로 {h}h · 이월 진척 {h}h` 보조 줄은 그대로 둔다.
> - **미해결(Fable 대기)**: 분자가 누적이 되면 `진척/가용` 이 100% 를 넘을 수 있다
>   (`DEVELOPMENT_PLAN.md` 의 "진척/가용 439% · 기간 환산 누락" 과 같은 뿌리). 바 폭은 100% 로 캡되지만
>   퍼센트 칩은 실수치를 그대로 낸다. 분모를 가용시간으로 둘지, 그 집합의 Σ예측으로 둘지가 미결이다.
