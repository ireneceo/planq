...
| 선 | 정의 | EVM 대응 |
|----|------|---------|
| **계획 페이스(대각선 점선)** | 0 → **계획(Σ예상시간 = weekTotalEst)**, 영업일 균등 페이스. **종점이 가로 가용선과 만나는 지점은 주마다 다르다** (계획이 가용보다 많을 수도 적을 수도 있고, 그 차이가 곧 읽을거리다) | — (계획선) |
| **PV (판정 전용, 비표시)** | Σ예측 × 경과영업일/총영업일 — 판정칩 SPI 계산에만 사용, 선으로 그리지 않음 | PV |
| **진척 (예상시간)** | Σ(**예측시간 × 진행률**) — 예측 기준으로 한 만큼 | EV (획득가치) |
...
> - **진척(예상시간) = 예측시간 × 진행률 / 실제 업무시간 = 실제시간 × 진행률** — 두 선이 같은 축.
> - **기준선 차감(Δ) 폐기** — 하향 정정이 영구 0 클램프를 만들던 구멍(#385).
> - **페이스 대각선 종점을 가용시간 → 계획(Σ예상시간)** 으로. Irene: "끝이 가용시간과 만나는 지점이
