...
### Phase 2 — 워크스페이스 간 이전 (2026-06-23 완료, 복사 방식)
- 결정: **복사(원본 유지)**. 이전 = 본인 L1 파일 + 본인 작성 문서를 본인이 멤버인 다른 워크스페이스로 복사. 원본 워크스페이스 자료는 그대로.
- API: `GET /:biz/me/transfer-targets`(본인 멤버 다른 워크스페이스) · `POST /:biz/me/transfer { target_business_id }`(복사 실행). 양쪽 멤버 검증, content_hash dedup(타겟 물리파일 공유 또는 복사), 타겟 쿼터 갱신, 본인 중복 보유 skip, AuditLog `data.transfer.self`. 1회 최대 1000 파일·1000 문서.
- UI: DataExportSettings "다른 워크스페이스로 이전" 카드(PlanQSelect + ConfirmDialog).
- 검증: 실복사 E2E PASS(biz5→biz73 복사·원본 유지)·권한 403·검증 400·미인증 401. commit 83737db.

### Phase 3 (미착수 — 향후)
- 타겟 워크스페이스 자료 소유권 완전 이양(이동=원본 삭제) 옵션. Q note 메모(/qnote API) 포함. 비동기 대용량 export(job + 다운로드 링크).
